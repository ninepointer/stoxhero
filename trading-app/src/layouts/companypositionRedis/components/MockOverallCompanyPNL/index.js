...
function MockOverallCompantPNL({ socket }) {
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/infinityRedis/mock/overallcompanypnltoday`)
      .then((res) => {
    ...
    axios
      .get(`${baseUrl}api/v1/infinityRedis/mock/letestTradeCompany`)
      .then((res) => {
      ...
      {rows.length === 1 ? (
        ...
        >
          ...
          <Typography mb={2} fontSize={15} color="grey">
            Active mock trades will show up here.
          </Typography>
        </MDBox>
      ) : (
        <MDBox>
