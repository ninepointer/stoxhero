...
export default function UserTodayTradeData() {
  ...
  useEffect(()=>{
    ...
    axios.get(`${baseUrl}api/v1/todayXTS?skip=${skip}&limit=${limitSetting}`,{
...
function backHandler(){
  ...
  setData([]);
  setIsLoading(true)
  axios.get(`${baseUrl}api/v1/todayXTS?skip=${skip-limitSetting}&limit=${limitSetting}`,{
      withCredentials: true,
      headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
    ...
    <MDBox bgColor="dark" color="light" mb={1} borderRadius={10} minWidth='100%' minHeight='auto'>
      <Grid container spacing={1}>
        <Grid container p={1} style={{ border: '1px solid white', borderRadius: 5 }}>
          ...
          <Grid item xs={12} md={2} lg={2} display="flex" justifyContent="center" alignContent="center" alignItems="center">
            <MDTypography color="light" fontSize={13} fontWeight="bold">Exchange Order Id</MDTypography>
          </Grid>
          ...
          <Grid item xs={12} md={2} lg={1}>
            <MDTypography color="light" fontSize={13} fontWeight="bold" display="flex" justifyContent="center" alignContent="center" alignItems="center">Placed By</MDTypography>
          </Grid>
          ...
          data?.map((elem) => {
            ...
            return (
              ...
              <Grid container mt={1} p={1} style={{ border: '1px solid white', borderRadius: 5 }}>
                ...
                <Grid item xs={12} md={2} lg={.75} display="flex" justifyContent="center" alignContent="center" alignItems="center">
                  <MDTypography color="light" fontSize={10} fontWeight="bold">₹{new Intl.NumberFormat(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Math.abs(elem?.average_price * elem?.quantity))}</MDTypography>
                </Grid>
                <Grid item xs={12} md={2} lg={.75} display="flex" justifyContent="center" alignContent="center" alignItems="center">
                  <MDTypography color={typecolor} fontSize={10} fontWeight="bold">{elem?.transaction_type}</MDTypography>
                </Grid>
                <Grid item xs={12} md={2} lg={2} display="flex" justifyContent="center" alignContent="center" alignItems="center">
                  <MDTypography color="light" fontSize={10} fontWeight="bold">{elem?.exchange_order_id ? elem?.exchange_order_id : "N/A"}</MDTypography>
                </Grid>
                <Grid item xs={12} md={2} lg={1} display="flex" justifyContent="center" alignContent="center" alignItems="center">
                  <MDTypography color="light" fontSize={10} fontWeight="bold">{elem?.status}</MDTypography>
                </Grid>
                <Grid item xs={12} md={2} lg={1} display="flex" justifyContent="center" alignContent="center" alignItems="center">
                  <MDTypography color="light" fontSize={10} fontWeight="bold">{elem?.placed_by}</MDTypography>
                </Grid>
                <Grid item xs={12} md={2} lg={1.75} display="flex" justifyContent="center" alignContent="center" alignItems="center">
                  <MDTypography color="light" fontSize={10} fontWeight="bold">{moment.utc(elem?.order_timestamp).utcOffset('+00:00').format('DD-MMM HH:mm:ss')}</MDTypography>
                </Grid>
              </Grid>
