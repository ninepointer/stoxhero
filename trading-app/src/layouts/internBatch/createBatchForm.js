...
function Index() {
  ...
  const [type, setType] = useState(
    id?.portfolio?.portfolioName.includes("Workshop") ? "Workshop" : "Job"
  );

  const [formState, setFormState] = useState({
    batchName: "" || id?.batchName,
    batchStartDate:
      dayjs(id?.batchStartDate) ??
      dayjs(new Date()).set("hour", 0).set("minute", 0).set("second", 0),
    batchEndDate:
      dayjs(id?.batchEndDate) ??
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/portfolio/internship`, { withCredentials: true })
      .then((res) => {
    ...
    axios
      .get(`${baseUrl}api/v1/career/all?type=${type}`, {
    ...
    axios
      .get(`${baseUrl}api/v1/internbatch/${id?._id}`, { withCredentials: true })
      .then((res) => {
        setBatch(res?.data?.data);
        setFormState({
          batchName: "" || res?.data?.data?.batchName,
          batchStartDate:
            dayjs(res?.data?.data?.batchStartDate) ??
            dayjs(new Date()).set("hour", 0).set("minute", 0).set("second", 0),
          batchEndDate:
            dayjs(res?.data?.data?.batchEndDate) ??
            dayjs(new Date())
              .set("hour", 23)
              .set("minute", 59)
              .set("second", 59),
          ...
          orientationDate:
            dayjs(res?.data?.data?.orientationDate) ??
            dayjs(new Date()).set("hour", 0).set("minute", 0).set("second", 0),
          ...
          consolationReward: {
            ...
            minAttendance:
              "" || res?.data?.data?.consolationReward?.minAttendance,
          },
        });
        setTimeout(() => {
          setIsLoading(false);
  ...
  async function onSubmit(e, formState) {
    ...
    if (
      (type == "Job" &&
        !formState?.portfolio.name.toLowerCase().includes("internship")) ||
      (type == "Workshop" &&
        !formState?.portfolio.name.toLowerCase().includes("workshop"))
    ) {
      return openErrorSB(
        "Wrong Portfolio",
        "Please check the portfolio and type compatibility"
      );
    }
  ...
  async function createGD(e, childFormState, setChildFormState) {
    ...
    setSaving(true);
    if (
      !childFormState?.gdTitle ||
      !childFormState?.gdTopic ||
      !childFormState?.meetLink ||
      !childFormState?.gdStartDate ||
      !childFormState?.gdEndDate
    ) {
      setTimeout(() => {
        setCreating(false);
        setIsSubmitted(false);
    ...
    const { gdTitle, gdTopic, meetLink, gdStartDate, gdEndDate } =
      childFormState;
            ...
            <Grid container spacing={2} mt={0.5} mb={0} xs={12} md={9} xl={12}>
              ...
              <Grid item xs={12} md={6} xl={3}>
                <TextField
                  ...
                  fullWidth
                  defaultValue={
                    editing ? formState?.referralCount : id?.referralCount
                  }
                  onChange={handleChange}
                />
              </Grid>
              ...
              <Grid item xs={12} md={6} xl={3}>
                <TextField
                  ...
                  fullWidth
                  defaultValue={
                    editing ? formState?.payoutPercentage : id?.payoutPercentage
                  }
                  onChange={handleChange}
                />
              </Grid>
                  ...
                  <DemoContainer components={["MobileDateTimePicker"]}>
                    <DemoItem>
                      <MobileDateTimePicker
                        ...
                        value={
                          formState?.batchStartDate ||
                          dayjs(batch?.batchStartDate)
                        }
                        onChange={(newValue) => {
                          if (newValue && newValue.isValid()) {
                            setFormState((prevState) => ({
                              ...prevState,
                              batchStartDate: newValue,
                            }));
                          }
                        }}
                        minDateTime={null}
                  ...
                  <DemoContainer components={["MobileDateTimePicker"]}>
                    <DemoItem>
                      <MobileDateTimePicker
                        ...
                        disabled={(isSubmitted || id) && (!editing || saving)}
                        value={
                          formState?.batchEndDate || dayjs(batch?.batchEndDate)
                        }
                        onChange={(newValue) => {
                          if (newValue && newValue.isValid()) {
                            setFormState((prevState) => ({
                              ...prevState,
                              batchEndDate: newValue,
                            }));
                          }
                        }}
                        minDateTime={null}
                  ...
                  <DemoContainer components={["MobileDateTimePicker"]}>
                    <DemoItem>
                      <MobileDateTimePicker
                        ...
                        value={
                          formState?.orientationDate ||
                          dayjs(batch?.orientationDate)
                        }
                        onChange={(newValue) => {
                          if (newValue && newValue.isValid()) {
                            setFormState((prevState) => ({
                              ...prevState,
                              orientationDate: newValue,
                            }));
                          }
                        }}
                        minDateTime={null}
                ...
                <Grid item xs={12} md={6} xl={3} mt={-2}>
                  <FormControl sx={{ minHeight: 10 }}>
                    ...
                    <Select
                      ...
                      MenuProps={MenuProps}
                    >
                      <MenuItem value="Job">Internship</MenuItem>
              ...
              <Grid item xs={12} md={3} xl={3} mt={-2}>
                <FormControl sx={{ minHeight: 10, width: "100%" }}>
                  ...
                  <Select
                    ...
                    // defaultValue={id ? portfolios?.portfolio : ''}
                    value={
                      formState?.portfolio?.portfolioName ||
                      batch?.portfolio?.portfolioName
                    }
              ...
              <Grid item xs={12} md={3} xl={3} mt={-2}>
                <FormControl sx={{ minHeight: 10, width: "100%" }}>
                  ...
                  <Select
                    ...
                    // defaultValue={id ? portfolios?.portfolio : ''}
                    value={
                      formState?.career?.jobTitle || batch?.career?.jobTitle
                    }
              ...
              <Grid item xs={12} md={6} xl={3}>
                <TextField
                  ...
                  defaultValue={
                    formState?.consolationReward?.minAttendance ||
                    id?.consolationReward?.minAttendance
                  }
                  onChange={(e) => {
                    setFormState((prevState) => ({
                      ...prevState,
          ...
          <Grid container mt={2} xs={12} md={12} xl={12}>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} md={6} xl={3}>
                    ...
                    <MDButton
                      ...
                      size="small"
                      onClick={(e) => {
                        createGD(e, childFormState, setChildFormState);
                      }}
                    >
                      Create GD
                    </MDButton>
            ...
            {(id || newObjectId) && (
              <Grid item xs={12} md={12} xl={12} mt={2} mb={2}>
                <MDBox>
                  <BatchParticipants
                    batch={newObjectId ? newObjectId : id?._id}
                    action={action}
                    setAction={setAction}
                  />
                </MDBox>
