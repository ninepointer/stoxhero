...
export default function Participants({ batch, action, setAction }) {
  ...
  async function getBatchParticipants() {
    let call1 = axios.get(
      `${baseUrl}api/v1/internbatch/batchparticipants/${batch}`,
      {
  ...
  const handleRemove = async (userId) => {
    const res = await axios.patch(
      `${apiUrl}internbatch/remove/${batch}/${userId}`,
      {},
      { withCredentials: true }
    );
  ...
  batchParticipants?.map((elem, index) => {
    ...
    featureObj.joinedon = (
      <MDTypography
        ...
        fontWeight="medium"
      >
        {new Date(elem?.joiningDate).toLocaleDateString("en-IN", {
          timeZone: "Asia/Kolkata",
        })}{" "}
        {new Date(elem?.joiningDate)
          ...
          >
            <Tooltip title="Download CSV">
              <MDBox
                ...
                onClick={() => {
                  handleDownload(pnlData, `selected-candidates`);
                }}
              >
                <DownloadIcon />
              </MDBox>
