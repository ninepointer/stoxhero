...
export default function InfinityMining() {
    ...
    setTimeout(() => {
      ...
      html2canvas(screenshotElement)
        .then((canvas) => {
          ...
          link.download = `${
            traderId?.first_name + " " + traderId?.last_name
          }.png`;
  ...
  useEffect(() => {
    ...
    Promise.all([call2])
      .then(([api1Response2]) => {
        ...
        setTraderSelectedOption(
          traderSelectedOption
            ? traderSelectedOption
            : api1Response2.data.data[0]
        );
        setTraderId(
          traderSelectedOption
            ? traderSelectedOption
  ...
  async function InfinityMining(traderId) {
    ...
    let call1 = await axios.get(
      `${baseUrl}api/v1/infinitymining/traderstats/${traderId?._id}`,
      {
        withCredentials: true,
        headers: {
          Accept: "application/json",
    ...
    let call3 = await axios.get(
      `${baseUrl}api/v1/infinitymining/tradertradesoverview/${traderId?._id}`,
      {
        withCredentials: true,
        headers: {
          Accept: "application/json",
    ...
    let call4 = await axios.get(
      `${baseUrl}api/v1/infinitymining/bothtradesdata/${traderId?._id}`,
      {
        withCredentials: true,
        headers: {
          Accept: "application/json",
    ...
    let call5 = await axios.get(
      `${baseUrl}api/v1/infinitymining/bothtradesdataweek/${traderId?._id}`,
      {
        withCredentials: true,
        headers: {
          Accept: "application/json",
  ...
  if (bothSideTradeData) {
    ...
    startDate = moment
      .utc(new Date(dates[0])?.toLocaleString("en-US", { timeZone: "UTC" }))
      .utcOffset("+00:00")
      .format("DD-MMM-YY");
    endDate = moment
      .utc(
        new Date(dates[arrayLength - 1])?.toLocaleString("en-US", {
          timeZone: "UTC",
        })
      )
      .utcOffset("+00:00")
  ...
  if (bothSideTradeData) {
    ...
    csvDataFile = csvpnlData?.map((elem) => {
      return [
        elem?.stoxHero?._id,
        moment
          .utc(new Date(elem?.stoxHero?._id))
          .utcOffset("+00:00")
          .format("dddd"),
        elem?.stoxHero?.gpnl,
        elem?.infinity?.gpnl,
            ...
            <Grid container spacing={0}>
              <Grid item xs={12} md={8} lg={4}>
                <Grid
                  ...
                  width="100%"
                >
                  {traderId && bothSideTradeData && !isLoading && (
                    <TraderDetails traderId={traderId} isLoading={isLoading} />
                  )}
                </Grid>
              </Grid>
                  ...
                  >
                    ...
                    >
                      <MDBox ml={2}>
                        ...
                        >
                          {traderId?.first_name} {traderId?.last_name}'s PNL
                        ...
                        {showDownloadButton && (
                          <Tooltip title="Screenshot">
                            <MDButton
                              variant="contained"
                              onClick={() => {
                                captureScreenshot("screenshot-component2");
                              }}
                            >
                    ...
                    {infinityMiningData && !isLoading && (
                      <PNLSummary
                        infinityMiningData={infinityMiningData}
                    ...
                    <>
                      <MDBox mb={0.5} mt={1}>
                        <MDTypography
                          ...
                          style={{ textAlign: "center" }}
                        >
                          {traderId?.first_name} {traderId?.last_name}'s Average
                          Net P&L Weekday Wise (Period - {startDate} to{" "}
                          {endDate})
                        </MDTypography>
                      </MDBox>
                      <LiveMockInfinityWeekDayChart
                        bothSideWeeklyTradeData={bothSideWeeklyTradeData}
                        isLoading={isLoading}
                      />
                    </>
                  )}
                ...
                >
                  {bothSideWeeklyTradeData && !isLoading && (
                    <>
                      ...
                      >
                        <MDBox ml={2}>
                          <MDTypography
                            ...
                            style={{ textAlign: "center" }}
                          >
                            {traderId?.first_name} {traderId?.last_name}'s
                            Weekday Cumm. P&L StoxHero Vs Infinity(Period -{" "}
                          ...
                          {showDownloadButton && (
                            <Tooltip title="Screenshot">
                              <MDButton
                                variant="contained"
                                onClick={() => {
                                  captureScreenshot("screenshot-component1");
                                }}
                              >
                                <ScreenshotMonitorIcon />
                              </MDButton>
                      ...
                      <LiveMockInfinityWeekdayDataTable
                        bothSideWeeklyTradeData={bothSideWeeklyTradeData}
                        isLoading={isLoading}
                      />
                    </>
                    ...
                    <>
                      ...
                      >
                        <MDTypography
                          ...
                          style={{ textAlign: "center" }}
                        >
                          {traderId?.first_name} {traderId?.last_name}'s Daily
                          Net P&L StoxHero Vs Infinity (Period - {startDate} to{" "}
                          {endDate})
                        </MDTypography>
                      </MDBox>
                      <LiveMockInfinityDailyData
                        bothSideTradeData={bothSideTradeData}
                        isLoading={isLoading}
                      />
                    </>
                  )}
                    ...
                    <>
                      ...
                      >
                        <MDBox ml={1}>
                          ...
                          >
                            {traderId?.first_name} {traderId?.last_name}'s Daily
                            Net P&L (Period - {startDate} to {endDate})
                          </MDTypography>
                        </MDBox>
                        <MDBox mr={1}>
                          {showDownloadButton && (
                            <Tooltip title="Screenshot">
                              <MDButton
                                variant="contained"
                                onClick={() => {
                                  captureScreenshot("screenshot-component");
                                }}
                              >
                                <ScreenshotMonitorIcon />
                              </MDButton>
