...
export default function LabTabs({setClicked}) {
    ...
    useEffect(() => {
        setIsLoading(true)
        axios.get(`${baseUrl}api/v1/marginx/userupcoming`, {
               ...
               {marginX.length != 0 ? 
                <Grid container xs={12} md={12} lg={12} display='flex'>
                    ...
                    <Grid item xs={12} md={6} lg={12} mt={.5}>
                            <UpcomingMarginXList socket={socket} marginX={marginX} isInterested={isInterested} setIsInterested={setIsInterested} showPay={showPay} setShowPay={setShowPay}/>
                    </Grid>
