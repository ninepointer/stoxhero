...
const Payment = ({ elem, setShowPay, showPay, whichTab }) => {
  ...
  async function captureIntent() {
    ...
    const res = await fetch(
      `${baseUrl}api/v1/marginxs/purchaseintent/${elem._id}`,
      {
        method: "PUT",
        credentials: "include",
        headers: {
  ...
  const buySubscription = async () => {
    ...
    const res = await fetch(`${baseUrl}api/v1/marginx/feededuct`, {
      ...
      body: JSON.stringify({
        entryFee: Number(amount - discountAmount - bonusRedemption).toFixed(2),
        marginXName: elem?.marginXName,
        marginXId: elem?._id,
        coupon: verifiedCode,
        bonusRedemption,
      }),
    ...
    if (dataResp.status === "error" || dataResp.error || !dataResp) {
      ...
      setMessege({
        ...
        ).toFixed(2)}, your seat is booked for the MarginX - ${
          elem.marginXName
        }, please click on "Start Trading" once the MarginX starts.`,
      });
    }
  };
  ...
  const initiatePayment = async () => {
    try {
      const res = await axios.post(
        ...
        {
          amount: Number(amount - discountAmount) * 100 + actualAmount * 100,
                            ...
                            <MDBox
                              ...
                              onClick={() => {
                                window.webengage.track(
                                  "marginx_intent_to_apply_couponcode_clicked",
                                  {
                                    user: getDetails?.userDetails?._id,
                                    marginxId: elem?._id,
                                    amount: Number(
                          ...
                          {bonusBalance > 0 && (
                            ...
                            >
                              <Checkbox
                                ...
                                onChange={() => {
                                  window.webengage.track(
                                    "marginx_herocash_apply_clicked",
                                    {
                                      user: getDetails?.userDetails?._id,
                                      marginxId: elem?._id,
                                    }
