...
function ContestResultPage() {
  ...
  const marginxId = location?.state?.marginxId;
  const nevigate = useNavigate();
  const [isLoading, setIsLoading] = useState(true);
  const [marginxData, setMarginx] = useState([]);
  ...
  React.useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/marginx/${marginxId}/user`, {
  ...
  const xFactor =
    marginxData?.marginXTemplate?.portfolioValue /
    marginxData?.marginXTemplate?.entryFee;
                ...
                {pnl?.netPnl >= 0 ? (
                  <div style={{ position: "relative" }}>
                    ...
                    >
                      ...
                      >
                        {reward
                          ? `You have won ₹${reward.toFixed(2)}`
                          : "Please wait while your return is loading"}
                      </MDTypography>
                      <MDTypography
                        mt={2}
