...
const Rejected = () => {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const res = await axios.get(`${apiUrl}withdrawals/rejected?skip=${skip}&limit=${limitSetting}`, { withCredentials: true });
          ...
          isLoading ?
            ...
            data.length > 0 ?
              data.map((withdrawal) => <WithDrawalCard key={withdrawal._id}
                ...
                withdrawalId={withdrawal?._id} action={action} setAction={setAction} rejectionReason={withdrawal?.rejectionReason}
              />) : <MDBox sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '60vh' }}>
                <MDTypography>
