...
const WithDrawalCard = ({walletBalance, amount, withdrawalStatus, withdrawalRequestDate, user, userWallet, walletTransactionId, withdrawalId, setAction, action, transactionId, transactionDocument, rejectionReason, withdrawalSettlementDate}) => {
    ...
    <MDBox style={{ marginBottom: '12px', padding: '12px', borderRadius: '16px', boxShadow: "0px 4px 6px -2px rgba(0, 0, 0, 0.5)" }}>
      <MDBox style={{ display: 'flex', justifyContent: 'space-between' }}>
        <MDBox>
          ...
          {(withdrawalStatus == 'Pending' || withdrawalStatus == 'Initiated') && <MDTypography style={{ fontSize: '14px', marginBottom: '8px' }}>Wallet Balance:{(walletBalance)?.toFixed(2)}</MDTypography>}
        </MDBox>
