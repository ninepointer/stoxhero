...
function TenxOrderDetail({
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/tenX/mySubscription`, {
  ...
  function onChangeSubs(e) {
    let data = allSubsciption.filter(
      (item) => item.subscriptionName == e.target.value
    )[0];
    ...
    <MDBox sx={{ display: "flex", alignItems: "center" }}>
      ...
      {infinityView !== "today" && (
        <CustomTextField2
          select
          label=""
          value={`${selectedSubscription?.subscriptionName}-${changeDateFormat(
            userSubs?.subscribedOn
          )}`}
          minHeight="4em"
          ...
          onChange={(e) => {
            setUserSubs(
              selectedSubscription?.userPurchaseDetail.filter(
                (item) =>
                  changeDateFormat(item.subscribedOn) ==
        ...
        >
          {selectedSubscription?.userPurchaseDetail?.map((option) => (
            <MenuItem
              ...
              }-${changeDateFormat(option?.subscribedOn)}`}
              minHeight="4em"
              width="300px"
            >
              {`${selectedSubscription?.subscriptionName}-${changeDateFormat(
                option?.subscribedOn
              )}`}
            </MenuItem>
          ))}
