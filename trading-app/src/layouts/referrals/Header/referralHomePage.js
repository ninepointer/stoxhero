...
function ReferralHomePage() {
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/referrals/active`, { withCredentials: true })
      .then((res) => {
  ...
  const getEarnings = async () => {
    const res = await axios.get(`${baseUrl}api/v1/earnings`, {
  ...
  const getMyReferrals = async () => {
    const res = await axios.get(`${baseUrl}api/v1//myreferrals/${id}`, {
      withCredentials: true,
    });
  ...
  const fetchData = async () => {
    const res = await axios.get(`${baseUrl}api/v1/referrals/leaderboard`, {
      withCredentials: true,
    });
    // console.log('referral data',res.data.data);
    setReferralRanks(res.data.data);
    const rankRes = await axios.get(`${baseUrl}api/v1/referrals/myrank`, {
      withCredentials: true,
    });
  ...
  joinedData?.map((elem, index) => {
    ...
    joinedRowData.doj = (
      <MDTypography variant="Contained" color="dark" fontWeight="small">
        {new Date(elem?.joining_date).toLocaleDateString("en-IN", {
          timeZone: "Asia/Kolkata",
        })}{" "}
        {new Date(elem?.joining_date)
    ...
    joinedRowData.earnings = (
      <MDTypography variant="Contained" color="dark" fontWeight="small">
        ₹
        {elem?.referrals.reduce((acc, referral) => {
          return acc + referral?.referralEarning;
        }, 0)}
      </MDTypography>
    );
  ...
  referralRanks?.map((elem, index) => {
    ...
    if (rank && rank.rank == index + 1) {
      ...
      if (
        index == 0 &&
        referralRanks[0]?.user != getDetails.userDetails.employeeid
      ) {
        refData.rank = (
          ...
          <Grid container spacing={4}>
            <Grid item xs={12} md={12} lg={8}>
              ...
              >
                ...
                {activeReferralProgram?.referralProgramName}
              </MDBox>
                ...
                <Grid container>
                  ...
                  >
                    <MDTypography fontSize={13}>
                      Get {activeReferralProgram?.currency}{" "}
                      {activeReferralProgram?.rewardPerReferral} for every
                      referral in your StoxHero wallet
                    </MDTypography>
                  </Grid>
                  ...
                  <Grid item xs={12} md={12} lg={12} mt={1.5}>
                    <MDBox display="flex" alignItems="start">
                      ...
                      />
                      <MDTypography ml={0.5} fontSize={13}>
                        Your friend gets 10 Lakhs Virtual Currency for Paper
                  ...
                  <Grid item xs={12} md={12} lg={12} mt={1.5}>
                    <MDBox display="flex" alignItems="start">
                      ...
                      />
                      <MDTypography ml={0.5} fontSize={13}>
                        Your friend gets INR 100 in his StoxHero Wallet.
                      </MDTypography>
                    </MDBox>
                  </Grid>
                      ...
                      <a
                        ...
                        onClick={() => {
                          window.webengage.track(
                            "referral_whatsapp_share_clicked",
                            {
                              user: getDetails?.userDetails?._id,
                            }
                          );
          ...
          <Grid container>
            <Grid items xs={12} md={6} lg={4}>
              ...
              >
                ...
                >
                  ...
                  <QRCode
                    value={`https://stoxhero.com?referral=${referralCode}`}
                    fgColor="green"
                    style={{ padding: "20px" }}
                  />
                </MDBox>
                ...
                >
                  <MDTypography fontSize="13px" lineHeight={1}>
                    My Referral Code
                  </MDTypography>
                  <MDTypography
                    style={{
            ...
            <Grid items xs={12} md={6} lg={4}>
              ...
              >
                ...
                >
                  <MDTypography fontSize="30px" lineHeight={1}>
                    {" "}
                    {joinedCount}{" "}
                  </MDTypography>
                  <MDTypography fontSize="15px" lineHeight={1}>
                    Friends Joined
                  </MDTypography>
                </MDBox>
              </MDBox>
          ...
          <Grid container>
            ...
            <Grid items xs={12} md={6} lg={4}>
              ...
              >
                ...
                >
                  <MDTypography fontSize="30px" lineHeight={1}>
                    {affiliateSummery?.payout
                      ? new Intl.NumberFormat(undefined, {
                          minimumFractionDigits: 0,
                          maximumFractionDigits: 0,
                        }).format(affiliateSummery?.payout)
                      : 0}
                  </MDTypography>
                  <MDTypography fontSize="15px" lineHeight={1}>
                    Earnings (Product) in INR
                  </MDTypography>
                </MDBox>
              </MDBox>
            </Grid>
                ...
                >
                  <MDTypography fontSize="30px" lineHeight={1}>
                    {new Intl.NumberFormat(undefined, {
                      ...
                      (earnings || 0) + (affiliateSummery?.payout || 0)
                    )}
                  </MDTypography>
                  <MDTypography fontSize="15px" lineHeight={1}>
