...
const XTSToken = () => {
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/xtsTokenActive`)
      .then((res) => {
    ...
    axios
      .get(`${baseUrl}api/v1/xtsTokenInactive`)
      .then((res) => {
  ...
  async function autoGenerate() {
    axios
      .get(`${baseUrl}api/v1/autoLoginXTS`, {
  ...
  activeData.map((elem) => {
    ...
    activeparameter.status = (
      <MDBox mt={0.5}>
        <Switch
          checked={elem.status === "Active"}
          onChange={() => {
            changeStatus(elem._id, elem.status);
          }}
        />
      </MDBox>
    );
