  ...
  const [trackEvent, setTrackEvent] = useState({});
    ...
    useEffect(()=>{
      ...
      (async () => {
           ...
           const { data } = await axios.get(`${baseUrl}api/v1/infinityTrade/pnl/traderWise/${selectedTrader}`,{
    ...
    }, [render, trackEvent, selectedTrader])
  ...
  return (
    <Card>
        ...
        <TextField
                ...
                onChange={(e)=>{setSelectedTrader(trader.filter((item)=>item.name == e.target.value)[0]._id)}}
        >
          {trader?.map((option) => (
