...
function OverallTraderPNL({
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/internbatch/active`, { withCredentials: true })
      .then((res) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${baseUrl}api/v1/internship/pnlAllTrader/${selectedBatch}`, {
    ...
    <Card>
      ...
      <MDBox sx={{ display: "flex", alignItems: "center", marginLeft: "24px" }}>
        ...
        <TextField
          ...
          onChange={(e) => {
            setSelectedBatch(
              batches.filter((item) => item.batchName == e.target.value)[0]._id
            );
          }}
        >
          {batches?.map((option) => (
            <MenuItem
              key={option.batchName}
