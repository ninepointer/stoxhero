...
function Transactions({ render }) {
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/infinityTrade/pnlandCreditData`)
      .then((res) => {
      ...
      <MDBox pt={3} pb={2} px={2}>
        ...
        >
          ...
          {traderPNLDetails.map((elem) => {
            let obj = {};
            let pnlAmountString = elem.npnl
              ? elem.npnl >= 0
                ? "+₹" + elem.npnl.toFixed(0).toLocaleString()
                : "-₹" + (-elem.npnl.toFixed(0)).toLocaleString()
              : 0;
            let totalCreditString = elem.totalCredit
              ? elem.totalCredit >= 0
                ? "+₹" + elem.totalCredit.toLocaleString()
                : "-₹" + (-elem.totalCredit).toLocaleString()
              : 0;
            let availableMarginString = elem.availableMargin
              ? elem.availableMargin >= 0
                ? "+₹" + elem.availableMargin.toFixed(0).toLocaleString()
                : "-₹" + (-elem.availableMargin.toFixed(0)).toLocaleString()
              : 0;
            let color = elem.npnl >= 0 ? "success" : "error";
            let colorTotal = elem.totalCredit >= 0 ? "success" : "error";
            let colorTotalAvailable =
              elem.availableMargin >= 0 ? "success" : "error";
            obj = (
              <Transaction
                color={color}
                colorTotal={colorTotal}
