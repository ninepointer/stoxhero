...
function TradersRanking({
  ...
  const leaderBoardEndPoint =
    isFromHistory || isFromResult ? "historyLeaderboard" : "leaderboard";
  const myRankEndPoint =
    isFromHistory || isFromResult ? "historyMyrank" : "myrank";
  ...
  const fetchData = async () => {
    ...
    const api1Response = await axios.get(
      `${baseUrl}api/v1/contest/${contestId}/trades/${leaderBoardEndPoint}`,
      {
    ...
    const api2Response = await axios.get(
      `${baseUrl}api/v1/contest/${contestId}/trades/${myRankEndPoint}`,
      {
        withCredentials: true,
        headers: {
          Accept: "application/json",
  ...
  useEffect(() => {
    ...
    if (isFromHistory || isFromResult) {
      ...
      const intervalId = setInterval(
        fetchData,
        setting[0]?.leaderBoardTimming * 1000
  ...
  const myProfitPercentage =
    (myRank?.npnl * 100) / Number(myRank?.investedAmount);
          ...
          {isLoading ? (
            ...
            <>
              ...
              >
                ...
                >
                  <MDTypography fontSize={10} color="light">
                    {getDetails?.userDetails?.employeeid}
                  </MDTypography>
                </Grid>
                <Grid
                  item
                ...
                >
                  <MDTypography
                    fontSize={10}
                    color={myRank?.npnl >= 0 ? "success" : "error"}
                  >
                    {myRank?.npnl
                      ? myRank?.npnl >= 0.0
                        ? "+₹" + myRank?.npnl?.toFixed(2)
                        : "-₹" + (-myRank?.npnl).toFixed(2)
                      : "+₹" + 0.0}
                  </MDTypography>
                </Grid>
                ...
                >
                  ...
                  >
                    {myProfitPercentage
                      ? myProfitPercentage >= 0.0
                        ? "+" + myProfitPercentage?.toFixed(2) + "%"
                        : "-" + (-myProfitPercentage).toFixed(2) + "%"
                      : "+0.00%"}
                  </MDTypography>
                </Grid>
                ...
                >
                  <MDTypography fontSize={10} color="light">
                    {myReward && myReward?.length && myReward[0]?.reward
                      ? `${myReward[0]?.currency} ${myReward[0]?.reward}`
                      : `${reward[0].currency} 0`}
                  </MDTypography>
                </Grid>
              </Grid>
              ...
              {rankData.map((elem, index) => {
                const rewards = reward?.filter((elem) => {
                  return (
                    elem?.rankStart <= index + 1 && elem?.rankEnd >= index + 1
                  );
                });

                const profitPercentage =
                  (elem.npnl * 100) / elem.investedAmount;
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <MDTypography
                        fontSize={10}
                        color={elem.npnl >= 0 ? "success" : "error"}
                      >
                        {elem.npnl >= 0.0
                          ? "+₹" + elem.npnl?.toFixed(2)
                          : "-₹" + (-elem.npnl).toFixed(2)}
                    ...
                    >
                      <MDTypography fontSize={10} color="light">
                        {rewards && rewards?.length && rewards[0]?.reward
                          ? `${rewards[0]?.currency} ${rewards[0]?.reward}`
                          : `${reward[0].currency} 0`}
                      </MDTypography>
                    </Grid>
                  </Grid>
