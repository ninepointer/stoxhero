...
const CompletedContest = () => {
  ...
  useEffect(() => {
    let call1 = axios.get(`${apiUrl}battles/completedadmin`, {
    ...
    Promise.all([call1])
      .then(([api1Response]) => {
        ...
        setInactiveTemplate(api1Response.data.data);
      })
      .catch((error) => {
        // Handle errors here
        console.error(error);
  ...
  return (
    <>
      {inactiveTemplate.length > 0 ? (
        <MDBox>
          <Grid container spacing={2} bgColor="dark">
            {inactiveTemplate?.map((e) => {
              return (
