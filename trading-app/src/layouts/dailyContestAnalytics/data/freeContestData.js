...
export default function FreeContestData() {
  ...
  useEffect(() => {
    let call1 = axios.get(
      `${baseUrl}api/v1/dailycontest/dailyfreecontestdata?skip=${skip}&limit=${limitSetting}`,
      {
    ...
    let call2 = axios.get(
      `${baseUrl}api/v1/dailycontest/freecontestuserdata?skip=${userskip}&limit=${userlimitSetting}`,
      {
        withCredentials: true,
        headers: {
          Accept: "application/json",
  ...
  function backHandler() {
    ...
    axios
      .get(
        `${baseUrl}api/v1/dailycontest/dailyfreecontestdata?skip=${
          skip - limitSetting
        }&limit=${limitSetting}`,
        {
          withCredentials: true,
  ...
  function backHandlerUser() {
    ...
    axios
      .get(
        `${baseUrl}api/v1/dailycontest/freecontestuserdata?skip=${
          userskip - userlimitSetting
        }&limit=${userlimitSetting}`,
        {
          withCredentials: true,
          headers: {
            Accept: "application/json",
      ...
      <Grid container spacing={1}>
        ...
        >
          <Grid
            ...
            alignItems="center"
          >
            <MDTypography color="light" fontSize={10} fontWeight="bold">
              Contest Name
            </MDTypography>
          </Grid>
          <Grid
        ...
        {!isLoading || !isDataLoading ? (
          freeContestData?.map((elem) => {
            ...
            const typecolor =
              elem?.transaction_type === "Buy" ? "success" : "error";
            return (
              <Grid
                container
                mt={1}
                ...
                >
                  <MDTypography color="light" fontSize={10} fontWeight="bold">
                    {new Intl.NumberFormat(undefined, {
                      minimumFractionDigits: 0,
                      maximumFractionDigits: 0,
                    }).format(elem?.percentageLossMakingTraders)}
                  </MDTypography>
                </Grid>
                <Grid
                  item
        ...
        {(!isLoading || !isDataLoading) && count !== 0 && (
          ...
          >
            ...
            <MDTypography color="light" fontSize={15} fontWeight="bold">
              Total TestZones: {!count ? 0 : count} | Page{" "}
                ...
                >
                  <MDTypography color="light" fontSize={10} fontWeight="bold">
                    {new Intl.NumberFormat(undefined, {
                      minimumFractionDigits: 0,
                      maximumFractionDigits: 0,
                    }).format(elem?.daysSinceLastContest)}
                  </MDTypography>
                </Grid>
                <Grid
                  item
        ...
        {(!isLoading || !isUserLoading) && userCount !== 0 && (
          ...
          >
            <MDButton
              ...
              color="warning"
              disabled={
                (userskip + userlimitSetting) / userlimitSetting === 1
                  ? true
                  : false
              }
              size="small"
              onClick={backHandlerUser}
            >
              Back
            </MDButton>
            <MDTypography color="light" fontSize={15} fontWeight="bold">
              Total TestZone Participants: {!userCount ? 0 : userCount} | Page{" "}
              {(userskip + userlimitSetting) / userlimitSetting} of{" "}
              {!userCount ? 1 : Math.ceil(userCount / userlimitSetting)}
            </MDTypography>
            <MDButton
              variant="outlined"
              color="warning"
              disabled={
                Math.ceil(userCount / userlimitSetting) ===
                (userskip + userlimitSetting) / userlimitSetting
                  ? true
                  : !userCount
                  ? true
                  : false
