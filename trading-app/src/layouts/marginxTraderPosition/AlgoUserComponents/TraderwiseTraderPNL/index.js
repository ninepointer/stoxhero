...
function TraderwiseTraderPNL({ socket }) {
  ...
  useEffect(() => {
    axios
      .get(`${apiUrl}marginx/today`, { withCredentials: true })
      .then((res) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${apiUrl}marginxtrade/${selectedMarginx?._id}/traderWisePnlTside`, {
  ...
  const xFactor =
    selectedMarginx?.marginXTemplate?.portfolioValue /
    selectedMarginx?.marginXTemplate?.entryFee;
  ...
  finalTraderPnl.map((subelem, index) => {
    ...
    obj.return = (
      <MDTypography
        ...
        fontWeight="medium"
      >
        {(subelem.totalPnl - subelem.brokerage) / xFactor >= 0.0
          ? "+₹" + ((subelem.totalPnl - subelem.brokerage) / xFactor).toFixed(2)
          : "-₹" +
            (-((subelem.totalPnl - subelem.brokerage) / xFactor)).toFixed(2)}
      </MDTypography>
    );
    ...
    obj.cumm_return = (
      <MDTypography
        component="a"
        variant="caption"
        color={subelem?.cumm_return >= 0 ? "success" : "error"}
        padding="5px"
        borderRadius="5px"
        backgroundColor="#e0e1e5"
        fontWeight="medium"
      >
        {subelem?.cumm_return >= 0.0
          ? "+₹" + (subelem?.cumm_return).toFixed(2)
          : "-₹" + (-subelem?.cumm_return).toFixed(2)}
      </MDTypography>
    );
        ...
        <Card>
          ...
          <MDBox>
            ...
            <TextField
              ...
              label=""
              value={
                selectedMarginx?.marginXName
                  ? selectedMarginx?.marginXName
                  : marginx[0]?.marginXName
              }
              minHeight="4em"
              ...
              onChange={(e) => {
                setselectedMarginx(
                  marginx.filter(
                    (item) => item.marginXName == e.target.value
                  )[0]
                );
              }}
            >
              {marginx?.map((option) => (
                <MenuItem
                  key={option.marginXName}
                  value={option.marginXName}
                  minHeight="4em"
                >
                  {option.marginXName}
                </MenuItem>
            ...
            <MDBox
              style={{
                backgroundColor: `${
                  selectedMarginx?.marginXTemplate?.entryFee > 0
                    ? "lightgreen"
                    : "yellow"
                }`,
              }}
              ...
              <Grid container xs={12} md={12} lg={12}>
                ...
                <Grid item xs={12} md={12} lg={2}>
                  ...
                  >
                    ...
                    <MDTypography fontSize={13} fontWeight="bold">
                      {selectedMarginx?.maxParticipants}
                    </MDTypography>
                  </MDBox>
                </Grid>
                <Grid item xs={12} md={12} lg={2}>
                  ...
                  >
                    ...
                    <MDTypography fontSize={13} fontWeight="bold">
                      {selectedMarginx?.participants?.length}
                    </MDTypography>
                  </MDBox>
                </Grid>
                <Grid item xs={12} md={12} lg={2}>
                ...
                <Grid item xs={12} md={12} lg={2}>
                  ...
                  >
                    ...
                    <MDTypography fontSize={13} fontWeight="bold">
                      {selectedMarginx?.isNifty === true ? "Yes" : "No"}
                    </MDTypography>
                  </MDBox>
                </Grid>
                <Grid item xs={12} md={12} lg={2}>
                  ...
                  >
                    ...
                    <MDTypography fontSize={13} fontWeight="bold">
                      {selectedMarginx?.isBankNifty === true ? "Yes" : "No"}
                    </MDTypography>
                  </MDBox>
                </Grid>
                <Grid item xs={12} md={12} lg={2}>
                  ...
                  >
                    ...
                    <MDTypography fontSize={13} fontWeight="bold">
                      {selectedMarginx?.isFinNifty === true ? "Yes" : "No"}
                    </MDTypography>
                  </MDBox>
                </Grid>
              </Grid>
