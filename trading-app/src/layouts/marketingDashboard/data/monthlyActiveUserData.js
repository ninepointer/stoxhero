...
export default function LiveTenXSubscribers({ monthNumber, yearNumber }) {
  ...
  useEffect(() => {
    axios
      .get(
        `${baseUrl}api/v1/stoxherouserdashboard/marketingfunnelactiveusers/${monthNumber}/${yearNumber}?page=${page}&perPage=${perPage}`,
        {
  ...
  function backHandler() {
    ...
    axios
      .get(
        `${baseUrl}api/v1/stoxherouserdashboard/marketingfunnelactiveusers/${monthNumber}/${yearNumber}?page=${
          page - 1
        }&perPage=${perPage}`,
        {
          withCredentials: true,
        ...
        {!isLoading && activeUserCount !== 0 && (
          ...
          >
            ...
            <MDTypography color="light" fontSize={15} fontWeight="bold">
              Total Active Users: {!activeUserCount ? 0 : activeUserCount} |
              Page {page} of{" "}
              {!activeUserCount ? 1 : Math.ceil(activeUserCount / perPage)}
            </MDTypography>
            <MDButton
              variant="outlined"
              color="warning"
              disabled={
                Math.ceil(activeUserCount / perPage) === page
                  ? true
                  : !activeUserCount
                  ? true
                  : false
