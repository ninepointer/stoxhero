...
export default function LabTabs() {
  ...
  useEffect(()=>{
    axios.get(`${apiUrl}dailycontest/contests/completedadminLive`, {withCredentials: true})
    .then((res)=>{
  ...
  const handleShowDetails = async() => {
    if (selectedSubscription?._id) {
      ...
      if(selectTab === "Trader Side"){
        endpoint = "live/traderwisetraderpnlreport"
      } else{
        endpoint = "live/traderwisecompanypnlreport"
      }
      const res = await axios.get(`${apiUrl}dailycontest/${endpoint}/${selectedSubscription?._id}`, { withCredentials: true });
