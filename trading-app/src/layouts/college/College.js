...
const College = () => {
  ...
  //       setCollegeData(response.data.data.reverse()); // Reverse the order of colleges before setting the state
  //     })
  //     .catch((error) => {
  //       console.error(error);
  //     });
  ...
  useEffect(() => {
    fetchCollegeData(); // Fetch college data on component mount
  }, []);

  useEffect(() => {
    if (isDataChanged) {
      fetchCollegeData(); // Fetch college data whenever isDataChanged is true
    ...
    <MDBox style={{ marginTop: '10px', height: '80vh', background: '#344767', borderRadius: '12px' }}>
        ...
        <>
          ...
          <Grid
            ...
            p={1}
            container
            sx={{ background: '#fff', borderRadius: '8px', color: '#000', marginLeft: { xs: '13px', md: '50px' }, width: '92%' }}
          >
            <Grid item xs={4} md={4} lg={4} display="flex" justifyContent="center" alignContent="center" alignItems="center">
              <MDTypography fontSize={13} fontWeight="bold">College</MDTypography>
            </Grid>
          ...
          <MDBox sx={{ maxHeight: '500px', overflow: 'auto' }} mt={3}>
            {collegeData.map((elem) => (
              <Grid
                key={elem._id}
                container
                sx={{ background: 'whitesmoke', marginLeft: { xs: '13px', md: '50px' }, width: '92%' }}
                borderRadius="8px"
                mt={1}
                mb={1}
                p={1}
              >
                <Grid item xs={4} md={4} lg={4} display="flex" justifyContent="center" alignContent="center" alignItems="center">
                  <MDTypography color="#000" fontSize={13} fontWeight="bold">
                    {elem.collegeName}
                  </MDTypography>
                </Grid>
                ...
                <Grid item xs={4} md={4} lg={4} display="flex" justifyContent="center" alignContent="center" alignItems="center">
                  <MDButton
                    size="small"
                    style={{ color: '#fff', fontSize: '13px', fontWeight: '500', width: '60px', marginBottom: '2px', marginTop: '1px', background: 'rgb(88, 115, 196)' }}
                    component={Link}
                    to={{ pathname: '/collegeEdit' }}
                    state={{ data: elem }}
                  >
                    <EditIcon />
                  </MDButton>
