...
export default function Wallet() {
  ...
  useEffect(() => {
    ...
    const slicedData = myWallet?.transactions?.slice(
      startIndex,
      startIndex + perPage
    );
    setData(slicedData);
  ...
  useEffect(() => {
    ...
    Promise.all([call1])
      .then(([api1Response]) => {
        ...
        const startIndex = (currentPage - 1) * perPage;
        const slicedData = api1Response.data.data?.transactions?.slice(
          startIndex,
          startIndex + perPage
        );
        setData(slicedData);
  ...
  useEffect(() => {
    axios
      .get(`${apiUrl}withdrawals/mywithdrawals`, { withCredentials: true })
      .then((res) => {
        // console.log(res.data.data);
        setMyWithdrawals(
          res.data.data.filter((item) => item.withdrawalStatus == "Processed")
        );
        const startIndex = (currentPage - 1) * perPage;
        const slicedData = res.data.data
          .filter((item) => item.withdrawalStatus == "Processed")
          .slice(startIndex, startIndex + perPageWd);
        setDataWd(slicedData);
      });
  }, [open]);
      ...
      >
        <Grid container xs={12} md={6} lg={12}>
              ...
              <>
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <MDTypography
                            color={elem?.amount >= 0 ? "success" : "error"}
                            fontSize={12}
                            fontWeight="bold"
                          >
                ...
                {data?.length > 0 && (
                  ...
                  >
                    ...
                    <MDTypography color="light" fontSize={12} fontWeight="bold">
                      Transactions: {myWallet?.transactions.length} | Page{" "}
                      {currentPage} of{" "}
                      {Math.ceil(myWallet?.transactions.length / perPage)}
                    </MDTypography>
                    <MDButton
                      variant="outlined"
                      color="warning"
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    InfinityTraderRole
                      ? `Invite your friends to the app to earn cash & bonus.`
                      : `Participate in Battles and invite your friends to the app to earn cash & bonus.`}
                  </MDTypography>
                  <MDBox
                    mt={3}
                    display="flex"
                ...
                >
                  ...
                  >
                    <MDTypography color="light" fontWeight="bold" fontSize={12}>
                      {myWallet?.userId?.first_name}{" "}
                      {myWallet?.userId?.last_name}
                    </MDTypography>
                  </Grid>
                  <Grid
                    item
                    ...
                    alignItems="center"
                  >
                    <MDTypography color="light" fontSize={10}>
                      KYC Status: {myWallet?.userId?.KYCStatus}
                    </MDTypography>
                  </Grid>
                  <Grid
                    item
                  ...
                  >
                    <MDTypography color="light" fontSize={10}>
                      Bank Details:{" "}
                      {myWallet?.userId?.accountNumber ||
                      myWallet?.userId?.bankName ||
                      myWallet?.userId?.nameAsPerBankAccount ||
                      myWallet?.userId?.ifscCode
                        ? "Updated"
                        : "Not Updated"}
                    </MDTypography>
                  </Grid>
                ...
                >
                  ...
                  >
                    <MDTypography color="light" fontSize={12} fontWeight="bold">
                      ...
                      }).format(totalDepositAmount ? totalDepositAmount : 0)}
                    </MDTypography>
                  </Grid>
                  <Grid
                    item
                  ...
                  >
                    <MDTypography color="light" fontSize={12} fontWeight="bold">
                      ...
                      {new Intl.NumberFormat(undefined, {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      }).format(totalBonusAmount ? totalBonusAmount : 0)}
                    </MDTypography>
                  </Grid>
                  <Grid
                    item
                    ...
                    <MDButton
                      ...
                      onClick={() => {
                        window.webengage.track(
                          "wallet_redeem_herocash_clicked",
                          {
                            user: getDetails?.userDetails?._id,
                          }
                        );
                  ...
                  >
                    <MDTypography color="light" fontSize={12} fontWeight="bold">
                      ...
                      {new Intl.NumberFormat(undefined, {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      }).format(totalWithdrawals ? totalWithdrawals : 0)}
                    </MDTypography>
                  </Grid>
                  <Grid
                    item
                  ...
                  return (
                    ...
                    >
                      <MDTypography color="light" fontSize={12}>
                        ...
                        }).format(elem?.amount ? elem?.amount : 0)}
                      </MDTypography>
                      <MDTypography color="light" fontSize={12}>
                        Transfer Date:{" "}
                        {moment
                          .utc(elem?.withdrawalSettlementDate)
                          .utcOffset("+05:30")
                          .format("DD-MMM-YY hh:mm:ss a")}
                      </MDTypography>
                      <MDTypography color="light" fontSize={12}>
                        Status: {elem?.withdrawalStatus}
                      </MDTypography>
                      <MDTypography color="light" fontSize={12}>
                        Wallet TransactionId: {elem?.walletTransactionId}
                      </MDTypography>
                      <MDTypography color="light" fontSize={12}>
                        Transfer TransactionId: {elem?.settlementTransactionId}
                      </MDTypography>
                      <MDTypography color="light" fontSize={12}>
                        Transferred Mode: {elem?.settlementMethod}
                      </MDTypography>
                    </Grid>
                  );
                })}
              {dataWd?.length > 0 && (
                ...
                >
                  <MDButton
                    variant="outlined"
                    color="warning"
                    disabled={currentPageWd === 1 ? true : false}
                    size="small"
                    onClick={handlePrevPageWd}
                  >
                    Back
                  </MDButton>
                  <MDTypography color="light" fontSize={12} fontWeight="bold">
                    Transactions: {mywithdrawals.length} | Page {currentPageWd}{" "}
                    of {Math.ceil(mywithdrawals.length / perPageWd)}
                  </MDTypography>
                  <MDButton
                    variant="outlined"
                    color="warning"
                    disabled={
                      Math.ceil(mywithdrawals.length / perPageWd) ===
                      currentPageWd
                        ? true
                        : false
                    }
                    size="small"
                    onClick={handleNextPageWd}
                  >
                    Next
                  </MDButton>
                </MDBox>
          ...
          <WithdrawalModal
            ...
            walletBalance={totalCashAmount ?? 0}
          />
        </Grid>
      </MDBox>
    </>
