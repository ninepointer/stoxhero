...
function CreateCoupon() {
  ...
  const [selectedProduct, setSelectedProduct] = useState(
    id?.eligibleProducts ? id?.eligibleProducts : []
  );
  const [selectedPlatform, setSelectedPlatform] = useState(
    id?.eligiblePlatforms ? id?.eligiblePlatforms : []
  );
  ...
  async function onSubmit(e, formState) {
    ...
    if (
      ...
      !formState?.eligiblePlatforms.length ||
      !formState?.eligiblePlatforms.length
    ) {
      setTimeout(() => {
        setCreating(false);
          ...
          <Grid container spacing={2} mt={0.5}>
            ...
            <Grid item xs={12} md={6} xl={2}>
              <TextField
                ...
                value={
                  formState?.minOrderValue || editing
            ...
            <Grid item xs={12} md={6} xl={3}>
              <TextField
                ...
                value={
                  formState?.affiliatePercentage || editing
                    ? formState?.affiliatePercentage
                    : couponData?.affiliatePercentage
                }
                fullWidth
                onChange={(e) => {
                  setFormState((prevState) => ({
            ...
            {(id || isObjectNew) && (
              <Grid item xs={12} md={12} xl={12} mt={2} mb={2}>
                <MDBox>
                  <SuccessfullAppliedUser
                    couponData={id?._id ? id : couponData}
                  />
                </MDBox>
              </Grid>
            )}
