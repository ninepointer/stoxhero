...
const PnlMaxMinData = () => {
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/dailypnlmaxmindata`, { withCredentials: true })
      .then((res) => {
  ...
  FDPNLData?.map((elem1) => {
    ...
    pnldata.gpnl = (
      <MDTypography
        ...
        fontWeight="medium"
      >
        {elem1.lastTimestampSum >= 0.0
          ? "+₹" + elem1.lastTimestampSum.toFixed(0)
          : "-₹" + (-elem1.lastTimestampSum).toFixed(0)}
      </MDTypography>
    );
    pnldata.maxpnl = (
