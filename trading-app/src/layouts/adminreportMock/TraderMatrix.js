...
const TableThree = () => {
  ...
  let valueInStartDate = `${date.getFullYear()}-${String(
    date.getMonth() + 1
  ).padStart(2, "0")}-01`;
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${baseUrl}api/v1/infinityTrade/tradermatrixpnlreport/${firstDate}/${secondDate}`
      )
      .then((res) => {
  ...
  traderpnldata?.map((elem) => {
    ...
    tradermatrix.push(
      JSON.parse(
        JSON.stringify({ totalPositivePnl, totalNegativePnl, createdBy })
      )
    );
  });
  ...
  pnlmatrixArr.sort((a, b) => {
    if (
      Math.abs(a.NegativePnl / a.PositivePnl) <
      Math.abs(b.NegativePnl / b.PositivePnl)
    ) {
      return 1;
    }
    if (
  ...
  pnlmatrixArr?.map((elem) => {
    ...
    let ratio = 0;
    let averagereddaysgpnl =
      elem.RedDays != 0 ? elem.NegativePnl / elem.RedDays : 0;
    let averagegreendaysgpnl =
      elem.GreenDays != 0 ? elem.PositivePnl / elem.GreenDays : 0;
    const averagereddaysgpnlcolor =
      averagereddaysgpnl >= 0 ? "success" : "error";
    const averagegreendaysgpnlcolor =
      averagegreendaysgpnl >= 0 ? "success" : "error";
    const probableavgpnl =
      (elem.RedDays / elem.TradingDays) * averagereddaysgpnl +
      (elem.GreenDays / elem.TradingDays) * averagegreendaysgpnl;
    ...
    tpnl.agreendaysgpnl = (
      <MDTypography
        component="a"
        variant="caption"
        color={averagegreendaysgpnlcolor}
        fontWeight="medium"
      >
        {averagegreendaysgpnl >= 0
          ? "+₹" + averagegreendaysgpnl.toFixed(0)
          : "-₹" + (-averagegreendaysgpnl).toFixed(0)}
      </MDTypography>
    );
    tpnl.areddaysgpnl = (
      <MDTypography
        component="a"
        variant="caption"
        color={averagereddaysgpnlcolor}
        fontWeight="medium"
      >
        {averagereddaysgpnl >= 0
          ? "+₹" + averagereddaysgpnl.toFixed(0)
          : "-₹" + (-averagereddaysgpnl).toFixed(0)}
      </MDTypography>
    );
  ...
  const firstDateFormat = `${String(new Date(firstDate).getDate()).padStart(
    2,
    "0"
  )}-${String(new Date(firstDate).getMonth() + 1).padStart(2, "0")}-${new Date(
    firstDate
  ).getFullYear()}`;
  const secondDateFormat = `${String(new Date(secondDate).getDate()).padStart(
    2,
    "0"
  )}-${String(new Date(secondDate).getMonth() + 1).padStart(2, "0")}-${new Date(
    secondDate
  ).getFullYear()}`;
    ...
    <MDBox pt={1} pb={0.5}>
      <Grid container spacing={1}>
        ...
        <Grid item xs={6} md={1} xl={12}>
          ...
          >
            <MDBox>
              <MDTypography variant="h6" textAlign="center" py={0.5}>
                &nbsp;&nbsp;&nbsp;&nbsp;Gross P&L&nbsp;&nbsp;&nbsp;&nbsp;
              </MDTypography>
              <MDTypography
                variant="h6"
                textAlign="center"
                color={totalgpnlcolor}
          ...
          <Card>
            ...
            >
              <MDTypography variant="h6" color="white">
                Trader Metrics (Mock) for the period {firstDateFormat} to{" "}
                {secondDateFormat}
              </MDTypography>
            </MDBox>
            <MDBox pt={2}>
