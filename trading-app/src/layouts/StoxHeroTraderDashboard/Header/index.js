...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    let call2 = axios.get(
      `${baseUrl}api/v1/virtualtradingperformance/traderstats/${userId}`,
      {
        withCredentials: true,
        headers: {
          Accept: "application/json",
    ...
    let call5 = axios.get(
      `${baseUrl}api/v1/dailycontest/lastpaidcontestchampions`,
      {
        withCredentials: true,
        headers: {
          Accept: "application/json",
  ...
  const getTraderSummary = async () => {
    try {
      const res = await axios.get(`${baseUrl}api/v1/userdashboard/summary`, {
        withCredentials: true,
      });
      ...
      >
        <Grid item xs={12} md={12} lg={12} mt={1}>
          {!isLoading ? (
            <MDBox style={{ backgroundColor: "white", borderRadius: 5 }}>
              <TestZoneChampions
                lastPaidContests={lastPaidContests}
