...
function TraderwiseTraderPNL({ socket }) {
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/tenX/subswithliveuser`, { withCredentials: true })
      .then((res) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${baseUrl}api/v1/tenX/${selectedSubscription}/trade/traderWisePnl`,
      ...
      <MDBox sx={{ display: "flex", alignItems: "center", marginLeft: "24px" }}>
        ...
        >
          {subscriptions?.map((option) => (
            <MenuItem
              key={option.plan_name}
              value={option.plan_name}
              minHeight="4em"
            >
              {option.plan_name}
            </MenuItem>
