...
const PopupMessage = ({
  ...
  async function registerUserToContest(id) {
    ...
    window.webengage.track("college_testzone_get_notified_clicked", {
      user: getDetails?.userDetails?._id,
      contestId: id,
    });
    const res = await fetch(
      `${baseUrl}api/v1/dailycontest/contest/${id}/register`,
      {
        method: "PUT",
        credentials: "include",
        headers: {
    ...
    <div>
      {!isInterested[elem._id]?.interested && !showThanksMessage ? (
        <MDButton
          ...
          size="small"
          onClick={() => {
            registerUserToContest(elem._id);
          }}
        >
          <MDTypography color="info" fontWeight="bold" fontSize={10}>
            Get Notified
          </MDTypography>
        </MDButton>
      ) : (
