...
function Header({ contest }) {
  ...
  const handleDownload = async (id) => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}api/v1/dailycontest/download/${id}`,
        {
          responseType: "blob",
          withCredentials: true,
        }
    ...
    axios
      ...
      .then((res) => {
        if (res.data.count > 0) {
          ...
          openSuccessSB(
            "error",
            "You do not have any trade for this TestZone."
          );
        }
      })
      .catch((err) => {
  ...
  async function handleLeaderboardNavigate(id, name) {
    window.webengage.track(
      "college_testzone_leaderboard_in_completed_clicked",
      {
        user: getDetails?.userDetails?._id,
        contestId: id,
      }
                                  ...
                                  >
                                    ...
                                    >
                                      <MDTypography
                                        ...
                                        sx={{ color: "#DBB670" }}
                                      >
                                        {/* {elem.payoutPercentage}% of the net P&L */}{" "}
