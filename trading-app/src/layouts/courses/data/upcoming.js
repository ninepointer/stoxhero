...
const UpcomingCourses = ({ status }) => {
  const [upcomingCourses, setUpcomingCourses] = useState([]);

  useEffect(() => {
    let call1 = axios.get(`${apiUrl}courses/adminupcoming`, {
    ...
    Promise.all([call1])
      .then(([api1Response]) => {
        ...
        setUpcomingCourses(api1Response.data.data)
      ...
      {upcomingCourses.length > 0 ?
        ...
        <MDBox>
          <Grid container spacing={4} bgColor="dark" display="flex" justifyContent="flex-start" alignItems='center'>
            {upcomingCourses?.map((elem, index) => {
        ...
        <Grid container spacing={1} xs={12} md={6} lg={12}>
          <Grid item mt={2} xs={6} md={3} lg={12} display="flex" justifyContent="center">
            <MDTypography color="light">No Upcoming Course(s)</MDTypography>
          </Grid>
        </Grid>
      }
