...
const CompletedCourses = ({ status }) => {
  const [completedCourses, setCompletedCourses] = useState([]);

  useEffect(() => {
    let call1 = axios.get(`${apiUrl}courses/admincompleted`, {
    ...
    Promise.all([call1])
      .then(([api1Response]) => {
        ...
        setCompletedCourses(api1Response.data.data)
      ...
      {completedCourses.length > 0 ?
        ...
        <MDBox>
          <Grid container spacing={4} bgColor="dark" display="flex" justifyContent="flex-start" alignItems='center'>
            {completedCourses?.map((elem, index) => {
