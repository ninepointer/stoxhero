...
const CompletedContest = () => {
  ...
  useEffect(() => {
    let call1 = axios.get(`${baseUrl}api/v1/notificationgroup/draft?skip=${skip}&limit=${limitSetting}`, {
  ...
  function backHandler(){
    ...
    setActive([]);
    setIsLoading(true)
    axios.get(`${baseUrl}api/v1/notificationgroup/draft?skip=${skip-limitSetting}&limit=${limitSetting}`,{
        withCredentials: true,
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
