...
const CompletedContest = () => {
  ...
  useEffect(() => {
    let call1 = axios.get(
      `${baseUrl}api/v1/notificationgroup/draft?skip=${skip}&limit=${limitSetting}`,
      {
  ...
  function backHandler() {
    ...
    axios
      .get(
        `${baseUrl}api/v1/notificationgroup/draft?skip=${
          skip - limitSetting
        }&limit=${limitSetting}`,
        {
          withCredentials: true,
    ...
    <>
      {active?.length > 0 ? (
        ...
        <Grid container spacing={1} xs={12} md={6} lg={12}>
          ...
          >
            <MDTypography color="light">No Draft Group(s)</MDTypography>
          </Grid>
        </Grid>
      )}
    </>
