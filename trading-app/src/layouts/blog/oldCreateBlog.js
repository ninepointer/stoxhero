...
function Index() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${baseUrl}api/v1/blogs/${id ? id?._id : newObjectId?._id}`, {
  ...
  async function onAddContent(e, childFormState, setChildFormState) {
    ...
    try {
      ...
      Object.keys(childFormState.blogContent[0]).forEach((key) => {
        ...
        formData1.append(
          "blogContent." + key,
          childFormState.blogContent[0][key]
        );
        // console.log("data appended",formData)
      ...
      if (
        !childFormState.blogContent[0].serialNumber ||
      ...
      const res = await fetch(
        `${baseUrl}api/v1/blogs/${id ? id?._id : newObjectId?._id}`,
        {
          method: "PATCH",
          credentials: "include",
          headers: {
      ...
      if (data.data) {
        ...
        openSuccessSB(
          "Content added",
          "New Content line item has been added in the Blog"
        );
        setTimeout(() => {
          setSaving(false);
          setEditing(false);
              ...
              {newObjectId?.status && (
                <>
                  <Grid
                    ...
                    justifyContent="center"
                  >
                    <MDTypography fontSize={13} color="success">
                      Status : {newObjectId?.status}
                    </MDTypography>
                  </Grid>
                  ...
                  >
                    <MDTypography fontSize={13}>
                      {newObjectId?.status} On :{" "}
                      {moment
                        .utc(newObjectId?.lastModifiedOn)
                        .utcOffset("+05:30")
                        .format("DD-MMM-YY HH:mm a")}
                    </MDTypography>
                  </Grid>
                  ...
                  >
                    <MDTypography fontSize={13}>
                      Last Modified By :{" "}
                      {newObjectId?.lastModifiedBy?.first_name}{" "}
                      {newObjectId?.lastModifiedBy?.last_name}
                    </MDTypography>
                  </Grid>
                </>
              )}
                ...
                <>
                  <MDButton
                    ...
                    onClick={() => {
                      updateStatus(
                        newObjectId?._id,
                        newObjectId?.status === "Created"
                          ? "Published"
                          : newObjectId?.status === "Published"
                          ? "Unpublished"
                          : newObjectId?.status === "Unpublished"
                          ? "Published"
                          : ""
                      );
                    }}
              ...
              <Grid container spacing={1}>
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setChildFormState((prevState) => ({
                          ...prevState,
                          blogContent: prevState.blogContent.map(
                            (item, index) =>
                              index === indexToUpdate
                ...
                <Grid item xs={12} md={0.6} xl={1.2} mt={-0.7}>
                  <IoMdAddCircle
                    cursor="pointer"
                    onClick={(e) => {
                      onAddContent(e, childFormState, setChildFormState);
                    }}
                  />
                </Grid>
