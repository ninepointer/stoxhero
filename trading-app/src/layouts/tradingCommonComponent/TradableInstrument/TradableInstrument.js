...
function TradableInstrument({
  ...
  async function subscribeInstrument(instrumentData, addOrRemove) {
    ...
    let maxLot = tradingsymbol?.includes("BANKNIFTY")
      ? maxLot_BankNifty
      : tradingsymbol?.includes("FINNIFTY")
      ? maxLot_FinNifty
      : maxLot_Nifty;

    dispatch({
      type: "setInstrumentName",
      payload: `${strike} ${instrument_type}`,
    });
    if (addOrRemove === "Add") {
      ...
      const response = await fetch(
        `${baseUrl}api/v1/inactiveInstrument/${instrument_token}`,
        {
          method: "PATCH",
          credentials: "include",
          headers: {
  ...
  let content = `${state.instrumentName} is ${
    state.addOrRemoveCheck ? "Added to" : "Removed from"
  } your watchlist`;
  ...
  const renderSuccessSB = (
    <MDSnackbar
      ...
      bgWhite="info"
      sx={{
        borderLeft: `10px solid ${state.addOrRemoveCheck ? "green" : "red"}`,
        borderRadius: "15px",
      }}
    />
  );
        ...
        <TextField
          ...
          InputProps={{
            ...
            endAdornment: (
              <MDButton
                variant="text"
                color={from === paperTrader ? "dark" : "light"}
                onClick={handleClear}
              >
                {state.text && <RxCross2 />}
              </MDButton>
        ...
        <MDBox>
          {state.instrumentsData?.length > 0 &&
            state.instrumentsData.map((elem, index) => {
              ...
              let perticularMarketData = marketDetails.marketData.filter(
                (subElem) => {
                      ...
                      sx={{
                        ...
                        "&:hover": {
                          color:
                            ...
                            "#1e2e4a",
                          backgroundColor:
                            from === paperTrader ? "lightgray" : "lightgray",
                          cursor: "pointer",
                          fontWeight: 600,
                        },
                      }}
                        ...
                        <Grid>
                          <Tooltip title="Buy" placement="top">
                            {!elem.buyState ? (
                              <BuyModel
                                fromTradable={true}
                                chartInstrument={elem.chartInstrument}
                                socket={socket}
                          ...
                          <Grid lg={2.2}>
                            <Tooltip title="Remove Instrument" placement="top">
                              <MDButton
                                ...
                                onClick={() => {
                                  subscribeInstrument(elem, "Remove");
                                }}
                              >
                                -
                              </MDButton>
