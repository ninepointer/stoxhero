...
const SellModel = ({ chartInstrument, isOption, setOpenOptionChain, traderId, socket, subscriptionId, buyState, exchange, symbol, instrumentToken, symbolName, lotSize, maxLot, ltp, fromSearchInstrument, expiry, from, setSellState, exchangeSegment, exchangeInstrumentToken, module }) => {
  ...
  async function handleQuantity(e) {
    ...
    if (sellFormDetails.OrderType === "LIMIT" && sellFormDetails.price && sellFormDetails.Quantity) {
      await checkMargin();
    }
    if (sellFormDetails.OrderType !== "LIMIT" && sellFormDetails.Quantity) {
      await checkMargin();
    }
  ...
  const checkMargin = async () => {
    ...
    const response = await fetch(`${baseUrl}api/v1/marginrequired`, {
      ...
      body: JSON.stringify({
        exchange, symbol, buyOrSell: "SELL", Quantity: runningLotsSymbol > 0 ? Quantity - Math.abs(runningLotsSymbol) : Quantity, Product, OrderType, validity, variety, price, last_price: Number(newLtp)
      })
    });
          ...
          <DialogContent>
            <DialogContentText sx={{ display: "flex", flexDirection: "column", marginLeft: 2, justifyContent: "center", width: "320px" }}>
              ...
              <MDBox sx={{ display: "flex", justifyContent: 'space-between', textAlign: "center", gap: "5px", marginTop: "20px" }}>
                ...
                <MDBox sx={{ backgroundColor: ordertype === "LIMIT" ? "#F44335" : "#FFFFFF", color: ordertype === "LIMIT" ? "#FFFFFF" : "#8D91A8", minHeight: "2px", width: "150px", padding: "5px", borderRadius: "5px", cursor: "pointer", fontWeight: 600, fontSize: "13px", border: ordertype !== "LIMIT" && ".5px solid #8D91A8" }}>
                  Limit
                </MDBox>

                <MDBox onClick={from === "TenX Trader" ? () => { marketHandleChange("SL/SP-M") } : () => { }} sx={{ backgroundColor: ordertype === "SL/SP-M" ? "#F44335" : "#FFFFFF", color: ordertype === "SL/SP-M" ? "#FFFFFF" : "#8D91A8", minHeight: "2px", width: "150px", padding: "5px", borderRadius: "5px", cursor: "pointer", fontWeight: 600, fontSize: "13px", border: ordertype !== "SL/SP-M" && ".5px solid #8D91A8" }}>
                  SL/SP-M
                </MDBox>
              </MDBox>
