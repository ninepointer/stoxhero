...
function ExitPosition({
  ...
  const [filledQuantity, setFilledQuantity] = useState(
    Math.abs(quantity) > maxLot ? maxLot : Math.abs(quantity)
  );
  ...
  const handleClickOpen = async () => {
    ...
    if (Math.abs(quantity) === 0) {
      openSuccessSB(
        "error",
        "You do not have any open position for this symbol."
      );
      return;
    }
    ...
    exitPositionFormDetails.Quantity =
      Math.abs(quantity) > maxLot ? maxLot : Math.abs(quantity);
    setFilledQuantity(
      Math.abs(quantity) > maxLot ? maxLot : Math.abs(quantity)
    );
    ...
    if (exitPositionFormDetails.Quantity % lotSize !== 0) {
      setErrorMessageQuantity(
        `The quantity should be a multiple of ${lotSize}. Try again with ${
          parseInt(exitPositionFormDetails.Quantity / lotSize) * lotSize
        } or ${
          parseInt(exitPositionFormDetails.Quantity / lotSize + 1) * lotSize
        }.`
      );
      return;
    }
  ...
  async function handleQuantity(e) {
    ...
    if (
      e.target.value > maxLot ||
      e.target.value % lotSize !== 0 ||
  ...
  const checkMargin = async () => {
    ...
    const response = await fetch(`${baseUrl}api/v1/marginrequired`, {
      ...
      body: JSON.stringify({
        exchange,
        symbol,
        buyOrSell: checkBuyOrSell === "BUY" ? "SELL" : "BUY",
        Quantity: Number(exitPositionFormDetails.Quantity),
        Product,
        order_type: "MARKET",
        validity,
        variety,
              ...
              >
                ...
                >
                  <MDBox>
                    <RemoveIcon
                      onClick={
                        filledQuantity !== lotSize && filledQuantity > lotSize
                          ? removeQuantity
                          : () => {}
                      }
                      sx={{
                  ...
                  <MDBox>
                    <AddIcon
                      onClick={
                        filledQuantity !== maxLot && filledQuantity < maxLot
                          ? addQuantity
                          : () => {}
                      }
