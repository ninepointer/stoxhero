...
function OverallGrid({
  ...
  const { updateNetPnl, setPnlData, pendingOrderQuantity } =
    useContext(NetPnlContext);
  ...
  let pnlEndPoint =
    from === paperTrader
      ...
      ? "infinityTrade/pnl"
      : from === tenxTrader
      ? `tenX/${subscriptionId}/trade/pnl`
      : from === internshipTrader
      ? `internship/pnl/${subscriptionId}`
      : from === dailyContest
      ? `dailycontest/trade/${subscriptionId}/pnl`
      : from === marginX
      ? `marginxtrade/${subscriptionId}/pnl`
  ...
  useEffect(() => {
    socket?.on(
      `sendOrderResponse${getDetails.userDetails._id.toString()}`,
      (data) => {
  ...
  tradeData.map((subelem) => {
    if (!subelem?._id?.isLimit) {
      ...
      let liveDetail = marketDetails.marketData.filter((elem) => {
        ...
        return (
          subelem._id.instrumentToken == elem.instrument_token ||
          subelem._id.exchangeInstrumentToken == elem.instrument_token
        );
      });
      ...
      let lotSize = subelem._id.symbol?.includes("BANKNIFTY")
        ? lotSize_BankNifty
        : subelem._id.symbol?.includes("FINNIFTY")
        ? lotSize_FinNifty
        : lotSize_Nifty;
      let maxLot = subelem._id.symbol?.includes("BANKNIFTY")
        ? maxLot_BankNifty
        : subelem._id.symbol?.includes("FINNIFTY")
        ? maxLot_FinNifty
        : getDetails?.userDetails?.role?.roleName === infinityTrader
        ? maxLot_Nifty / 2
        : maxLot_Nifty;
      ...
      const instrumentcolor =
        subelem._id.symbol?.slice(-2) == "CE" ? "success" : "error";
      ...
      const productcolor =
        subelem._id.product === "NRML"
          ? "info"
          : subelem._id.product == "MIS"
          ? "warning"
          : "error";
      ...
      if (liveDetail[0]?.change) {
        ...
        obj.change = (
          <MDTypography
            ...
            fontWeight="medium"
          >
            {liveDetail[0]?.average_price
              ? (
                  ((liveDetail[0]?.last_price - liveDetail[0]?.average_price) /
                    liveDetail[0]?.average_price) *
                  100
                )?.toFixed(2) + "%"
              : "0.00%"}
      ...
      obj.exit = (
        <ExitPosition
          ltp={liveDetail[0]?.last_price?.toFixed(2)}
          module={moduleData}
          maxLot={maxLot}
          lotSize={lotSize}
          socket={socket}
          exchangeInstrumentToken={subelem._id.exchangeInstrumentToken}
          subscriptionId={subscriptionId}
          from={from}
          render={render}
          setRender={setRender}
          product={subelem._id.product}
          symbol={subelem._id.symbol}
          quantity={subelem.lots}
          instrumentToken={subelem._id.instrumentToken}
          exchange={subelem._id.exchange}
          setExitState={setExitState}
          exitState={exitState}
        />
      );
      obj.buy = (
        <Buy
          ...
          exchange={subelem._id.exchange}
          instrumentToken={subelem._id.instrumentToken}
          symbolName={subelem._id.symbol?.slice(-7)}
          lotSize={lotSize}
          maxLot={maxLot}
          ltp={liveDetail[0]?.last_price?.toFixed(2)}
  ...
  if (moduleData?.allData?.payoutType === "Percentage") {
    ...
    if (moduleData?.allData?.entryFee > 0) {
      payoutCap =
        (moduleData?.allData?.entryFee *
          moduleData?.allData?.payoutCapPercentage) /
        100;
    } else {
      payoutCap =
        (moduleData?.allData?.portfolio?.portfolioValue *
          moduleData?.allData?.payoutCapPercentage) /
        100;
    }
    const payout = Math.min(
      (moduleData?.allData?.payoutPercentage *
        (totalGrossPnl - totalTransactionCost)) /
        100,
      payoutCap
    );
    if (moduleData?.allData?.entryFee > 0) {
      if (payout - moduleData?.allData?.entryFee > 0) {
        tdsAmount =
          ((payout - moduleData?.allData?.entryFee) *
    ...
    if (myRank) {
      ...
      for (let elem of rewards) {
        if (
          Number(myRank) >= Number(elem.rankStart) &&
          Number(myRank) <= Number(elem.rankEnd)
        ) {
          myReward = "+₹" + elem.prize;
          break;
        } else {
      ...
      {rows.length === 0 ? (
        ...
        >
          ...
          <Typography mb={2} fontSize={15} color="grey">
            Add instruments and start trading.
          </Typography>
          <MDButton
            variant="outlined"
            size="small"
            ...
            >
              <Grid
                ...
                lg={
                  from === marginX
                    ? 1.42
                    : from === dailyContest &&
                      moduleData?.allData?.rewardType !== "Goodies"
                    ? 1.71
                    : 3
                }
                display="flex"
              ...
              >
                <MDTypography
                  fontSize={".70rem"}
                  backgroundColor="#CCCCCC"
                  color={`${totalGrossPnl > 0 ? "success" : "error"}`}
                  style={{
                    borderRadius: "5px",
                    padding: "5px",
                    fontWeight: "600",
              ...
              {from === marginX && (
                <>
                  ...
                  >
                    ...
                    >
                      Invested Amount: {moduleData?.entryFee}
                    </MDTypography>
                  </Grid>
                  ...
                  >
                    <MDTypography
                      ...
                      backgroundColor="#CCCCCC"
                      color={`${
                        (totalGrossPnl - totalTransactionCost) / xFactor > 0
                          ? "success"
                          : "error"
                      }`}
                      style={{
                    ...
                    >
                      Return:{" "}
                      {(totalGrossPnl - totalTransactionCost) / xFactor >= 0.0
                        ? "+₹" +
                          (
                            (totalGrossPnl - totalTransactionCost) /
                            xFactor
                        ...
                        : "-₹" +
                          (
                            -(totalGrossPnl - totalTransactionCost) / xFactor
                          ).toFixed(2)}
                    </MDTypography>
                  </Grid>
