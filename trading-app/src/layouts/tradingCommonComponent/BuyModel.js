...
const BuyModel = ({
  ...
  let dummyOrderId = `${date.getFullYear() - 2000}${String(
    date.getMonth() + 1
  ...
  const handleClickOpen = async () => {
    if (!ltp && !fromTradable) {
      openSuccessSB(
        "error",
        "This instrument is expired. Please trade on valid instrument."
      );
      return;
    }
  ...
  const stopProfit = async (e) => {
    setBuyFormDetails({
      ...buyFormDetails,
      stopProfitPrice: Number(e.target.value),
    });
    const inputPrice = Number(e.target.value);
    if (inputPrice < 0) {
    ...
    if (Number(newLtp) > Number(e.target.value)) {
      setErrorMessageStopProfit(
        "Stop Profit price should be greater then LTP."
      );
    }
    if (e.target.value === "") {
  ...
  async function buyFunction(e, uId) {
    ...
    if (buyFormDetails.Quantity > maxLot) {
      setErrorMessageQuantity(
        `Quantity must be lower than max limit: ${maxLot}`
      );
      return;
    }
    if (buyFormDetails.Quantity % lotSize !== 0) {
      setErrorMessageQuantity(
        `The quantity should be a multiple of ${lotSize}. Try again with ${
          parseInt(buyFormDetails.Quantity / lotSize) * lotSize
        } or ${parseInt(buyFormDetails.Quantity / lotSize + 1) * lotSize}.`
      );
      return;
    }
    if (buyFormDetails.Quantity < lotSize) {
      setErrorMessageQuantity(
        `Quantity must be greater than min limit: ${lotSize}`
      );
      return;
    }

    if (
      buyFormDetails.order_type === "SL/SP-M" &&
      !buyFormDetails.stopLossPrice &&
      !buyFormDetails.stopProfitPrice
    ) {
  ...
  async function placeOrder() {
    ...
    const res = await fetch(`${baseUrl}api/v1/${endPoint}`, {
      ...
      body: JSON.stringify({
        ...
        Quantity,
        stopLoss,
        contestId: module?.data,
        battleId: subscriptionId,
        Product,
        order_type,
        TriggerPrice,
        stopProfitPrice,
  ...
  async function removeInstrument() {
    const response = await fetch(
      `${baseUrl}api/v1/inactiveInstrument/${instrumentToken}`,
      {
        method: "PATCH",
        credentials: "include",
        headers: {
  ...
  async function handleQuantity(e) {
    ...
    if (
      Number(e.target.value) <= Math.abs(runningLotsSymbol) &&
      runningLotsSymbol < 0
    ) {
    ...
    if (
      Number(e.target.value) > maxLot ||
      Number(e.target.value) % lotSize !== 0 ||
      Number(e.target.value) < lotSize
    ) {
      return;
    }

    if (
      buyFormDetails.order_type === "LIMIT" &&
      buyFormDetails.price &&
      checkQuantity
    ) {
      await checkMargin();
    }
  ...
  const checkMargin = async () => {
    ...
    const { Quantity, Product, order_type, validity, variety, price } =
      buyFormDetails;
    const response = await fetch(`${baseUrl}api/v1/marginrequired`, {
      ...
      body: JSON.stringify({
        ...
        Quantity:
          runningLotsSymbol < 0
            ? Quantity - Math.abs(runningLotsSymbol)
            : Quantity,
        Product,
        order_type,
        validity,
  ...
  async function removeQuantity() {
    ...
    if (
      buyFormDetails.Quantity <= Math.abs(runningLotsSymbol) &&
      runningLotsSymbol < 0
    ) {
      return setMargin("0.00");
    }
  ...
  async function addQuantity() {
    if (buyFormDetails.Quantity % lotSize !== 0) {
      setCheckQuantity(
        parseInt(buyFormDetails.Quantity / lotSize + 1) * lotSize
      );
      buyFormDetails.Quantity =
        parseInt(buyFormDetails.Quantity / lotSize + 1) * lotSize;
    } else {
  ...
  function notAvailable(value) {
    ...
    openSuccessSB(
      "notAvailable",
      "This feature is not available on stoxhero currently."
    );
  }
  ...
  return (
    <div>
      <MDButton
        size="small"
        color="info"
        sx={{ marginRight: 0.5, minWidth: 2, minHeight: 3 }}
        ...
        >
            ...
            >
              ...
              >
                ...
                >
                  <MDBox>
                    <RemoveIcon
                      onClick={
                        checkQuantity !== lotSize && checkQuantity > lotSize
                          ? removeQuantity
                          : () => {}
                      }
                      sx={{
                  ...
                  <MDBox>
                    <AddIcon
                      onClick={
                        checkQuantity !== maxLot && checkQuantity < maxLot
                          ? addQuantity
                          : () => {}
                      }
                ...
                <TextField
                  ...
                  sx={{ width: "140px", innerHeight: "1px" }}
                  type="number"
                  value={buyFormDetails.price === 0 ? "" : buyFormDetails.price}
                />
              </MDBox>
                ...
                >
                  <TextField
                    ...
                    disabled={
                      ...
                      buyFormDetails.order_type === "LIMIT" ||
                      (runningLotsSymbol < 0 &&
                        checkQuantity <= Math.abs(runningLotsSymbol))
                    }
                    label="SL price"
                    variant="outlined"
                    onChange={(e) => {
                    ...
                    type="number"
                    value={
                      buyFormDetails.stopLossPrice === 0
                        ? ""
                        : buyFormDetails.stopLossPrice
                    }
                  />
                  ...
                  <TextField
                    ...
                    type="number"
                    value={
                      buyFormDetails.stopProfitPrice === 0
                        ? ""
                        : buyFormDetails.stopProfitPrice
                    }
                  />
              ...
              >
                <Typography fontSize={15} color={"error"}>
                  {" "}
                  {buyFormDetails.stopLossPrice &&
                    errorMessageStopLoss &&
                    errorMessageStopLoss}
                </Typography>
                <Typography fontSize={15} color={"error"}>
                  {buyFormDetails.stopProfitPrice &&
                    errorMessageStopProfit &&
                    errorMessageStopProfit}
                </Typography>
                <Typography fontSize={15} color={"error"}>
              ...
              >
                <MDBox
                  onClick={() => {
                    marketHandleChange("MARKET");
                  }}
                  sx={{
                    backgroundColor:
                      ordertype === "MARKET" ? "#1A73E8" : "#FFFFFF",
                    color: ordertype === "MARKET" ? "#FFFFFF" : "#8D91A8",
                    minHeight: "2px",
                    width: "150px",
                    padding: "5px",
                    borderRadius: "5px",
                    ...
                    fontWeight: 600,
                    fontSize: "13px",
                    border: ordertype !== "MARKET" && ".5px solid #8D91A8",
                  }}
                >
                  Market
                </MDBox>
                ...
                <MDBox
                  ...
                  sx={{
                    backgroundColor:
                      ordertype === "LIMIT" ? "#1A73E8" : "#FFFFFF",
                    color: ordertype === "LIMIT" ? "#FFFFFF" : "#8D91A8",
                    minHeight: "2px",
                    width: "150px",
                    padding: "5px",
                    borderRadius: "5px",
                    ...
                    fontWeight: 600,
                    fontSize: "13px",
                    border: ordertype !== "LIMIT" && ".5px solid #8D91A8",
                  }}
                >
                  Limit
                </MDBox>
                ...
                <MDBox
                  ...
                  sx={{
                    backgroundColor:
                      ordertype === "SL/SP-M" ? "#1A73E8" : "#FFFFFF",
                    color: ordertype === "SL/SP-M" ? "#FFFFFF" : "#8D91A8",
                    minHeight: "2px",
                    width: "150px",
                    padding: "5px",
                    borderRadius: "5px",
                    ...
                    fontWeight: 600,
                    fontSize: "13px",
                    border: ordertype !== "SL/SP-M" && ".5px solid #8D91A8",
                  }}
                >
                  SL/SP-M
                </MDBox>
                  ...
                  >
                    ...
                    <span>
                      <RefreshIcon
                        onClick={
                          buyFormDetails.Quantity <=
                            Math.abs(runningLotsSymbol) && runningLotsSymbol < 0
                            ? () => {}
                            : async () => {
                                await checkMargin();
                              }
          ...
          <DialogActions>
            ...
            <MDButton
              ...
              disabled={
                (buyFormDetails.stopLossPrice &&
                  Number(ltp) < buyFormDetails.stopLossPrice) ||
                (buyFormDetails.stopProfitPrice &&
                  Number(ltp) > buyFormDetails.stopProfitPrice) ||
                (buyFormDetails.price && buyFormDetails.price > Number(ltp))
              }
              autoFocus
              variant="contained"
              color="info"
