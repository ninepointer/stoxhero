...
export default function LiveTenXSubscribers() {
  let baseUrl =
  ...
  const [liveTenXSubscribers, setLiveTenXSubscribers] = useState([]);

  useEffect(() => {
    axios
      .get(
        `${baseUrl}api/v1/tenx/livetenxsubscribers?skip=${skip}&limit=${limitSetting}`,
        {
      ...
      .then((res) => {
        setLiveTenXSubscribers(res.data.data);
  ...
  function backHandler() {
    ...
    setLiveTenXSubscribers([]);
    setIsLoading(true);
    axios
      .get(
        `${baseUrl}api/v1/tenx/livetenxsubscribers?skip=${
          skip - limitSetting
        }&limit=${limitSetting}`,
        {
          withCredentials: true,
  ...
  return (
    ...
    >
      <Grid container spacing={1}>
        ...
        {!isLoading ? (
          liveTenXSubscribers?.map((elem, index) => {
            return (
              <Grid
                container
                mt={1}
