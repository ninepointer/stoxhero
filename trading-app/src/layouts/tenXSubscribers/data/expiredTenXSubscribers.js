...
export default function ExpiredTenXSubscribers() {
  let baseUrl =
  ...
  const [expiredTenXSubscribers, setExpiredTenXSubscribers] = useState([]);

  useEffect(() => {
    axios
      .get(
        `${baseUrl}api/v1/tenx/expiredtenxsubscribers?skip=${skip}&limit=${limitSetting}`,
        {
      ...
      .then((res) => {
        setExpiredTenXSubscribers(res.data.data);
  ...
  function backHandler() {
    ...
    setExpiredTenXSubscribers([]);
    setIsLoading(true);
    axios
      .get(
        `${baseUrl}api/v1/tenx/expiredtenxsubscribers?skip=${
          skip - limitSetting
        }&limit=${limitSetting}`,
        {
          withCredentials: true,
  ...
  return (
    ...
    >
      <Grid container spacing={1}>
        ...
        {!isLoading ? (
          expiredTenXSubscribers?.map((elem, index) => {
            return (
              <Grid
                container
                mt={1}
