...
function MyRank({ socket, id, data, setRank }) {
  ...
  if (data?.allData?.payoutType === "Percentage") {
    ...
    if (data?.allData?.entryFee > 0) {
      ...
      payoutCap =
        (data?.allData?.portfolioValue * data?.allData?.payoutCapPercentage) /
        100;
    }
    myReward = Math.min(
      payoutCap,
      (pnl?.netPnl * data?.allData?.payoutPercentage) / 100 > 0
        ? (pnl?.netPnl * data?.allData?.payoutPercentage) / 100
        : 0
    );
  } else {
