...
function TradeViewTenX() {
  ...
  const BatchId =
    getDetails?.userDetails.internshipBatch[batchArr.length - 1]?._id || "123";
      ...
      if (!id) {
        if (location?.pathname == "/workshop/trade") {
          const res = await axios.get(
            `${baseUrl}api/v1/internbatch/currentworkshop`,
        ...
        {id ? (
          ...
          >
            <MDTypography color="white">
              Your internship has ended. Continue using the app or enroll in
              other internships.
            </MDTypography>
          </MDBox>
        )}
