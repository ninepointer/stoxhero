...
function CreateContest({ createRuleForm, setCreateRuleForm }) {
  ...
  React.useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/contestrule/${id}`)
      .then((res) => {
  ...
  async function onNext(e, formState) {
    ...
    if (!formState?.ruleName || !formState?.status) {
      setTimeout(() => {
        setCreating(false);
    ...
    if (!data.error) {
      ...
      openSuccessSB(
        data.message,
        `Contest Rule Created with name: ${data.data?.ruleName}`
      );
    } else {
      setTimeout(() => {
        setCreating(false);
  ...
  async function onAdd(e, childFormState, setChildFormState) {
    ...
    if (data.status === 422 || data.error || !data) {
      ...
      openSuccessSB(
        "New Rule Added",
        "New Rule line item has been added in the contest rule"
      );
      setTimeout(() => {
        setSaving(false);
        setEditing(false);
                ...
                <Grid container spacing={1} mt={0.5}>
                  ...
                  >
                    <IoMdAddCircle
                      cursor="pointer"
                      onClick={(e) => {
                        onAdd(e, formState, setFormState);
                      }}
                    />
                  </Grid>
                </Grid>
            ...
            {isSubmitted && (
              <Grid item xs={12} md={6} xl={12}>
                ...
                <MDBox>
                  <RuleData
                    id={newObjectId}
                    addRuleObject={addRuleObject}
                    setAddRuleObject={setAddRuleObject}
                  />
                </MDBox>
              </Grid>
            )}
