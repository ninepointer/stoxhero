...
function Billing() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${baseUrl}api/v1/gettraderpnlformargin/${id}`)
      .then((res) => {
        console.log(res.data);
    ...
    axios
      .get(`${baseUrl}api/v1/gettraderpnlforavailablemargin/${id}`)
      .then((res) => {
        console.log(res.data);
  ...
  let totalCreditString =
    totalCredit >= 0
      ? "+₹" + totalCredit.toLocaleString()
      : "-₹" + (-totalCredit).toLocaleString();
  let lifetimenetpnl = lifetimePNL[0]
    ? Number(lifetimePNL[0].npnl.toFixed(0))
    : 0;
  ...
  let openingBalanceString =
    openingBalance >= 0
      ...
      : "-₹" + (-Number(openingBalance)).toLocaleString();
  let availableMarginpnl = availableMarginPNL[0]
    ? Number(availableMarginPNL[0].npnl.toFixed(0))
    : 0;
