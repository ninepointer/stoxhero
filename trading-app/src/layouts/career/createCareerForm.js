...
function Index() {
  ...
  async function onAddFeature(e, childFormState, setChildFormState) {
    ...
    if (data.status === 422 || data.error || !data) {
      ...
      openSuccessSB(
        "New Role and Responsibility Added",
        "New Role and Responsibility line item has been added in the career"
      );
      setTimeout(() => {
        setSaving(false);
        setEditing(false);
  ...
  async function onEdit(e, formState) {
    ...
    const { jobTitle, jobDescription, jobLocation, jobType, status } =
      formState;
        ...
        <MDBox pl={2} pr={2} mt={4}>
          ...
          >
            ...
            <MDTypography variant="caption" fontWeight="bold" color="text">
              Link:{" "}
              {Url + "careers/careerform/" + id?.jobTitle.replace(/\s/g, "%20")}
            </MDTypography>
          </MDBox>
            ...
            <Grid container spacing={1} mt={0.5} mb={0} xs={12} md={9} xl={12}>
              ...
              <Grid item xs={12} md={6} xl={9}>
                <TextField
                  ...
                  // defaultValue={portfolioData?.portfolioName}
                  defaultValue={
                    editing ? formState?.jobDescription : id?.jobDescription
                  }
                  onChange={(e) => {
                    setFormState((prevState) => ({
                      ...prevState,
          ...
          <Grid container mt={2} xs={12} md={12} xl={12}>
            ...
            {(id || newObjectId) && (
              <Grid item xs={12} md={12} xl={12} mt={2}>
                <MDBox>
                  <CareerApplication
                    career={newObjectId ? newObjectId : id?._id}
                    action={action}
                    setAction={setAction}
                  />
                </MDBox>
