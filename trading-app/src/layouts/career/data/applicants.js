...
export default function Applicants({ career, action, setAction }) {
  ...
  async function getCareerApplications() {
    let call1 = axios.get(
      `${baseUrl}api/v1/career/careerapplications/${career}`,
      {
  ...
  async function rejectApplication(id) {
    try {
      const res = await axios.patch(
        `${baseUrl}api/v1/career/reject/${id}`,
        {},
        {
          withCredentials: true,
          headers: {
  ...
  let columns = [
    ...
    {
      Header: "In an Active Batch",
      accessor: "inActiveBatch",
      align: "center",
    },
  ...
  careerApplications?.map((elem, index) => {
    ...
    featureObj.assign = (
      <MDButton
        ...
        color="text"
        fontWeight="medium"
        disabled={elem?.applicationStatus != "Applied"}
        onClick={() => {
          handleOpen(elem._id, `${elem.first_name} ${elem.last_name}`);
        }}
      >
        Assign GD
      </MDButton>
    ...
    featureObj.dob = (
      <MDButton
        ...
        fontWeight="medium"
      >
        {new Date(elem?.dob).toLocaleDateString("en-IN", {
    ...
    featureObj.appliedon = (
      <MDTypography
        ...
        fontWeight="medium"
      >
        {new Date(elem?.appliedOn).toLocaleDateString("en-IN", {
          timeZone: "Asia/Kolkata",
        })}{" "}
        {new Date(elem?.appliedOn)
          ...
          >
            <Tooltip title="Download CSV">
              <MDBox
                ...
                onClick={() => {
                  handleDownload(pnlData, `career-applications`);
                }}
              >
                <DownloadIcon />
              </MDBox>
