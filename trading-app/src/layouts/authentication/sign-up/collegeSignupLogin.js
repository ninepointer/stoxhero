...
function Cover(props) {
  ...
  async function formSubmit() {
    ...
    const { first_name, last_name, email, mobile, referrerCode, rollno } =
      formstate;
  ...
  async function otpConfirmation() {
    ...
    if (data.status === "Success") {
      ...
      if (userData?.role?.roleName === adminRole) {
        ...
        const from =
          location.state?.from ||
          `/${userData?.collegeDetails?.college?.route}/home`;
        navigate(from);
                                ...
                                {!showEmailOTP && (
                                  <Grid item xs={12} md={12} lg={6}>
                                    <TextField
                                      ...
                                      fullWidth
                                      onChange={(e) => {
                                        formstate.rollno = e.target.value;
                                      }}
                                    />
                                  </Grid>
                                )}
