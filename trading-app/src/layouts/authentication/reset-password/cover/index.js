...
function Cover() {
  ...
  const resendOTP = async () => {
    ...
    const res = await fetch(`${baseUrl}api/v1/schoolgenerateotp`, {
  ...
  const resetPassword = async () => {
    ...
    const res = await fetch(`${baseUrl}api/v1/schoolresetpassword`, {
                    ...
                    {!editable &&
                      <>
                        ...
                        < MDBox mb={4} display="flex" flexDirection="column" alignItems="center" justifyContent="center">
                          ...
                          {(onReset && (formstate.confirm_password) || (formstate.password != formstate.confirm_password)) && <Typography style={{ color: "red", fontSize: 15 }}>Password match failed</Typography>}
                        </MDBox>

                        < MDBox mb={4} display="flex" flexDirection="column" alignItems="center" justifyContent="center">
                          <MDInput type="text" label="Confirm Password*" variant="standard" fullWidth onChange={(e) => { formstate.confirm_password = e.target.value }} />
                          {(onReset && (formstate.password) || (formstate.password != formstate.confirm_password)) && <Typography style={{ color: "red", fontSize: 15 }}>Password match failed</Typography>}
                        </MDBox>
