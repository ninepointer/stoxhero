...
function SwitchWindow(props) {
  ...
  const getContest = async () => {
    const res = await axios.get(`${apiUrl}dailycontest/contest/${props.id}`, {
  ...
  let columns = [
    ...
    {
      Header: "Total TestZone",
      accessor: "totalContest",
      width: "12.5%",
      align: "center",
    },
    {
      Header: "Paid TestZone",
      accessor: "paidContest",
      width: "12.5%",
      align: "center",
    },
    {
      Header: "Free TestZone",
      accessor: "freeContest",
      width: "12.5%",
      align: "center",
    },
    {
  ...
  useEffect(() => {
    axios
      .get(
        `${baseUrl}api/v1/dailycontest/live/singleswitchcontest/${props.id}`,
        { withCredentials: true }
      )
      .then((res) => {
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/dailycontest/usercontestdata/${props.id}`, {
        withCredentials: true,
      })
      .then((res) => {
  ...
  async function switchUser(userId) {
    axios
      .get(`${baseUrl}api/v1/contestrealmocksingleuser/${userId}/${props.id}`, {
        withCredentials: true,
      })
      .then((res) => {
  ...
  if (allTrade.length !== 0) {
    ...
    for (let i = 0; i < allTrade.length; i++) {
      if (mapForParticularUser.has(allTrade[i].traderId)) {
        let marketDataInstrument = marketData.filter((elem) => {
          return (
            elem.instrument_token == Number(allTrade[i].symbol) ||
            elem.instrument_token == Number(allTrade[i].exchangeInstrumentToken)
          );
        });
          ...
          return (
            ...
            (elem.instrument_token == Number(allTrade[i].symbol) ||
              elem.instrument_token ==
                Number(allTrade[i].exchangeInstrumentToken))
          );
        });
    ...
    finalTraderPnl.map((subelem, index) => {
      ...
      let runninglotscolor =
        subelem.runninglots > 0
          ...
          ? "error"
          : "dark";
      let traderbackgroundcolor =
        subelem.runninglots != 0 ? "white" : "#e0e1e5";
      ...
      obj.mockLive = (
        ...
        >
          <Switch
            checked={subelem.isLive}
            onChange={() => {
              switchUser(subelem?.userId);
            }}
          />
        </MDTypography>
      );
    ...
    const totalnetPnlcolor =
      totalGrossPnlGrid - totalTransactionCost >= 0 ? "success" : "error";
    ...
    obj.grossPnl = (
      <MDTypography
        ...
        fontWeight="medium"
      >
        {totalGrossPnlGrid >= 0.0
          ? "+₹" + totalGrossPnlGrid.toFixed(2)
          : "-₹" + (-totalGrossPnlGrid).toFixed(2)}
      </MDTypography>
    );
    ...
    obj.netPnl = (
      <MDTypography
        ...
        fontWeight="medium"
      >
        {totalGrossPnlGrid - totalTransactionCost >= 0.0
          ? "+₹" + (totalGrossPnlGrid - totalTransactionCost).toFixed(2)
          : "-₹" + (-(totalGrossPnlGrid - totalTransactionCost)).toFixed(2)}
      </MDTypography>
    );
  ...
  return (
    <MDBox display="flex" justifyContent="center" flexDirection="column" m={1}>
      ...
      >
        <MDBox>
          <MDTypography fontSize={15} fontWeight="bold" color="light">
            Switch Window
          </MDTypography>
        </MDBox>
