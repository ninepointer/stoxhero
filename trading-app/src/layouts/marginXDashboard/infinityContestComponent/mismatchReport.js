...
function MismatchDetails({ socket, id }) {
  ...
  let columns = [
    ...
    {
      Header: "App Running Lots",
      accessor: "apprunninglots",
      width: "10%",
      align: "center",
    },
    {
      Header: "Zerodha Running Lots",
      accessor: "zerodharunninglots",
      width: "10%",
      align: "center",
    },
    {
      Header: "app gross p&l",
      accessor: "appgrosspnl",
      width: "10%",
      align: "center",
    },
    {
      Header: "zerodha gross p&l",
      accessor: "zerodhagrosspnl",
      width: "10%",
      align: "center",
    },
  ];
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/getPositions`)
      .then((res) => {
    ...
    axios
      .get(`${baseUrl}api/v1/dailycontest/livePnlCompanymismatch`, {
  ...
  if (OpenPositionData.length !== 0) {
    OpenPositionData.map((elem) => {
      let appPnlData = tradeData.filter((element) => {
        return (
          (element?.exchangeInstrumentToken == elem?.ExchangeInstrumentId ||
            element?.instrumentToken == elem?.ExchangeInstrumentId) &&
          elem?.ProductType == element?.product
        );
      });
      ...
      let liveDetail = marketData.filter((element) => {
        ...
        return (
          element !== undefined &&
          element?.instrument_token == appPnlData[0]?.instrumentToken
        );
      });

      let XTSPnl =
        Number(elem?.NetAmount) +
        Number(elem?.Quantity) * liveDetail[0]?.last_price;
      let updatedValue =
        appPnlData[0]?.amount + appPnlData[0]?.lots * liveDetail[0]?.last_price;
      ...
      if (
        appPnlData[0]?.exchangeInstrumentToken === elem?.ExchangeInstrumentId
      ) {
        appAndZerodhaSameSymbolRunningLotTotal += Math.abs(elem?.Quantity);
        appAndZerodhaSameSymbolPnlTotal += XTSPnl;
      }
      otmRunningLotsTotal = Math.abs(
        zerodharunninglotsTotal - apprunninglotsTotal
      );
      ...
      const instrumentcolor = elem?.TradingSymbol.includes("CE")
        ? "success"
        : "error";
    ...
    const zerodhaplusapplotsbgcolor =
      appAndZerodhaSameSymbolRunningLotTotal == apprunninglotsTotal
        ? "#e0e1e5"
        : "#ffff00";
    const otmgrosspnlcolor =
      zerodhaPnlTotal - appPnlTotal >= 0 ? "success" : "error";
    ...
    obj.product = (
      ...
      >
        OTM GP&L :{" "}
        {zerodhaPnlTotal - appPnlTotal >= 0.0
          ? "+₹" + (zerodhaPnlTotal - appPnlTotal).toFixed(2)
          : "-₹" + (-(zerodhaPnlTotal - appPnlTotal)).toFixed(2)}
      </MDTypography>
    );
    obj.instrument = (
      <MDTypography
        ...
        variant="caption"
        color="dark"
        backgroundColor={zerodhaplusapplotsbgcolor}
        borderRadius="5px"
        padding="5px"
        fontWeight="medium"
      >
      ...
      {tradeData.length > 0 && (
        ...
        >
          <MDBox bgColor="grey" p={1} borderRadius={3}>
            <MDTypography fontSize={15} color="light" fontWeight="bold">
              Mismatch Report
            </MDTypography>
          </MDBox>
