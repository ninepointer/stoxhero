...
export default function Dialogue({subscription ,amount, name, id, walletCash, setCheckPayment, checkPayment, allowRenewal}) {
  ...
  const buySubscription = async () => {
    if(walletCash <  Number(amount-discountAmount)){
    ...
    const res = await fetch(`${baseUrl}api/v1/userwallet/deduct`, {
      ...
      body: JSON.stringify({
        subscriptionAmount: Number(amount-discountAmount).toFixed(2), subscriptionName: name, subscribedId: id, coupon:verifiedCode
      })
    });
  ...
  const initiatePayment = async() => {
    ...
    try{
      const res = await axios.post(`${apiUrl}payment/initiate`,{amount:Number((subs_amount-discountAmount)*100) + subs_actualAmount*100, redirectTo:window.location.href, paymentFor:'TenX', productId:id, coupon:verifiedCode},{withCredentials: true});
