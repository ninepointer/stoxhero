...
export default function Dialogue({
  ...
  const handleClickOpen = () => {
    window.webengage.track("tenx_payment_clicked", {
  ...
  const buySubscription = async () => {
    if (walletCash < Number(amount - discountAmount - bonusRedemption)) {
      window.webengage.track("tenx_payment_low_balance", {
        user: getDetails?.userDetails?._id,
        subscriptionId: id?.toString(),
    ...
    window.webengage.track("tenx_payment_process_clicked", {
      user: getDetails?.userDetails?._id,
      subscriptionId: id,
      amount: Number(amount - discountAmount - bonusRedemption),
    });
  ...
  const redeemableBonus =
    Math.min(
      ((subs_amount - discountAmount) * setting?.maxBonusRedemptionPercentage) /
        100,
      bonusCash / setting?.bonusToUnitCashRatio ?? 1
    ) ?? 0;
  const bonusRedemption = checked
    ? Math.min(
        ((subs_amount - discountAmount) *
          setting?.maxBonusRedemptionPercentage) /
          100,
        bonusCash / setting?.bonusToUnitCashRatio ?? 1
      )
    : 0;
  ...
  const initiatePayment = async () => {
    ...
    try {
      const res = await axios.post(
        ...
        {
          amount:
            Number((subs_amount - discountAmount - bonusRedemption) * 100) +
            subs_actualAmount * 100,
  ...
  const applyPromoCode = async () => {
    window.webengage.track("tenx_apply_couponcode_clicked", {
      user: getDetails?.userDetails?._id,
      subscriptionId: id,
      amount: Number(amount - discountAmount - bonusRedemption),
    });
      ...
      {isSubscribed ? (
        ...
        >
          ...
          >
            <Grid
              item
              xs={isSubscribed ? 12 : 0}
              md={isSubscribed ? 12 : 0}
              lg={isSubscribed ? 12 : 0}
              display="flex"
              justifyContent="center"
              alignItems="center"
              style={{ width: "100%" }}
        ...
        >
          <Grid
            container
            spacing={1}
            xs={messege.thanksMessege ? 12 : 0}
            md={messege.thanksMessege ? 12 : 0}
            lg={messege.thanksMessege ? 12 : 0}
            display="flex"
            justifyContent="center"
            alignItems="center"
            flexDirection="row"
                      ...
                      {value == "wallet" && (
                        ...
                        >
                            ...
                            <MDBox
                              ...
                              onClick={() => {
                                window.webengage.track(
                                  "tenx_intent_to_apply_couponcode_clicked",
                                  {
                                    user: getDetails?.userDetails?._id,
                                    subscriptionId: id,
                                    amount: Number(
                          ...
                          >
                            ...
                            {Number(
                              subs_amount - discountAmount - bonusRedemption
                            ).toFixed(2)}
                          </Typography>
                          {bonusCash > 0 && (
                            <MDBox
        ...
        {value !== "wallet" && (
          <DialogActions>
            ...
            <MDButton color={"success"} onClick={initiatePayment} autoFocus>
              {`Pay ₹${
                Number(subs_amount - discountAmount - bonusRedemption) +
                subs_actualAmount
              } securely`}
            </MDButton>
          </DialogActions>
