...
function CreateContest({
  ...
  React.useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/contestrule`)
      .then((res) => {
  ...
  async function onSubmit(e, formState) {
    ...
    if (
      ...
      !formState?.stockType ||
      !formState?.contestOn ||
      !formState.contestRule ||
      !formState?.contestStartDate ||
      !formState?.contestEndDate ||
      !formState?.entryOpeningDate ||
      !formState?.entryClosingDate ||
      !formState?.entryFee?.amount ||
      !formState?.entryFee?.currency ||
      !formState?.status
    ) {
      setTimeout(() => {
        setCreating(false);
  ...
  async function onAddReward(e, childFormState, setChildFormState) {
    ...
    if (
      !childFormState?.rankStart ||
      !childFormState?.rankEnd ||
      !childFormState?.reward ||
    ...
    if (data.status === 422 || data.error || !data) {
      ...
      openSuccessSB(
        "New Reward Added",
        "New Reward line item has been added in the contest"
      );
      setTimeout(() => {
        setSaving(false);
        setEditing(false);
          ...
          <Grid container spacing={1} mt={0.5}>
            ...
            <Grid item xs={12} md={6} xl={3}>
              <FormControl sx={{ width: "100%" }}>
                ...
                <Select
                  ...
                  defaultValue={
                    oldObjectId ? contestData.stockType : formState?.stockType
                  }
                  disabled={(isSubmitted || id) && (!editing || saving)}
                  onChange={(e) => {
                    setFormState((prevState) => ({
            ...
            <Grid item xs={12} md={6} xl={3}>
              <TextField
                ...
                label="Contest On *"
                defaultValue={
                  oldObjectId ? contestData?.contestOn : formState?.contestOn
                }
                fullWidth
                onChange={(e) => {
                  setFormState((prevState) => ({
                ...
                <DemoContainer components={["MobileDateTimePicker"]}>
                  <DemoItem>
                    <MobileDateTimePicker
                      ...
                      defaultValue={dayjs(
                        oldObjectId
                          ? contestData?.entryOpeningDate
                          : setFormState?.entryOpeningDate
                      )}
                      onChange={(e) => {
                        setFormState((prevState) => ({
                          ...prevState,
                ...
                <DemoContainer components={["MobileDateTimePicker"]}>
                  <DemoItem>
                    <MobileDateTimePicker
                      ...
                      defaultValue={dayjs(
                        oldObjectId
                          ? contestData?.entryClosingDate
                          : setFormState?.entryClosingDate
                      )}
                      onChange={(e) => {
                        setFormState((prevState) => ({
                          ...prevState,
            ...
            <Grid item xs={12} md={3} xl={3} mb={-3}>
              <FormControl sx={{ minHeight: 10, minWidth: 245 }}>
                ...
                <Select
                  ...
                  disabled={(isSubmitted || id) && (!editing || saving)}
                  defaultValue={
                    oldObjectId ? contestData?.contestRule?.ruleName : ruleName
                  }
              ...
              {(isSubmitted || isObjectNew) && !editing && (
                <>
                  ...
                  <MDButton
                    ...
                    disabled={editing}
                    onClick={() => {
                      id ? navigate("/contests") : setIsSubmitted(false);
                    }}
                  >
                    Back
                  </MDButton>
                ...
                <Grid container spacing={1}>
                  ...
                  <Grid item xs={12} md={0.6} xl={1.2} mt={-0.7}>
                    <IoMdAddCircle
                      cursor="pointer"
                      onClick={(e) => {
                        onAddReward(e, formState, setFormState);
                      }}
                    />
                  </Grid>
                </Grid>
              ...
              <Grid item xs={12} md={12} xl={12} mt={2}>
                <MDBox>
                  <RewardsData
                    ...
                    addRewardObject={addRewardObject}
                    setAddRewardObject={setAddRewardObject}
                  />
                </MDBox>
              </Grid>
            )}
            ...
            {(isSubmitted || oldObjectId) && (
              <Grid item xs={12} md={12} xl={12} mt={2}>
                <MDBox>
                  <LinkedContestRuleData
                    linkedRuleId={linkedContestRule}
                    setLinkedRuleId={setLinkedContestRule}
