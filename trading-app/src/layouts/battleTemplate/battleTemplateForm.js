...
function Index() {
  ...
  async function onSubmit(e, formState) {
    ...
    try {
      ...
      if (
        !formState.battleTemplateName ||
        !formState.portfolioValue ||
        !formState.entryFee ||
        !formState.winnerPercentage ||
        !formState.platformCommissionPercentage ||
        !formState.minParticipants ||
        !formState.gstPercentage ||
        !formState.battleType ||
        !formState.battleTemplateType ||
        !formState.status
      ) {
        setTimeout(() => {
          setCreating(false);
  ...
  async function createRankingPayout(e, childFormState, setChildFormState) {
    ...
    const res = await fetch(
      `${baseUrl}api/v1/battletemplates/${template?._id}`,
      {
        method: "PATCH",
        credentials: "include",
        headers: {
        ...
        body: JSON.stringify({
          rankingPayout: [
            ...template?.rankingPayout,
            { rank, rewardPercentage },
          ],
        }),
      }
    );
  ...
  async function editRankingPayout(e, childFormState, setChildFormState) {
    ...
    console.log(
      "new rewards",
      ranks.map((elem) => {
        return { rank: elem?.rank, rewardPercentage: elem?.rewardPercentage };
      })
    );
    const res = await fetch(
      `${baseUrl}api/v1/battletemplates/${template?._id}`,
  ...
  let totalCollection = template
    ? template?.entryFee * template?.minParticipants
    : formState?.expectedCollection
    ? formState?.expectedCollection
    : 0;
  let gst = template
    ? (template?.entryFee *
      ...
      100
    : (formState.expectedCollection * formState?.gstPercentage) / 100;
  let collectionAfterTax = totalCollection - gst;
  let platfromFee = template
    ? (collectionAfterTax * template?.platformCommissionPercentage) / 100
    : (collectionAfterTax * formState?.platformCommissionPercentage) / 100;
  let prizePool =
    template?.entryFee != 0
      ? collectionAfterTax - platfromFee
      : template?.freePrizePool;
  let totalNumberOfWinners = template
    ? ((template?.minParticipants * template?.winnerPercentage) / 100).toFixed(
        0
      )
    : (
        (formState?.minParticipants * formState?.winnerPercentage) /
        100
      ).toFixed(0);
            ...
            <Grid container spacing={2} mt={0.5} mb={0} xs={12} md={9} xl={12}>
              ...
              <Grid item xs={12} md={6} xl={3} mb={1}>
                <TextField
                  ...
                  defaultValue={
                    editing ? formState?.entryFee : template?.entryFee
                  }
                  // onChange={handleChange}
                  // onChange={(e) => {
                  //   setFormState(prevState => ({
...

              {
                <Grid item xs={12} md={6} xl={3} mb={1}>
                  <TextField
                    ...
                    value={
                      template && isSubmitted
                        ? template?.freeWinnerCount
                        : formState?.freeWinnerCount
                    }
                    defaultValue={
                      editing
                        ? formState?.freeWinnerCount
              ...
              <Grid item xs={12} md={6} xl={3} mb={1}>
                <TextField
                  ...
                  defaultValue={
                    editing ? formState?.gstPercentage : template?.gstPercentage
                  }
                  // onChange={handleChange}
                  onChange={(e) => {
          ...
          <Grid container mt={2} xs={12} md={12} xl={12}>
            ...
            <Grid item xs={12} md={6} xl={3} mb={1}>
              <MDTypography fontSize={15}>
                Total Collection(Exp.): ₹{totalCollection}
              </MDTypography>
            </Grid>
            ...
            <Grid item xs={12} md={6} xl={3} mb={1}>
              <MDTypography fontSize={15}>
                Collection After Tax(Exp.): ₹{collectionAfterTax}
              </MDTypography>
            </Grid>

            <Grid item xs={12} md={6} xl={3} mb={1}>
              <MDTypography fontSize={15}>
                Platform Fee(Exp.): ₹{platfromFee}
              </MDTypography>
            </Grid>
            ...
            <Grid item xs={12} md={6} xl={3} mb={1}>
              <MDTypography fontSize={15}>
                Prize Pool(Exp.): ₹
                {template?.entryFee != 0 ? prizePool : template?.freePrizePool}
              </MDTypography>
            </Grid>
            ...
            <Grid item xs={12} md={6} xl={3} mb={1}>
              <MDTypography fontSize={15}>
                Top # of Winners:{" "}
                {template?.entryFee != 0
                  ? template?.rankingPayout.length
                  : template?.freeWinnerCount}
              </MDTypography>
            </Grid>

            {template?.rankingPayout?.length > 0 && (
              <Grid item xs={12} md={6} xl={3} mb={1}>
                <MDTypography fontSize={15}>
                  Total # of Winners:{" "}
                  {template?.entryFee != 0
            ...
            {template?.rankingPayout?.length > 0 && template?.entryFee != 0 && (
              <Grid item xs={12} md={6} xl={3} mb={1}>
                <MDTypography fontSize={15}>
                  Reward(Remaining Winners): ₹
                  {((100 -
                    template?.rankingPayout.reduce((total, currentItem) => {
              ...
              <Grid item xs={12} md={6} xl={3} mb={1}>
                <MDTypography fontSize={15}>
                  ...
                  {((100 -
                    ...
                    prizePool) /
                    100 /
                    (totalNumberOfWinners - template?.rankingPayout.length)}
                </MDTypography>
              </Grid>
            )}
          </Grid>
              ...
              {!isSubmitted && !Boolean(template) && (
                <>
                  ...
                  <MDButton
                    ...
                    disabled={creating}
                    onClick={() => {
                      navigate("/battledashboard/battletemplate");
                    }}
                  >
                    Cancel
                  </MDButton>
                ...
                {rankingPayoutMode == "create" ? (
                  ...
                  <Grid item xs={12} md={5} xl={5}>
                    ...
                    <MDButton
                      ...
                      size="small"
                      onClick={(e) => {
                        editRankingPayout(e, childFormState, setChildFormState);
                      }}
                    >
                      Edit Reward Payout
                    </MDButton>
                ...
                {rankingPayoutMode == "edit" && (
                  <Grid item xl={1} ml={-35}>
                    ...
                    <MDButton
                      ...
                      size="small"
                      onClick={(e) => {
                        setChildFormState({ rank: "", rewardPercentage: "" });
                        setRankingPayoutMode("create");
                      }}
                    >
                      Cancel
            ...
            <Grid item xs={12} md={12} xl={12} mt={2} mb={2}>
              <MDBox>
                <RankingPayout
                  ...
                  setChildFormState={setChildFormState}
                  rankingPayoutMode={rankingPayoutMode}
                  setRankingPayoutMode={setRankingPayoutMode}
                  rankingPayout={rankingPayout}
                  setRankingPayout={setRankingPayout}
                  ranking
                />
              </MDBox>
            </Grid>
