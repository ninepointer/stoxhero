...
export default function GroupDiscussions({
  ...
  let gst =
    (template?.entryFee * template?.minParticipants * template?.gstPercentage) /
    100;
  ...
  let totalNumberOfWinners = (
    (template?.minParticipants * template?.winnerPercentage) /
    100
  ).toFixed(0);
    ...
    featureObj.amount = (
      ...
      >
        ...
        {new Intl.NumberFormat(undefined, {
          ...
          elem?.entryFee != 0
            ? (elem?.rewardPercentage * prizePool) / 100
            : (elem?.rewardPercentage * elem?.template?.freePrizePool) / 100
        )}
      </MDTypography>
    );
  ...
  let remainingWinners =
    template?.entryFee != 0
      ? (template?.minParticipants * template?.winnerPercentage) / 100 -
        template?.rankingPayout?.length
      : template?.freeWinnerCount - template?.rankingPayout?.length;
  let prizePoolToppersPercentage = template?.rankingPayout.reduce(
    (total, currentItem) => {
  ...
  let remainingPrizePool =
    (expectedPrizePool * remainingPrizePoolPercentage) / 100;
  ...
  remainingObj.rank = (
    <MDTypography
      ...
      fontWeight="medium"
    >
      {template?.rankingPayout?.length + 1 !== template?.freeWinnerCount
        ? template?.entryFee != 0
          ? template?.rankingPayout?.length +
            1 +
            "-" +
  ...
  remainingObj.amount = (
    ...
    >
      ...
      {new Intl.NumberFormat(undefined, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }).format(remainingPrizePool / remainingWinners)}
    </MDTypography>
  );
