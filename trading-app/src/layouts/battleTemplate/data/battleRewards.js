...
export default function ChallengeParameters({
  saving,
  template,
  action,
  setAction,
  ...
  let remainingWinners =
    (template?.minParticipants?.length * template?.winnerPercentage) / 100 -
  ...
  let remainingPrizePool =
    (actualPrizePool * remainingPrizePoolPercentage) / 100;
  ...
  remainingObj.rank = (
    <MDTypography
      ...
      fontWeight="medium"
    >
      {battle?.battleTemplate?.rankingPayout?.length +
        1 +
        "-" +
        (
          (battle?.battleTemplate?.minParticipants *
            battle?.battleTemplate?.winnerPercentage) /
          100
        ).toFixed(0)}
    </MDTypography>
  );
  remainingObj.rewardpercentage = (
    ...
    >
      {(
        (100 -
          battle?.battleTemplate?.rankingPayout.reduce((total, currentItem) => {
            return total + currentItem.rewardPercentage;
          }, 0)) /
        remainingWinners
      ).toFixed(2)}
