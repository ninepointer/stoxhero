...
function LiveTraderwiseCompantPNL(props) {
  ...
  useEffect(() => {
    axios
      .get(
        `${baseUrl}api/v1/gettraderwisepnllivetradecompanytoday/algowisedata/${props.algoId}`,
  ...
  useEffect(() => {
    ...
    axios
      .get(`${baseUrl}api/v1/getlastestlivetradecompany`, {
        withCredentials: true,
      })
  ...
  finalTraderPnl.map((subelem, index) => {
    ...
    obj.orders = <LiveTraderwiseOrders userId={subelem.userId} />;
    obj.realOrMock = <MockRealSwitch props={props} userId={subelem.userId} />;
        ...
        <MDBox>
          ...
          <MDBox display="flex" alignItems="center" lineHeight={0}>
            ...
            <MDTypography variant="button" fontWeight="regular" color="text">
              &nbsp;<strong>last trade</strong> {lastestLiveTradeBy}{" "}
              {lastestLiveTradeType === "BUY" ? "bought" : "sold"}{" "}
              {Math.abs(lastestLiveTradeQunaity)} quantity of{" "}
              {lastestLiveTradeSymbol} at {lastestLiveTradeTime} -{" "}
              {lastestLiveTradeStatus}
            </MDTypography>
          </MDBox>
        </MDBox>
