...
function MockOverallCompantPNL({ socket, algoId, algoName }) {
  ...
  useEffect(() => {
    axios
      .get(
        `${baseUrl}api/v1/getoverallpnlmocktradecompanytoday/algowisedata/${algoId}`
      )
      .then((res) => {
        ...
        res.data.map((elem) => {
          marketData.map((subElem) => {
            if (
              subElem !== undefined &&
              subElem.instrument_token == elem._id.instrumentToken
            ) {
    ...
    axios
      .get(`${baseUrl}api/v1/getlastestmocktradecompany`)
      .then((res) => {
  ...
  tradeData.map((subelem, index) => {
    ...
    let updatedValue =
      subelem.amount + subelem.lots * liveDetail[index]?.last_price;
    ...
    if (liveDetail[index]?.change) {
      ...
      obj.change = (
        ...
        >
          {(
            ((liveDetail[index]?.last_price -
              liveDetail[index]?.average_price) /
              liveDetail[index]?.average_price) *
            100
          ).toFixed(2) + "%"}
        </MDTypography>
        ...
        <MDBox>
          ...
          <MDBox display="flex" alignItems="center" lineHeight={0}>
            ...
            <MDTypography variant="button" fontWeight="regular" color="text">
              &nbsp;<strong>last trade</strong> {lastestTradeBy}{" "}
              {lastestTradeType === "BUY" ? "bought" : "sold"}{" "}
              {Math.abs(lastestTradeQunaity)} quantity of {lastestTradeSymbol}{" "}
              at {lastestTradeTime} - {lastestTradeStatus}
            </MDTypography>
          </MDBox>
        </MDBox>
