...
const InstrumentMappingModel = ({ Render }) => {
  ...
  async function formSubmit(e) {
    ...
    const res = await fetch(`${baseUrl}api/v1/instrumentAlgo`, {
      ...
      body: JSON.stringify({
        InstrumentNameIncoming: incoming_instrument,
        IncomingInstrumentCode: incoming_instrument_code,
        InstrumentNameOutgoing: outgoing_instrument,
        OutgoingInstrumentCode: outgoing_instrument_code,
        Status: status,
        lastModified,
        uId,
        createdBy,
      ...
      >
        ...
        <DialogContent>
          <DialogContentText sx={{ display: "flex", flexDirection: "column" }}>
            <TextField
              id="outlined-basic"
              label="Instrument Name (Incoming)"
              variant="standard"
              sx={{ margin: 1, padding: 1, width: "300px" }}
              onChange={(e) => {
                formData.incoming_instrument = e.target.value;
              }}
            />
            ...
            <TextField
              id="outlined-basic"
              label="Instrument Name (Outgoing)e"
              variant="standard"
              sx={{ margin: 1, padding: 1, width: "300px" }}
              onChange={(e) => {
                formData.outgoing_instrument = e.target.value;
              }}
            />
