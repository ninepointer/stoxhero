...
export default function AllowedUsers({ dailyContest }) {
  ...
  useEffect(() => {
    let call1 = axios.get(
      `${baseUrl}api/v1/dailycontest/trade/${dailyContest?._id}/leaderboard`,
      {
  ...
  leaderBoard?.map((elem, index) => {
    ...
    featureObj.signupMethod = (
      <MDTypography
        ...
        fontWeight="medium"
      >
        {elem?.creationProcess !== "Auto SignUp"
          ? elem?.creationProcess
          : elem?.creationProcess === "Auto SignUp" &&
            (elem?.referredBy || elem?.referredBy === " ")
          ? "Referral SignUp"
          : "Organic SignUp"}
      </MDTypography>
    );
    ...
    featureObj.grosspnl = (
      ...
      >
        ...
        }).format(elem?.grosspnl >= 0 ? elem?.grosspnl : -elem?.grosspnl)}
      </MDTypography>
    );
    ...
    featureObj.netpnl = (
      ...
      >
        ...
        {new Intl.NumberFormat(undefined, {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }).format(elem?.netpnl >= 0 ? elem?.netpnl : -elem?.netpnl)}
      </MDTypography>
    );
    ...
    featureObj.payout = (
      ...
      >
        ...
        {new Intl.NumberFormat(undefined, {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }).format(elem?.payout >= 0 ? elem?.payout : -elem?.payout)}
      </MDTypography>
    );
            ...
            <Tooltip title="Download CSV">
              <MDBox
                ...
                onClick={() => {
                  handleDownload(
                    pnlData,
                    `Leaderboard-${dailyContest?.contestName}`
                  );
                }}
              >
                <DownloadIcon />
