...
let baseUrl = process.env.NODE_ENV === "production" ? "/" : "http://localhost:5001/"
  ...
  useEffect(()=>{
    let call1 = axios.get(`${baseUrl}api/v1/dailycontest/contests/collegeupcoming?skip=${skip}&limit=${limitSetting}`,{
  ...
  function backHandler(){
    ...
    setUpcomingContest([]);
    setIsLoading(true)
    axios.get(`${baseUrl}api/v1/dailycontest/contests/collegeupcoming?skip=${skip-limitSetting}&limit=${limitSetting}`,{
        withCredentials: true,
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
      ...
      {upcomingContest.length > 0 ?
         ...
         <Grid container spacing={1} xs={12} md={6} lg={12}>
          <Grid item mt={2} xs={6} md={3} lg={12} display="flex" justifyContent="center">
            <MDTypography color="light">No Upcoming College TestZone(s)</MDTypography>
          </Grid>
         </Grid>
         } 
