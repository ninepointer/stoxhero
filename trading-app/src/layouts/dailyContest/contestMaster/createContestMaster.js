...
function Index() {
  ...
  async function onSubmit(e, formState) {
    ...
    const res = await fetch(`${baseUrl}api/v1/contestmaster`, {
      ...
      body: JSON.stringify({
        contestMaster: value?._id,
        contestMasterMobile: value?.mobile,
        stoxheroPOC: pocValue?._id,
        college,
        status,
        inviteCode,
      }),
  ...
  async function onEdit(e, formState) {
    ...
    if (
      !formState.contestMaster ||
      !formState.contestMasterMobile ||
      !formState.stoxheroPOC ||
      !formState.college ||
      !formState.status
    ) {
      setTimeout(() => {
    ...
    const { contestMaster, contestMasterMobile, stoxheroPOC, college, status } =
      formState;
            ...
            >
              ...
              <Grid item xs={12} md={6} xl={3} mb={2} lg={12}>
                <TextField
                  ...
                  type="number"
                  defaultValue={
                    editing ? formState?.inviteCode : contest?.inviteCode
                  }
