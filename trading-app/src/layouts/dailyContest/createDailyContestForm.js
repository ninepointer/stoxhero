...
function Index() {
    ...
    async function onEdit(e,formState){
        ...
        const res = await fetch(`${baseUrl}api/v1/dailycontest/contest/${id._id}`, {
            method: "PATCH",
            credentials:"include",
            headers: {
                "content-type" : "application/json",
    ...
    {isLoading ? (
      ...
      ( 
        ...
        <Grid container spacing={2} mt={0.5} mb={0} xs={12} md={9} xl={12}>
          ...
          {/* <Grid item xs={12} md={6} xl={3}>
              <FormControl sx={{width: "100%" }}>
                <InputLabel id="demo-simple-select-autowidth-label">Contest Type *</InputLabel>
                <Select
                labelId="demo-simple-select-autowidth-label"
                id="demo-simple-select-autowidth"
                name='contestType'
                value={formState?.contestType || id?.contestType}
                disabled={((isSubmitted || id) && (!editing || saving))}
                onChange={(e) => {setFormState(prevState => ({
                    ...prevState,
