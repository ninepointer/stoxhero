...
function TradersHeatMapWeekly() {
  ...
  let valueInDate1 = `${date.getFullYear()}-${String(
    date.getMonth() + 1
  ).padStart(2, "0")}-01`;
  ...
  function pnlCalculation(firstWeek, secondWeek) {
    ...
    axios
      .get(
        `${baseUrl}api/v1/getweeklytraderpnlLive/${firstWeek}/${secondWeek}`,
    ...
    axios
      .get(`${baseUrl}api/v1/getuniqueweeksLive/${firstWeek}/${secondWeek}`, {
        withCredentials: true,
      })
      .then((res) => {
        let data = res.data;
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      data[i][j] = overallPnl
        .filter((e) => {
          return (
            pnlweeks1[j] == e._id.weekNumber &&
            traderNameList[i] == e._id.trader
          );
        })
      ...
      <MDBox mt={2} mb={3} fontSize={13}>
        ...
        >
          <MDBox>
            ...
            >
              Trader Side HeatMap (Mock-Gross P&L)
            </Typography>
          </MDBox>
          <MDBox>
            <Typography
          ...
          <MDBox>
            <MDButton
              variant="contained"
              color="info"
              sx={{ margin: 1, marginLeft: 10, padding: 1 }}
              onClick=""
            >
              Download PDF
            </MDButton>
        ...
        <Typography sx={{ mt: -2 }} display="flex">
          ...
          <Typography sx={{ mt: -1 }}>{dateupdatenow}</Typography>
        </Typography>
      </MDBox>
    </>
  );
