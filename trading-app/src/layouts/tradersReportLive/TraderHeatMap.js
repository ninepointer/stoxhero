...
function TradersHeatMap() {
  ...
  function pnlCalculation(firstDate, secondDate) {
    ...
    axios
      .get(`${baseUrl}api/v1/getuserreportLive/${firstDate}/${secondDate}`, {
    ...
    axios
      .get(`${baseUrl}api/v1/getuniquedatesLive/${firstDate}/${secondDate}`, {
        withCredentials: true,
      })
      .then((res) => {
        let data = res.data;
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      data[i][j] = overallPnl
        .filter((e) => {
          return (
            pnldates1[j] == e._id.date && traderNameList[i] == e._id.trader
          );
        })
