...
export default function TenXSubscribers({ tenXSubscription }) {
  ...
  let columns = [
    ...
    { Header: "Expired On", accessor: "expiringon", align: "center" },
  ...
  tenXSubscription.forEach((item) => {
    if (typeof item.payout === "undefined") {
      item.payout = 0;
    }
  });
  let tenXSubscribers = tenXSubscription?.sort((a, b) => b.payout - a.payout);
  tenXSubscribers?.map((elem, index) => {
    let featureObj = {};
    featureObj.index = (
      <MDTypography
        component="a"
    ...
    featureObj.payout = (
      ...
      >
        ...
        }).format(elem?.payout ? elem?.payout : 0)}
      </MDTypography>
    );
      ...
      <MDBox display="flex" justifyContent="space-between" alignItems="left">
        ...
        >
          <MDTypography
            ...
            gutterBottom
          >
            Users with Expired {tenXSubscription?.plan_name} Subscriptions(
            {tenXSubscription?.length})
          </MDTypography>
        </MDBox>
      </MDBox>
      <MDBox mt={1}>
