...
export default function SubscriptionPurchaseIntent({
  tenXSubscription,
  purchaseIntentCount,
  setPurchaseIntentCount,
}) {
  ...
  async function getSubscriptionPurchaseIntent() {
    let call1 = axios.get(
      `${baseUrl}api/v1/tenx/subscriptionpurchaseintent/${tenXSubscription}`,
      {
  ...
  tenXSubsPurchaseIntent?.map((elem, index) => {
    ...
    featureObj.fullname = (
      <MDTypography
        ...
        fontWeight="medium"
      >
        {elem?.purchase_intent_by?.first_name}{" "}
        {elem?.purchase_intent_by?.last_name}
      </MDTypography>
    );
    featureObj.email = (
      <MDTypography
    ...
    featureObj.clickedon = (
      <MDTypography
        ...
        fontWeight="medium"
      >
        {new Date(elem?.clicked_On).toLocaleDateString("en-IN", {
          timeZone: "Asia/Kolkata",
        })}{" "}
        {new Date(elem?.clicked_On)
      ...
      <MDBox display="flex" justifyContent="space-between" alignItems="left">
        ...
        >
          <MDTypography
            ...
            gutterBottom
          >
            Users who clicked on Purchase button under Subscription(
            {purchaseIntentCount})
          </MDTypography>
        </MDBox>
      </MDBox>
