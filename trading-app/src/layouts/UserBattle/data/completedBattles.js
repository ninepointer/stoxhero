...
import CompletedBattleList from '../Header/completedBattleList'
...
export default function LabTabs({setClicked}) {
    ...
    useEffect(() => {
        setIsLoading(true)
        axios.get(`${baseUrl}api/v1/battles/usercompleted`, {
               ...
               {battle.length != 0 ? 
                <Grid container xs={12} md={12} lg={12} display='flex'>
                    ...
                    <Grid item xs={12} md={6} lg={12} mt={.5}>
                        <CompletedBattleList socket={socket} battle={battle} isInterested={isInterested} setIsInterested={setIsInterested} showPay={showPay} setShowPay={setShowPay}/>
                    </Grid>
                ...
                <MDBox style={{minHeight:"20vh"}} border='1px solid white' borderRadius={5} display="flex" justifyContent="center" flexDirection="column" alignContent="center" alignItems="center">
                    ...
                    <MDTypography color="light" fontSize={15} mb={1}>No Completed Battle(s)</MDTypography>
                    <MDButton color="info" size='small' fontSize={10}  onClick={()=>{setClicked("upcoming")}}>Check Upcoming Battle(s)</MDButton>
                </MDBox>
                }
            </>
            }
