...
const PnlAndMarginData = ({ battleId }) => {
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/battletrade/${battleId}/myPnlandCreditData`, {
    ...
    <Grid container spacing={1} xs={12} md={12} lg={12}>
        ...
        <MDButton style={{ minWidth: "100%" }}>
          <MDBox display="flex" alignItems="center">
            <MDBox display="flex" justifyContent="flex-start">
              <img src={AMargin} width="40px" height="40px" />
            </MDBox>
            <MDBox>
              <MDTypography ml={1} fontSize={11} fontWeight="bold">
                Portfolio:
              </MDTypography>
            </MDBox>
            <MDBox>
              <MDTypography ml={1} fontSize={11}>
                {totalCreditString}
              </MDTypography>
            </MDBox>
          </MDBox>
      ...
      <Grid item xs={12} md={6} lg={3}>
        <MDButton style={{ minWidth: "100%" }}>
          <MDBox display="flex" alignItems="center">
            <MDBox display="flex" justifyContent="flex-start">
              <img src={Tcost} width="40px" height="40px" />
            </MDBox>
            <MDBox>
              <MDTypography ml={1} fontSize={11} fontWeight="bold">
                Margin/Used Margin:
            ...
            <MDBox>
              <MDTypography
                ml={1}
                fontSize={11}
              >{`${availableMarginpnlstring}/${usedMarginString}`}</MDTypography>
            </MDBox>
          </MDBox>
        </MDButton>
      </Grid>
      <Grid item xs={12} md={6} lg={3}>
        <MDButton style={{ minWidth: "100%" }}>
          <MDBox display="flex" alignItems="center">
            <MDBox display="flex" justifyContent="flex-start">
              <img src={Profit} width="40px" height="40px" />
            </MDBox>
            <MDBox>
              <MDTypography ml={1} fontSize={11} fontWeight="bold">
                Gross Profit:
            ...
            <MDBox>
              <MDTypography ml={1} fontSize={11}>
                {" "}
                {pnl?.grossPnlAndBrokerage?.grossPnl >= 0
                  ? "+₹" +
                    new Intl.NumberFormat(undefined, {
                      minimumFractionDigits: 0,
                      maximumFractionDigits: 0,
                    }).format(pnl?.grossPnlAndBrokerage?.grossPnl)
                  : "-₹" +
                    new Intl.NumberFormat(undefined, {
                      minimumFractionDigits: 0,
                      maximumFractionDigits: 0,
