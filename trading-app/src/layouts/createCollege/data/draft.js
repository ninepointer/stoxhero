...
const Draft = () => {
  ...
  useEffect(() => {
    let call1 = axios.get(`${apiUrl}fullcollege/draft?skip=${skip}&limit=${limitSetting}`, {
    ...
    Promise.all([call1])
      .then(([api1Response]) => {
        ...
        setDraft(api1Response.data.data)
  ...
  function backHandler(){
    ...
    setDraft([]);
    setIsLoading(true)
    axios.get(`${apiUrl}fullcollege/draft?skip=${skip-limitSetting}&limit=${limitSetting}`,{
        withCredentials: true,
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
    ...
    {draft.length > 0 ?
       ...
       <Grid container spacing={1} xs={12} md={6} lg={12}>
        <Grid item mt={2} xs={6} md={3} lg={12} display="flex" justifyContent="center">
          <MDTypography color="light">No Draft College(s)</MDTypography>
        </Grid>
       </Grid>
       } 
