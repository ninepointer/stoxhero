...
const CompletedContest = () => {
  ...
  useEffect(() => {
    let call1 = axios.get(`${apiUrl}fullcollege/active?skip=${skip}&limit=${limitSetting}`, {
  ...
  function backHandler(){
    ...
    setActive([]);
    setIsLoading(true)
    axios.get(`${apiUrl}fullcollege/active?skip=${skip-limitSetting}&limit=${limitSetting}`,{
        withCredentials: true,
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
    ...
    {active.length > 0 ?
       ...
       <Grid container spacing={1} xs={12} md={6} lg={12}>
        <Grid item mt={2} xs={6} md={3} lg={12} display="flex" justifyContent="center">
          <MDTypography color="light">No Active College(s)</MDTypography>
        </Grid>
       </Grid>
       } 
