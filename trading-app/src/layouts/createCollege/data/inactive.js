...
const Inactive = () => {
  ...
  useEffect(() => {
    let call1 = axios.get(`${apiUrl}fullcollege/inactive?skip=${skip}&limit=${limitSetting}`, {
    ...
    Promise.all([call1])
      .then(([api1Response]) => {
        ...
        setInactive(api1Response.data.data)
  ...
  function backHandler(){
    ...
    setInactive([]);
    setIsLoading(true)
    axios.get(`${apiUrl}fullcollege/inactive?skip=${skip-limitSetting}&limit=${limitSetting}`,{
        withCredentials: true,
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
    ...
    {inactive.length > 0 ?
                          ...
                          <Grid container>
                            ...
                            <Grid item xs={12} md={6} lg={12} display={"flex"} justifyContent={"center"} alignItems={"center"}>
                              <Grid item xs={12} md={6} lg={2} mb={1} display="flex" justifyContent="center">
                                <MDTypography fontSize={9} style={{ color: "black" }}>Route: <span style={{ fontSize: 9, fontWeight: 700 }}>{e?.route}</span></MDTypography>
                              </Grid>

                              <Grid item xs={12} md={6} lg={2} mb={1} display="flex" justifyContent="center">
                                <MDTypography fontSize={9} style={{ color: "black" }}>event: <span style={{ fontSize: 9, fontWeight: 700 }}>{e?.event}</span></MDTypography>
                              </Grid>
                              ...
                              <Grid item xs={12} md={6} lg={2} mb={1} display="flex" justifyContent="center">
                                <MDTypography fontSize={9} style={{ color: "black" }}># Admins: <span style={{ fontSize: 9, fontWeight: 700 }}>{e?.admin?.length}</span></MDTypography>
                              </Grid>

                              <Grid item xs={12} md={6} lg={2} mb={1} display="flex" justifyContent="center">
                                <MDTypography fontSize={9} style={{ color: "black" }}># Students: <span style={{ fontSize: 9, fontWeight: 700 }}>{e?.student?.length}</span></MDTypography>
                              </Grid>
       ...
       <Grid container spacing={1} xs={12} md={6} lg={12}>
        <Grid item mt={2} xs={6} md={3} lg={12} display="flex" justifyContent="center">
          <MDTypography color="light">No Inactive College(s)</MDTypography>
        </Grid>
       </Grid>
       } 
