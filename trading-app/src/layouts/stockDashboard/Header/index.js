...
export default function LabTabs({ socket }) {
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}api/v1/getstockliveprice`)
      .then((res) => {
  ...
  useEffect(() => {
    setIsLoading(true);
    axios
      .get(`${baseUrl}api/v1/stock/stockoveralltraderpnltoday`, {
    ...
    axios
      .get(`${baseUrl}api/v1/stock/liveandtotaltradercounttoday`, {
        withCredentials: true,
      })
      .then((res) => {
    ...
    axios
      .get(`${baseUrl}api/v1/stock/dailystockusers`, { withCredentials: true })
      .then((res) => {
    ...
    axios
      .get(`${baseUrl}api/v1/stock/stockoveralltraderpnlyesterday`, {
        withCredentials: true,
      })
      .then((res) => {
    ...
    axios
      .get(`${baseUrl}api/v1/stock/liveandtotaltradercountyesterday`, {
        withCredentials: true,
      })
      .then((res) => {
  ...
  const totalnetPnlcolorCNC =
    totalGrossPnlCNC - totalTransactionCostCNC >= 0 ? "#6FBF72" : "#F44335";

  const totalGrossPnlcolorMIS = totalGrossPnlMIS >= 0 ? "success" : "error";
  const totalnetPnlcolorMIS =
    totalGrossPnlMIS - totalTransactionCostMIS >= 0 ? "success" : "error";
              ...
              <Grid container>
                <Grid item p={2} xs={12} lg={5.9}>
                  <MDTypography fontSize={16} fontWeight="bold" color="dark">
                    Today's Stock Trading Position (Trader Side)
                  </MDTypography>
                  <Grid container mt={1}>
                    <Grid item lg={4}>
                      <MDTypography
                      ...
                      >
                        ...
                        /
                        <span style={{ color: totalGrossPnlcolorCNC }}>
                          {totalGrossPnlCNC >= 0
                            ? "+₹" +
                              new Intl.NumberFormat(undefined, {
                                minimumFractionDigits: 0,
                                maximumFractionDigits: 0,
                              }).format(totalGrossPnlCNC)
                            : "-₹" +
                              new Intl.NumberFormat(undefined, {
                                minimumFractionDigits: 0,
                                maximumFractionDigits: 0,
                    ...
                    <Grid item lg={4}>
                      ...
                      >
                        ...
                        {new Intl.NumberFormat(undefined, {
                          minimumFractionDigits: 0,
                          maximumFractionDigits: 0,
                        }).format(totalTransactionCostMIS)}{" "}
                        /
                        <span>
                          ₹
                          {new Intl.NumberFormat(undefined, {
                            minimumFractionDigits: 0,
                            maximumFractionDigits: 0,
                          }).format(totalTransactionCostCNC)}
                        </span>{" "}
                      </MDTypography>
                    </Grid>
                    <Grid item lg={4}>
                      ...
                      <MDTypography
                        color={totalnetPnlcolorMIS}
                        fontSize={12}
                        display="flex"
                        justifyContent="right"
                      >
                        {totalGrossPnlMIS - totalTransactionCostMIS >= 0
                          ? "+₹" +
                            new Intl.NumberFormat(undefined, {
                              minimumFractionDigits: 0,
                              maximumFractionDigits: 0,
                            }).format(
                              totalGrossPnlMIS - totalTransactionCostMIS
                            )
                          : "-₹" +
                            new Intl.NumberFormat(undefined, {
                              minimumFractionDigits: 0,
                              ...
                              totalTransactionCostMIS - totalGrossPnlMIS
                            )}
                        /{" "}
                        <span style={{ color: totalnetPnlcolorCNC }}>
                          {totalGrossPnlCNC - totalTransactionCostCNC >= 0
                            ? "+₹" +
                              new Intl.NumberFormat(undefined, {
                                minimumFractionDigits: 0,
                                maximumFractionDigits: 0,
                              }).format(
                                totalGrossPnlCNC - totalTransactionCostCNC
                              )
                            : "-₹" +
                              new Intl.NumberFormat(undefined, {
                                minimumFractionDigits: 0,
                                ...
                                totalTransactionCostCNC - totalGrossPnlCNC
                              )}
                        </span>
                      </MDTypography>
                    </Grid>
                  ...
                  <Grid container mt={1}>
                    ...
                    <Grid item lg={6}>
                      <MDTypography
                        ...
                        justifyContent="center"
                      >
                        Running Lots(MIS/CNC)
                      </MDTypography>
                      <MDTypography
                        color={totalquantitycolorMIS}
                        fontSize={12}
                        display="flex"
                        justifyContent="center"
                      >
                        {totalRunningLotsMIS}/{" "}
                        <span style={{ color: totalquantitycolor }}>
                          {totalRunningLotsCNC}
                        </span>{" "}
                      </MDTypography>
                ...
                <Grid item p={2} xs={12} lg={5.9}>
                  <MDTypography fontSize={16} fontWeight="bold" color="dark">
                    Last Stock Trading Day (Trader Side) -{" "}
                    {new Date(lastTradingDate).toLocaleDateString("en-US", {
                      day: "numeric",
                      month: "short",
                      year: "numeric",
                      weekday: "short",
                  ...
                  <Grid container mt={1}>
                    ...
                    <Grid item lg={6}>
                      ...
                      <MDTypography
                        ...
                        justifyContent="center"
                      >
                        {tradeDataYesterday?.[0]?.lotsMIS || 0}/{" "}
                        <span>{tradeDataYesterday?.[0]?.lotsCNC || 0}</span>
                      </MDTypography>
                    </Grid>
                    <Grid item lg={3}>
                      <MDTypography
            ...
            <Grid container>
              ...
              >
                <MDBox display="flex" flexDirection="column">
                  ...
                  >
                    Check all trader's stock position here!
                  </MDTypography>
                </MDBox>
              </Grid>
            </Grid>
              ...
              >
                <MDBox display="flex" flexDirection="column">
                  <MDTypography
                    ...
                    style={{ color: "white", paddingLeft: 4, paddingRight: 4 }}
                  >
                    Check all stock trader's orders here!
                  </MDTypography>
                </MDBox>
              </Grid>
            </Grid>
