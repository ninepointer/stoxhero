...
function Index() {
  ...
  const [formState, setFormState] = useState({
    ...
    moduleColor: "" || id?.moduleColor,
    moduleStatus: "" || id?.moduleStatus,
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${baseUrl}api/v1/learningmodule/${id ? id?._id : newObjectId?._id}`,
      ...
      .then((res) => {
        ...
        setFormState({
          moduleNumber: res.data.data?.moduleNumber || "",
          moduleName: res.data.data?.moduleName || "",
          moduleColor: res.data.data?.moduleColor || "",
          moduleStatus: res.data?.moduleStatus || "",
          moduleDescription: res.data?.moduleDescription || "",
        });
        setTimeout(() => {
          setIsLoading(false);
        }, 500);
  ...
  async function onSubmit(e, data) {
    ...
    try {
      ...
      if (
        !formState.moduleNumber.toString() ||
        !formState.moduleName ||
        !formState.moduleColor ||
        !formState.moduleStatus ||
        !formState.moduleDescription
      ) {
        setCreating(false);
            ...
            <Grid container spacing={1} mt={0.5} mb={0} xs={12} md={9} xl={12}>
              ...
              <Grid item xs={12} md={6} lg={3}>
                <TextField
                  ...
                  label="Module Color *"
                  fullWidth
                  value={formState?.moduleColor}
                  onChange={(e) => {
                    setFormState((prevState) => ({
                      ...prevState,
                      moduleColor: e.target.value,
                    }));
                  }}
                />
              </Grid>
