...
function Index({ createIndexForm, setCreateIndexForm }) {
  ...
  const [selectedProduct, setSelectedProduct] = useState(
    id?.affiliateDetails?.eligibleProducts
      ? id?.affiliateDetails?.eligibleProducts
      : []
  );
  const [selectedPlatform, setSelectedPlatform] = useState(
    id?.affiliateDetails?.eligiblePlatforms
      ? id?.affiliateDetails?.eligiblePlatforms
      : []
  );
  ...
  async function getProducts() {
    ...
    if (res.status == 200) {
      setProducts(
        res?.data?.data?.filter((item) => item?.productName != "Internship")
      );
    }
  }
  useEffect(() => {
  ...
  async function onEdit(e, formState) {
    ...
    if (
      !formState?.affiliateDetails?.minOrderValue ||
      !formState?.affiliateDetails?.maxDiscount ||
      !formState?.affiliateDetails?.discountPercentage ||
      !formState?.affiliateDetails?.commissionPercentage ||
      !formState?.referralProgramName ||
      !formState?.referralProgramStartDate ||
      !formState?.referralProgramEndDate ||
      !formState?.rewardPerReferral ||
      !formState?.currency ||
      !formState?.status
    ) {
      setTimeout(() => {
  ...
  async function onSubmit(e, formState) {
    ...
    if (response) {
      ...
      openSuccessSB(
        ...
        response.data.referralProgramName +
          "|" +
          response.data.referralProgramName
      );
                ...
                <Select
                  ...
                  renderValue={(selectedIds) =>
                    selectedIds
                      .map(
                        (id) =>
                          products.find((prod) => prod._id === id)?.productName
                      )
                      .join(", ")
                  }
                  sx={{ minHeight: "44px" }}
                ...
                >
                  {products?.map((elem) => (
                    <MenuItem key={elem?._id} value={elem?._id}>
                      <Checkbox
                        checked={selectedProduct.indexOf(elem?._id) > -1}
                      />
            ...
            <Grid item xs={12} md={6} xl={3}>
              <FormControl sx={{ width: "100%" }}>
                ...
                <Select
                  ...
                  input={<OutlinedInput label="Tag" />}
                  renderValue={
                    (selectedIds) => selectedIds.map((id) => id).join(", ")
                ...
                >
                  ...
                  <MenuItem key={2} value="Android">
                    <Checkbox
                      checked={selectedPlatform.indexOf("Android") > -1}
                    />
