...
export default function LabTabs() {
    ...
    if (data) {
      ...
      csvDataFile = csvpnlData?.map((elem) => {
        return [
          ...
          elem?.referredBy
            ? elem?.referredBy?.first_name + " " + elem?.referredBy?.last_name
  ...
  async function handleShowDetails() {
    try {
      const res = await axios.get(
        `${apiUrl}user/filteredusers?campaign=${selectedTab?.campaign?.id}&referral=${selectedTab?.referral?.id}&startDate=${startDate}&endDate=${endDate}&referredBy=${selectedUser?.id} `,
        { withCredentials: true }
      );
      if (res.status == 200) {
        setFilteredUsers(res.data.data);
        const startIndex = (currentPage - 1) * perPage;
        const slicedData = res.data.data.slice(
          startIndex,
          startIndex + perPage
        );
        setData(slicedData);
  ...
  const handleReferralChange = (e) => {
    ...
    const selectedReferral = referralProgramme.filter(
      (elem) => elem?._id == referralId
    );
                  ...
                  >
                    <Tooltip title="Download CSV">
                      <MDBox
                        ...
                        onClick={() => {
                          handleDownload(pnlData, `filteredusers`);
                        }}
                      >
                        <DownloadIcon />
                      </MDBox>
            ...
            return (
              <>
                ...
                >
                  ...
                  >
                    <MDTypography color="light" fontSize={13}>
                      {new Date(elem?.joining_date).toLocaleDateString(
                        "en-IN",
          ...
          {!isLoading && data?.length > 0 && (
            ...
            >
              ...
              <MDTypography color="light" fontSize={15} fontWeight="bold">
                Total Data: {filteredUsers.length} | Page {currentPage} of{" "}
              ...
              <MDButton
                ...
                color="warning"
                disabled={
                  Math.ceil(filteredUsers.length / perPage) === currentPage
                    ? true
                    : false
                }
                size="small"
