...
export default function UserTodayTradeData({ select }) {
  let baseUrl =
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${baseUrl}api/v1/stock/todaysorders?product=${select}&skip=${skip}&limit=${limitSetting}`,
  ...
  function backHandler() {
    ...
    axios
      .get(
        `${baseUrl}api/v1/stock/todaysorders?product=${select}&skip=${
          skip - limitSetting
        }&limit=${limitSetting}`,
        {
          withCredentials: true,
    ...
    >
      <Grid container spacing={1}>
        ...
        {!isLoading ? (
          data?.map((elem) => {
            const fullName =
              elem?.trader?.first_name + " " + elem?.trader?.last_name;
