...
export default function UserTodayTradeData({select}) {
  ...
  useEffect(() => {
    setIsLoading(true)
    axios.get(`${baseUrl}api/v1/stock/todaysorders?product=${select}&skip=${skip}&limit=${limitSetting}`, { withCredentials: true })
      .then((res) => {
  ...
  function backHandler() {
    ...
    setData([]);
    setIsLoading(true)
    axios.get(`${baseUrl}api/v1/stock/todaysorders?product=${select}&skip=${skip - limitSetting}&limit=${limitSetting}`, {
