...
import ExitPosition from '../../PaperTrade/OverallP&L/ExitPosition';
import Buy from "../../PaperTrade/components/InstrumentDetails/data/BuyModel"
import Sell from "../../PaperTrade/components/InstrumentDetails/data/SellModel"
import OverallRow from '../../PaperTrade/OverallP&L/OverallRow';
...
function Pnl() {
  ...
  let rows = [];
    ...
    tradeData.map((subelem, index)=>{
      ...
      obj.buy = (
        <Buy  symbol={subelem._id.symbol} exchange={subelem._id.exchange} instrumentToken={subelem._id.instrumentToken} symbolName={(subelem._id.symbol).slice(-7)} lotSize={lotSize} maxLot={lotSize*36} ltp={(liveDetail[0]?.last_price)?.toFixed(2)}/>
      );
      
      obj.sell = (
        <Sell  symbol={subelem._id.symbol} exchange={subelem._id.exchange} instrumentToken={subelem._id.instrumentToken} symbolName={(subelem._id.symbol).slice(-7)} lotSize={lotSize} maxLot={lotSize*36} ltp={(liveDetail[0]?.last_price)?.toFixed(2)}/>
      );
  ...
  return (
    <Card>
      <MDBox display="flex" justifyContent="space-between" alignItems="center" p={3} sx={{backgroundColor: "#CCCCCC", borderRadius: "10px", py: 1.5}}>
        <MDBox>
      ...
      {rows.length === 0 ? (
      <MDBox display="flex" flexDirection="column" mb={4} sx={{alignItems:"center", mt: "30px"}}>
      ...
      (<MDBox>
        <TableContainer component={Paper} style={{ my: 1}}>
          <table style={{ borderCollapse: "collapse", width: "100%", borderSpacing: "10px 5px", borderRadius: "0px"}}>
            <thead style={{backgroundColor: "#CCCCCC", borderRadius: "0px"}}>
