...
function DayWiseTraderPNL() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${baseUrl}api/v1/getDayWiseTradersTradeDetails/${valueInDate1}/${valueInDate2}`
      )
      .then((res) => {
  ...
  function pnlCalculation(startDay, endDay) {
    ...
    axios
      .get(
        `${baseUrl}api/v1/getDayWiseTradersTradeDetails/${Number(
          startDay
        )}/${Number(endDay)}`
      )
      .then((res) => {
  ...
  for (let i = 0; i < rows; i++) {
    ...
    for (let j = 0; j < cols; j++) {
      data[i][j] = tradersPNLData
        ?.filter((e) => {
          let filterData = e.pnl_by_day?.filter((k) => {
            return k.serial_number - 1 == j;
          });
          ...
          return (
            xlabelnumber[j] == filterData[0]?.serial_number &&
            traderNameList[i] == e.trader_name
          );
        })
        ?.map((e) => {
          let filterData = e.pnl_by_day?.filter((k) => {
      ...
      <MDBox mt={2} mb={3} fontSize={13}>
        ...
        >
          <MDBox>
            ...
            >
              Trader Side Day Wise (Mock-Net P&L)
            </Typography>
          </MDBox>
          <MDBox>
            <Typography
          ...
          <TextField
            ...
            variant="standard"
            type="number"
            sx={{ margin: 1.5, padding: 1, width: 50 }}
            onChange={(e) => {
              startDay1(e);
            }}
            value={startDay}
