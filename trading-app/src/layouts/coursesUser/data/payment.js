...
const Payment = ({
  ...
  const handleClose = () => {
    ...
    setCode("");
    setVerifiedCode("");
    if (setShowPay) messege.thanksMessege && setShowPay(!showPay);
  ...
  async function captureIntent() {
    ...
    const res = await fetch(
      `${apiUrl}courses/user/${data._id}/purchaseintent`,
      {
        method: "PUT",
        credentials: "include",
        headers: {
  ...
  const buySubscription = async () => {
    ...
    const res = await fetch(`${apiUrl}courses/user/deductcoursefee`, {
      ...
      body: JSON.stringify({
        courseFee:
          Number(amount - discountAmount - bonusRedemption) *
          (1 + setting?.courseGstPercentage / 100).toFixed(2),
  ...
  const bonusRedemption = checked
    ...
    : 0;
  const actualAmount =
    ((data?.discountedPrice - discountAmount - bonusRedemption) *
      setting.courseGstPercentage) /
    100;
      ...
      const res = await axios.post(
        `${apiUrl}payment/initiate`,
        {
          amount:
            ...
            actualAmount * 100,
          redirectTo: "https://stoxhero.com/courses",
          paymentFor: "Course",
          productId: data?._id,
          coupon: verifiedCode,
          bonusRedemption,
    ...
    try {
      ...
      const res = await axios.post(
        `${apiUrl}coupons/verify`,
        {
          ...
          orderValue: data?.discountedPrice,
          platform: "Web",
          paymentMode: value,
        },
        { withCredentials: true }
  ...
  const handleOpenNewTab = async (data) => {
    if (workshop) {
      ...
      if (new Date(serverTime) <= new Date(data?.courseStartTime)) {
        openSuccessSB(
          "Information",
          `Workshop will start on ${moment(data?.courseStartTime)?.format(
            "DD MMM hh:mm a"
          )}.`
        );
    ...
    const newTab = window.open(
      `/watchcourse?course=${data?.courseSlug}`,
      "_blank"
    );
  };
  ...
  return (
    <>
      {!checkPaid ? (
        ...
        >
          {workshop && new Date(serverTime) <= new Date(data?.courseEndTime)
            ? "Join"
            : "View"}
        </MDButton>
      )}
                          ...
                          {value == "wallet" && (
                            ...
                            >
                                ...
                                <MDBox
                                  ...
                                  onClick={() => {
                                    window.webengage.track(
                                      "course_intent_to_apply_couponcode_clicked",
                                      {
                                        user: getDetails?.userDetails?._id,
                                        courseId: data?._id,
                                        amount: Number(
                              ...
                              <Typography
                                ...
                                variant="body2"
                              >
                                GST({setting?.courseGstPercentage}%) on Fee: ₹
