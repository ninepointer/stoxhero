...
function Users({ setReRender, reRender, selectedUser, setSelectedUser }) {
  let baseUrl =
  ...
  function sendRequest(data) {
    ...
    axios
      .get(`${baseUrl}api/v1/user/searchuserinfluencer?search=${data}`, {
    ...
    >
      ...
      >
        <TextField
          ...
          variant="outlined"
          type="text"
          placeholder="Search users for influencer addition"
          value={state.text}
          inputRef={textRef}
          InputProps={{
            onFocus: () => textRef.current.select(),
            endAdornment: (
              <MDButton variant="text" onClick={handleClear}>
                {state.text && <RxCross2 color={"black"} />}
              </MDButton>
            ),
