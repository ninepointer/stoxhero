  ...
  tradeData.map((subelem, index) => {
    ...
    const liveDetail = marketData.filter((elem) => {
      console.log(
        ...
        subelem.exchangeInstrumentToken,
        subelem.exchangeInstrumentToken == elem.instrument_token
      );
      return (
        elem !== undefined &&
        (subelem.instrumentToken == elem.instrument_token ||
          subelem.exchangeInstrumentToken == elem.instrument_token)
      );
    });
    ...
    console.log(
      "liveDetail",
      liveDetail[0]?.instrument_token,
      subelem?.instrumentToken
    );
    let updatedValue =
    ...
    const instrumentcolor =
      subelem.symbol.slice(-2) == "CE" ? "success" : "error";
      ...
      {rows.length === 0 ? (
        ...
        >
          ...
          <Typography mb={2} fontSize={15} color="grey">
            Active real trades will show up here.
          </Typography>
        </MDBox>
      ) : (
        <MDBox>
