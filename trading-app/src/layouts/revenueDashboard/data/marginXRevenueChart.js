...
export default function Charts({ marginXMonthlyRevenue }) {
  ...
  useEffect(() => {
    ...
    const options = {
      ...
      xAxis: [
        {
          type: 'category',
          data: marginXMonthlyRevenue?.map((item) => item?.formattedDate),
          axisPointer: {
            type: 'shadow',
          },
          axisLabel: {
      ...
      series: [
        {
          ...
          data: marginXMonthlyRevenue?.map((item) => (item?.monthGMV).toFixed(0)),
        },
        {
          name: 'Revenue',
          type: 'bar',
          yAxisIndex: 0, // Associate with the first y-axis
          data: marginXMonthlyRevenue?.map((item) => (item?.monthRevenue)?.toFixed(0)),
        },
        {
          name: 'Orders',
          type: 'line',
          yAxisIndex: 1, // Associate with the second y-axis
          data: marginXMonthlyRevenue?.map((item) => item?.totalOrder),
        },
        {
          name: 'Unique Paid Users',
          type: 'line',
          yAxisIndex: 1, // Associate with the second y-axis
          data: marginXMonthlyRevenue?.map((item) => item?.uniqueUsersCount),
        },
      ],
    };
