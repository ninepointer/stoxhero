        ...
        .then(async (response) => {
          ...
          setTimeout(() => {
            if (!isMissed && !dontSendResp) {
              return res
                .status(201)
                .json({ message: responseMsg, err: responseErr });
            }
          }, 0);
        })
        .catch(async (err) => {
          ...
          console.log(
            err,
            "err in retreiving data in placeorder---------------"
          );
        });
    }, 500);
  }
    ...
    for (let i = 0; i < 10; i++) {
      setTimeout(async () => {
        ...
        if (!breakingLoop && i >= 5) {
          ...
          if (i === 5) {
            res.status(400).json({
              message: `your trade of ${realSymbol} and quantity ${realQuantity} was not placed`,
            });
          }
