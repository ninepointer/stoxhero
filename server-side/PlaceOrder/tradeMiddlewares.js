...
const {maxLot_BankNifty, maxLot_Nifty, maxLot_FinNifty} = require('../constant');
...
exports.tradeChecks = async(req,res,next) => {
    try{
        ...
        if(Quantity > maxLot){
            return res.status(401).send({ message: `You can place maximum ${maxLot} quantity in this trade.` });
        } else{
            next();
        }
    }catch(e){
