...
router.get("/xtsMargin", async (req, res)=>{
  ...
  try{
    ...
    console.log(response.data?.result?.BalanceList[0]?.limitObject?.RMSSubLimits.marginUtilized)
    let position = response.data?.result?.BalanceList[0]?.limitObject?.RMSSubLimits?.marginUtilised;
