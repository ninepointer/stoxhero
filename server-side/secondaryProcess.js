...
const {
  subscribeInstrument,
  getXTSTicksForUserPosition,
  onDisconnect,
  getXTSTicksForCompanySide,
...
const {
  ...
  sendMyRankData,
  emitServerTime,
} = require("./controllers/dailyContestTradeController");
const {
  sendMyRankDataBattle,
  sendLeaderboardDataBattle,
} = require("./controllers/battles/battleTradeController");
...
async function secondaryProcess() {
  await setIOValue();
  const io = getIOValue();
  ...
  getKiteCred.getAccess().then(async (data) => {
    ...
    io.on("connection", async (socket) => {
      ...
      socket.on("userId", async (data) => {
        console.log("data in user id", data);
        socket.join(`${data}`);
      ...
      socket.on("company-ticks", async (data) => {
        ...
        if (
          setting?.ltp == zerodhaAccountType ||
          setting?.complete == zerodhaAccountType
        ) {
...
module.exports = { secondaryProcess };
