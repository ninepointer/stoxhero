...
const StockTrader = require("../../models/mock-trade/stockSchema")
...
const paperTradeMod = async () => {
  ...
  const tradeObjects = data.map((item)=>{
    return {
      ...
      instrumentToken: item?.instrumentToken, portfolioId: item?.portfolioId, exchangeInstrumentToken: item?.exchangeInstrumentToken,
      createdBy:system?._id, trader: item?.userId, amount: (Number(-item?.runningLots) * pricesByTokens[item?.instrumentToken.toString()]), 
      trade_time: new Date(new Date().getTime() + (5*60 + 30) * 60 * 1000),
    }
  });
  // console.log('tradeObjects', tradeObjects);
  
  await takePaperTrades(tradeObjects);
...
module.exports = { paperTradeMod, stockTradeMod, dailyContestSingleMockMod, dailyContestMock, tenx, paperTrade, infinityTrade, internship, infinityTradeLive, contestTradeLive, internshipTradeMod, dailyContestMockMod, marginXMockMod, battleTradeMod };
