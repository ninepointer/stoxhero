...
exports.editCouponCode = async (req, res) => {
  try {
    ...
    coupon = await Coupon.findOneAndUpdate({ _id: id }, updateFields, {
      new: true,
    });
  ...
  try {
    const activeCoupons = await Coupon.find({
      ...
      .populate(
        ...
        "first_name last_name email mobile joining_date creationProcess"
      )
      .populate(
        "usedBy.user",
        "first_name last_name email mobile creationProcess joining_date"
      )
...
exports.verifyCouponCode = async (req, res) => {
  try {
    ...
    if (!coupon) {
      ...
      if (affiliatePrograms.length != 0) {
        for (program of affiliatePrograms) {
          match = program?.affiliates?.some(
            (item) =>
              item?.affiliateCode.toString() == code?.toString() &&
          ...
          if (match) {
            ...
            if (
              ...
              !program?.eligiblePlatforms.includes(platform)
            ) {
              return res.status(400).json({
                status: "error",
            ...
            if (
              program?.eligibleProducts?.length != 0 &&
              !program?.eligibleProducts.includes(product)
            ) {
              return res.status(400).json({
                status: "error",
                message:
                  "This coupon is not valid for the product you're purchasing.",
              });
            }
      ...
      if (!match) {
        ...
        if (userCoupon) {
          if (
            referralProgram?.affiliateDetails?.eligiblePlatforms?.length != 0 &&
            !referralProgram?.affiliateDetails?.eligiblePlatforms.includes(
              platform
            )
          ) {
            return res.status(400).json({
          ...
          //check for eligible products
          if (
            referralProgram?.affiliateDetails?.eligibleProducts?.length != 0 &&
            !referralProgram?.affiliateDetails?.eligibleProducts.includes(
              product
            )
          ) {
            return res.status(400).json({
    ...
    if (
      coupon?.eligiblePlatforms?.length != 0 &&
      !coupon?.eligiblePlatforms.includes(platform)
    ) {
      return res.status(400).json({
        status: "error",
        message: "This coupon is not valid for your device platform",
    ...
    if (
      ...
      !coupon?.eligibleProducts.includes(product)
    ) {
      return res.status(400).json({
        status: "error",
...
exports.saveSuccessfulCouponUse = async (
  userId,
  code,
  product,
  specificProduct
  ...
  try {
    const coupon = await Coupon.findOne({ code }).select(
      "usedBy usedBySuccessful"
    );
    if (!coupon) {
    ...
    let userAppliedCoupons = coupon.usedBy.filter(
      (item) => item?.user.toString() === userId.toString()
    );

    if (userAppliedCoupons.length > 0) {
      const lastAppliedIndex = coupon.usedBy.findIndex(
        (item) =>
          item._id.toString() ===
          userAppliedCoupons[userAppliedCoupons.length - 1]._id.toString()
      );
...
async function getCollectionAndData(
  productName,
  specificProduct,
  coupon,
  userId,
  ...
  switch (productName) {
    case "TenX":
      const subscription = await TenX.findById(specificProduct).select(
        "discounted_price plan_name users"
      );
      participants = subscription?.users;
      participant = participants?.find(
        (elem) =>
          elem?.userId?.toString() == userId?._id?.toString() &&
          Math.abs(
            moment(appliedOn).diff(elem?.subscribedOn, "seconds", true)
          ) <= 5
      );
      ...
      gstAmount =
        (gstPercentage / 100) *
        (participant?.fee - participant?.bonusRedemption ?? 0);
      return {
        name: subscription?.plan_name,
        price: participant?.actualPrice ?? subscription?.discounted_price,
        discountAmount:
          participant?.actualPrice -
          participant?.fee +
          (participant?.bonusRedemption ?? 0)
            ? participant?.actualPrice -
              participant?.fee -
              (participant?.bonusRedemption ?? 0)
            : discountAmount,
      ...
      break;
    case "MarginX":
      const marginx = await MarginX.findById(specificProduct)
        .populate("marginXTemplate", "entryFee")
        .select("marginXName marginXTemplate participants");
      ...
      break;
    case "TestZone":
      const contest = await DailyContest.findById(specificProduct).select(
        "contestName entryFee participants"
      );
      ...
      participant = participants?.find(
        (elem) =>
          elem?.userId?.toString() == userId?._id?.toString() &&
          Math.abs(
            moment(appliedOn).diff(elem?.participatedOn, "seconds", true)
          ) <= 5
      );
  ...
  try {
    ...
    for (let coupon of coupons) {
      ...
      for (let i = 0; i < coupon.usedBySuccessful.length; i++) {
        ...
        const cacheKey = `${
          usedBySuccessful.product.productName
        }_${usedBySuccessful?.specificProduct?.toString()}`;
        if (specificProductCache.has(cacheKey)) {
          couponObj.usedBySuccessful[i].specificProductDetail =
            specificProductCache.get(cacheKey);
        } else {
          // Call getCollectionAndData function to get specific product details
          const specificProductDetail = await getCollectionAndData(
            usedBySuccessful.product.productName,
