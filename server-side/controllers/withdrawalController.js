...
exports.uploadToAzure = async (req, res, next) => {
  ...
  try {
    ...
    req.uploadUrl = blockBlobClient.url;
    next();
  } catch (err) {
    console.error(err);
...
exports.createWithdrawal = async (req, res, next) => {
  ...
  if (!user.KYCStatus || user?.KYCStatus != "Approved") {
    return res
      ...
      .json({
        status: "error",
        message: "KYC not completed. Complete KYC and try again.",
      });
  }
  if (!amount) {
    return res
      .status(400)
      .json({ status: "error", message: "Enter a valid amount" });
  }
  ...
  if (withdrawals.length > 0) {
    return res
      ...
      .json({
        status: "error",
        message:
          "Only one withdrawal can be made in a day. Try again tomorrow.",
      });
  }
  ...
  if (amount > walletBalance) {
    return res
      ...
      .json({
        status: "error",
        message: "You don't have enough funds for the withdrawal.",
      });
  }
  if (walletBalance - amount < (appSettings?.minWalletBalance ?? 200)) {
    return res
      ...
      .json({
        status: "error",
        message: `Your minimum wallet balance should be ${appSettings?.minWalletBalance}`,
      });
  }
  ...
  if (amount < appSettings.minWithdrawal) {
    return res
      ...
      .json({
        status: "error",
        message: `The minimum amount that can be withdrawn is ₹${appSettings.minWithdrawal}`,
      });
  }
  ...
  if (user?.role?.toString != "65dc6817586cba2182f05561") {
    if (walletBalance >= appSettings?.walletBalanceUpperLimit) {
      if (
        amount > walletBalance - appSettings?.maxWithdrawalHigh &&
        amount > appSettings?.maxWithdrawal
      ) {
        return res
          .status(400)
          .json({
            status: "error",
            message: `The maximum amount that can be withdrawn is ₹${Math.max(
              walletBalance - appSettings?.maxWithdrawalHigh,
              appSettings?.maxWithdrawal
            ).toFixed(2)}`,
          });
      }
    } else {
      if (amount > appSettings.maxWithdrawal) {
        return res
          ...
          .json({
            status: "error",
            message: `The maximum amount that can be withdrawn is ₹${appSettings.maxWithdrawal}`,
          });
      }
    }
  }
  ...
  userWallet.transactions.push({
    title: `Withdraw`,
    description: `User initiated withdrawal from wallet of ₹${amount} to bank account - Withdrawal #${
      withdrawalCount + 1
    }`,
    amount: -amount,
    transactionId,
  ...
  if (user?.fcmTokens?.length > 0) {
    await sendMultiNotifications(
      ...
      `User initiated withdrawal from wallet of ₹${amount} to bank account - Withdrawal #${
        withdrawalCount + 1
      }`,
  ...
  res
    .status(201)
    .json({ status: "success", message: "Withdrawal request successful" });
};
...
exports.getPendingWithdrawals = async (req, res, next) => {
  try {
                  ...
                  $sum: {
                    $map: {
                      ...
                      in: {
                        $cond: [
                          {
                            $eq: ["$$transaction.transactionType", "Cash"],
                          },
                          "$$transaction.amount",
                          0,
                        ],
    ...
    res
      .status(200)
      .json({ status: "success", data: pendingWithdrawals, results: count });
  } catch (e) {
    console.log(e);
    res.status(500).json({ status: "error", message: "Something went wrong" });
  }
...
exports.getRejectedWithdrawals = async (req, res, next) => {
  try {
    ...
    const rejectedWithdrawals = await Withdrawal.find({
      ...
      .populate(
        "user",
        "first_name last_name mobile upiId bankName accountNumber ifscCode googlePay_number phonePe_number payTM_number nameAsPerBankAccount"
      )
      .sort({ _id: -1 })
      .skip(skip)
      .limit(limit);
    res
      .status(200)
      .json({ status: "success", data: rejectedWithdrawals, results: count });
  } catch (e) {
    console.log(e);
    res.status(500).json({ status: "error", message: "Something went wrong" });
  }
...
exports.getInitiatedWithdrawals = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ status: "success", data: initiatedWithdrawals, results: count });
  } catch (e) {
    console.log(e);
    res.status(500).json({ status: "error", message: "Something went wrong" });
  }
...
exports.processWithdrawal = async (req, res, next) => {
  ...
  try {
    ...
    if (withdrawal.withdrawalStatus == "Initiated") {
      return res
        .status(400)
        .json({ status: "error", message: "Already initiated" });
    }
    ...
    res
      .status(200)
      .json({ status: "success", message: "Withdrawal Initiated" });
    if (process.env.PROD == "true") {
      try {
        await sendMail(
          user?.email,
          "Withdrawal Request Initiated - StoxHero",
          `
                <!DOCTYPE html>
                <html>
                <head>
...
exports.rejectWithdrawal = async (req, res, next) => {
  ...
  try {
    ...
    if (!withdrawal)
      return res
        .status(404)
        .json({ status: "error", message: "No withdrawal found." });
    if (withdrawal.withdrawalStatus == "Rejected")
      return res
        .status(404)
    ...
    if (
      withdrawal?.actions[withdrawal?.actions?.length - 1]?.actionStatus ==
      "Processing"
    ) {
      withdrawal.actions.push({
        actionDate: new Date(),
    ...
    const walletTxnObj = userWallet?.transactions?.filter(
      (elem) => elem?.transactionId == walletTxnId
    );
    const withdrawalNo = walletTxnObj[0]?.description?.split("#")[1];
    const withdrawalTransaction = userWallet?.transactions?.find(
      (item) => item?.transactionId == withdrawal?.walletTransactionId
    );
    if (withdrawalTransaction)
      withdrawalTransaction["transactionStatus"] = "Failed";
    const user = await User.findById(withdrawal.user).select(
      "email first_name last_name fcmTokens"
    );
    userWallet?.transactions?.push({
      title: `Refund`,
...
exports.approveWithdrawal = async (req, res, next) => {
  ...
  try {
    ...
    if (withdrawal.withdrawalStatus == "Processed") {
      return res
        .status(400)
        .json({ status: "error", message: "This request has been resolved." });
    }
    ...
    if (withdrawalTransaction)
      withdrawalTransaction["transactionStatus"] = "Completed";
    ...
    if (user?.fcmTokens?.length > 0) {
      await sendMultiNotifications(
        ...
        `₹${withdrawal?.amount?.toFixed(
          2
        )} is successfully deposited in your bank account.`,
        user?.fcmTokens?.map((item) => item.token),
        null,
        { route: "wallet" }
      );
    ...
    res
      .status(200)
      .json({ status: "success", message: "Withdrawal request approved" });
  } catch (e) {
    console.log(e);
    res.status(500).json({ status: "error", message: "Something went wrong" });
    await session.abortTransaction();
