...
client8
  .connect()
  .then((res) => {
    console.log("redis connected of client8", res);
  })
  .catch((err) => {
  ...
  try {
    ...
    if (referredBy && !workshop) {
      if (match) {
        ...
        if (referrerCode) {
          const saveReferrals = await User.findOne({
            myReferralCode: referrerCode,
          }).select("referrals");
            ...
            { myReferralCode: referrerCode },
            {
              $push: {
                referrals: {
                  ...
                  referralEarning:
                    saveReferrals?.referrals?.length <=
                    (referralProgramme?.maxReferralsPayoutCap ?? 3000)
                      ? referralProgramme.rewardPerReferral
                      : 0,
