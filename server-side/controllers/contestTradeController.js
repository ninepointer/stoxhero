...
exports.newTrade = async (req, res, next) => {
  ...
  const brokerageDetailBuy = await BrokerageDetail.find({
    transaction: "BUY",
    type: "Option",
  });
  ...
  ContestTrade.findOne({ order_id: order_id })
    .then(async (dateExist) => {
      ...
      contestTrade
        .save()
        .then(async () => {
          const newredisClient = await client.set(
            `${trader.toString()} ${contestId.toString()}`,
            JSON.stringify(contestTrade)
          );
          //console.log("userid check", `${trader.toString()} ${contestId.toString()} pnl`)
          if (
            await client.exists(
              `${trader.toString()} ${contestId.toString()} pnl`
            )
          ) {
            ...
            const matchingElement = pnl.find(
              (element) =>
                element._id.instrumentToken === contestTrade.instrumentToken &&
                element._id.product === contestTrade.Product
            );
          ...
          if (await client.exists(`${contestId.toString()} allranks`)) {
            ...
            const matchingUserElem = ranks.find(
              (element) =>
                element.userId.instrumentToken ===
                  contestTrade.instrumentToken &&
                element.userId.product === contestTrade.Product &&
                element.userId.trader.toString() ===
                  contestTrade.trader.toString()
            );

            if (matchingUserElem) {
              // Update the values of the matching element with the values of the first document
              matchingUserElem.totalAmount += contestTrade.amount * -1;
              matchingUserElem.investedAmount +=
                contestTrade.amount >= 0 && contestTrade.amount;
...
exports.getUserTrades = async (req, res, next) => {
  ...
  try {
    const userTrades = await ContestTrade.find({
      contestId: id,
...
exports.getContestPnl = async (req, res, next) => {
  ...
  try {
    if (
      await client.exists(`${userId.toString()} ${contestId.toString()} pnl`)
    ) {
      let pnl = await client.get(
        `${userId.toString()} ${contestId.toString()} pnl`
      );
      pnl = JSON.parse(pnl);
...
exports.getContestRank = async (req, res, next) => {
  ...
  try {
    ...
    if (!ranks) {
      return res
        .status(404)
        .json({ status: "error", message: "No ranking for the contest" });
    }

    res
      .status(200)
      .json({ status: "success", results: ranks.length, data: ranks });
  } catch (e) {
    console.log(e);
...
exports.getMyContestRank = async (req, res, next) => {
  ...
  try {
    ...
    const user = ranks.filter(
      (obj) => obj.userId.trader.toString() === userId.toString()
    );
    ...
    if (index == -1) {
      return res
        .status(404)
        .json({ status: "error", message: "User doesn't have a rank" });
    }

    res
      .status(200)
      .json({ status: "success", data: { rank: index + 1, data: user } });
  } catch (e) {
    res.status(500).json({ status: "error", message: "Something went wrong" });
  }
};
...
exports.autoTradeContest = async (req, res, next) => {
  ...
  const userIds = await contests.map(async (contest) => {
    ...
    return contest.participants.map(async (participant) => {
      ...
      let employeeidObj = await client.get(
        `${contest._id.toString()}employeeid`
      );
      ...
      if (!(await client.exists(`leaderboard:${contest._id}`))) {
        ...
        try {
          await updateParticularUserRank(contest, participant, employeeidObj);
          await client.del(
            `${employeeidObj[participant.userId.toString()]} investedAmount`
          );
        } catch (err) {
          console.log(err);
        }
  ...
  async function updateParticularUserRank(contest, participant, employeeidObj) {
    ...
    const leaderBoardRank = await client.ZREVRANK(
      `leaderboard:${contest._id}`,
      JSON.stringify({ name: employeeidObj[participant.userId.toString()] })
    );
    const leaderBoardScore = await client.ZSCORE(
      `leaderboard:${contest._id}`,
      JSON.stringify({ name: employeeidObj[participant.userId.toString()] })
    );

    const investedAmount = await client.get(
      `${employeeidObj[participant.userId.toString()]} investedAmount`
    );
    let obj = {
    ...
    try {
      ...
      if (
        leaderBoardRank + 1 <=
        contest.rewards[contest.rewards.length - 1].rankEnd
      ) {
        const wallet = await userWallet.findOne({ userId: participant.userId });
        for (reward of contest.rewards) {
          if (
            leaderBoardRank + 1 <= reward.rankEnd &&
            leaderBoardRank + 1 >= reward.rankStart
          ) {
...
exports.getTradeByPagination = async (req, res, next) => {
  ...
  try {
    const lastTrade = await ContestTrade.find(
      { trader: userId, contestId: contestId },
      {
        symbol: 1,
        buyOrSell: 1,
        Product: 1,
...
exports.countTrades = async (req, res, next) => {
  ...
  const contestId = req.params.id;
  try {
    const count = await ContestTrade.countDocuments({
      trader: userId,
      contestId: contestId,
    });
    console.log(count);
  ...
  try {
    ...
    if (await client.exists(`leaderboard:${id}`)) {
      ...
      if (dummyTesting) {
        ...
        const contestInstruments = await ContestInstrument.find({
          "contest.contestId": id,
        }).select("instrumentToken exchange symbol");
        ...
        for (let instrument in response.data.data) {
          livePrices[response.data.data[instrument].instrument_token] =
            response.data.data[instrument].last_price;
        }
      }
      ...
      await client.expire(
        `leaderboard:${id}`,
        appSetting[0].leaderBoardTimming
      );
...
exports.getRedisMyRank = async (req, res, next) => {
  ...
  try {
    if (await client.exists(`leaderboard:${id}`)) {
      const leaderBoardRank = await client.ZREVRANK(
        `leaderboard:${id}`,
        JSON.stringify({ name: req.user.employeeid })
      );
      const leaderBoardScore = await client.ZSCORE(
        `leaderboard:${id}`,
        JSON.stringify({ name: req.user.employeeid })
      ...
      const investedAmount = await client.get(
        `${req.user.employeeid} investedAmount`
      );
...
exports.getHistoryMyRank = async (req, res, next) => {
  ...
  while (
    ...
    Object.keys(contest.participants[0].myRank).length === 0
  ) {
    let contest = await Contest.findOne(
      { _id: contestId, "participants.userId": userId },
      { "participants.$": 1 }
  ...
  if (
    contest?.participants?.length !== 0 &&
    contest.participants[0]?.myRank?.rank &&
    contest.participants[0]?.myRank?.npnl
  ) {
