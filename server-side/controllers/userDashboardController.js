...
exports.getDashboardStats = async (req, res, next) => {
  try {
    ...
    const averageProfit =
      profitValues.reduce((a, b) => a + b, 0) / profitValues.length || 0;
    const averageLoss =
      lossValues.reduce((a, b) => a + b, 0) / lossValues.length || 0;
    const portfolio = await Portfolio.findById(
      result[result.length - 1]?.portfolio
    );
    ...
    const medianProfit =
      profitValues?.length % 2 === 0
        ? (profitValues[Math.floor(profitValues.length / 2)] +
            profitValues[Math.floor(profitValues.length / 2 - 1)]) /
          2
        : profitValues[Math.floor(profitValues.length / 2)];
    const medianLoss =
      lossValues.length % 2 === 0
        ? (lossValues[Math.floor(lossValues.length / 2)] +
            lossValues[Math.floor(lossValues.length / 2 - 1)]) /
          2
        : lossValues[Math.floor(lossValues.length / 2)];
    const maxProfitOpeningBalance =
      maxProfitIndex != 0
        ? portfolio?.portfolioValue ?? 0 + result[maxProfitIndex - 1]?.npnl
        : portfolio?.portfolioValue ?? 0 + result[maxProfitIndex]?.npnl;
    const maxLossOpeningBalance =
      maxLossIndex != 0
        ? portfolio?.portfolioValue ?? 0 + result[maxLossIndex - 1]?.npnl
        : portfolio.portfolioValue ?? 0 + result[maxLossIndex];
    ...
    const data = {
      ...
      totalBrokerage: result.reduce(
        (total, trade) => total + trade.total_brokerage,
        0
      ),
      totalNPNL: result.reduce((total, trade) => total + trade.npnl, 0),
      totalTrades: result.reduce(
        (total, trade) => total + trade.number_of_trades,
        0
      ),
      ...
      maxProfitDayProfitPercent:
        (maxProfitDay?.npnl / maxProfitOpeningBalance) * 100,
      maxLossDayLossPercent:
        Math.abs(maxLossDay?.npnl / maxLossOpeningBalance) * 100,
    };
...
exports.getDashboardStatsContest = async (req, res, next) => {
  try {
    ...
    const result = await ContestTrade.aggregate([
      ...
      },
      {
        $project: {
          date: "$_id.trade_time",
          contest: {
            $arrayElemAt: ["$contestDetails.contestName", 0],
          },
          contestPayOut: {
            $arrayElemAt: ["$contestDetails.payoutPercentage", 0],
          },
          npnl: "$npnl",
          portfolio: {
            $arrayElemAt: ["$portfolioDetails.portfolioValue", 0],
          },
        },
      },
      {
    ...
    const contests = await DailyContest.countDocuments({
      contestEndTime: { $lte: endDate.toDate(), $gte: startDate.toDate() },
    });
    ...
    const result = await TenXTrade.aggregate([
      ...
      },
      {
        $project: {
          date: "$_id.trade_time",
          contest: {
            $arrayElemAt: ["$tenx.subscriptionName", 0],
          },
          npnl: "$npnl",
          portfolio: {
            $arrayElemAt: ["$portfolioDetails.portfolioValue", 0],
...
async function countTradingDays(startDate, endDate) {
  ...
  const holidayDates = holidays.map((h) =>
    moment(h.holidayDate).format("YYYY-MM-DD")
  );

  for (let m = moment(start); m.isBefore(end); m.add(1, "days")) {
    if (m.isoWeekday() <= 5 && !holidayDates.includes(m.format("YYYY-MM-DD"))) {
      // Monday to Friday are considered trading days
      count++;
    }
  }
...
exports.getUserSummary = async (req, res, next) => {
  try {
    const userId = req.user._id;
    let endDate =
      new Date().getHours() >= 10 ? moment().subtract(1, "days") : moment();
    ...
    let tenxReturn = tenxData
      .map((item) => item.npnl / item.portfolio)
    ...
    const user = await UserDetails.findById(userId)
      .populate({
        ...
        populate: {
          // nested populate for 'portfolio' in 'subscriptionId'
          path: "portfolio",
    ...
    let totalTenXPortfolioValue = user.subscription.reduce(
      (total, subscription) => {
    ...
    res.status(200).json({
      status: "success",
      data: {
        totalTenXPortfolioValue,
        tenxData: tenxData[0] ?? {},
        virtualData: virtualData[0] ?? {},
        contestReturn,
        tenxReturn,
      },
    });
  ...
  try {
    ...
    for (let i = 0; i < tradeData.length; i++) {
      if (i == 0) {
        ...
        tradeData[i].expected_pnl = cumulativeNpnl / i;
        tradeData[i].expected_avg_profit =
          countPositiveNpnl > 0 ? sumPositiveNpnl / countPositiveNpnl : 0;
        tradeData[i].expected_avg_loss =
          countNegativeNpnl > 0 ? sumNegativeNpnl / countNegativeNpnl : 0;
        tradeData[i].riskRewardRatio = riskRewardRatio; // assign previous day's riskRewardRatio
      }
      ...
      if (i > 0) {
        const avgPositiveNpnl =
          countPositiveNpnl > 0 ? sumPositiveNpnl / countPositiveNpnl : 0;
        const avgNegativeNpnl =
          countNegativeNpnl > 0 ? sumNegativeNpnl / countNegativeNpnl : 0;
        riskRewardRatio =
          avgNegativeNpnl !== 0
            ? avgPositiveNpnl / Math.abs(avgNegativeNpnl)
            : 0;
      }
    }
    ...
    if (tradeData.length > 0) {
      ...
      tradeData[tradeData.length - 1].expected_avg_profit =
        countPositiveNpnl > 0 ? sumPositiveNpnl / countPositiveNpnl : 0;
      tradeData[tradeData.length - 1].expected_avg_loss =
        countNegativeNpnl > 0 ? sumNegativeNpnl / countNegativeNpnl : 0;
    }
