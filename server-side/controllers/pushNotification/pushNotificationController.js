...
exports.sendGroupNotifications = async (req, res, next) => {
    ...
    try {
        ...
        const mediaImage = req.files["notificationImage"]?.[0] && (await getAwsS3Key(req.files["notificationImage"]?.[0])).url;
        const csvFileUrl = req.files["csvFile"]?.[0] && await getAwsS3Key(req.files["csvFile"]?.[0]);
        let getMobile = [];

        if (csvFileUrl) {
            const fileIs = await downloadCsvFromS3(csvFileUrl);
            getMobile = await extractMobileNumbersFromCsv(fileIs);
        }
