...
exports.createPayment = async (req, res, next) => {
  ...
  try {
    if (await Payment.findOne({ transactionId: transactionId }))
      return res.status(400).json({ info: "This payment is already exists." });
    ...
    await createUserNotification(
      {
        ...
        description: `₹${amount?.toFixed(
          2
        )} credited in your wallet as wallet top up`,
    ...
    if (user?.fcmTokens?.length > 0) {
      await sendMultiNotifications(
        "Amount Credited in Your Wallet - Topup",
    ...
    try {
      if (process.env.PROD == "true") {
        whatsAppService.sendWhatsApp({
          ...
          campaignName: "wallet_credited_campaign",
          userName: user.first_name,
          source: user.creationProcess,
          templateParams: [
            user.first_name,
            amount.toLocaleString("en-IN"),
            (totalCashAmount + amount).toLocaleString("en-IN"),
...
exports.getSuccessfulPayment = async (req, res, next) => {
  ...
  const payment = await Payment.find({ paymentStatus: "succeeded" })
    .select(
      "_id paymentTime transactionId amount paymentBy paymentMode paymentStatus currency createdOn gatewayResponse utr"
    )
  ...
  res
    .status(201)
    .json({
      message: "successful payment retreived",
      data: payment,
      count: count,
    });
};
...
exports.getPayment = async (req, res, next) => {
  ...
  const payment = await Payment.find()
    .select(
      "_id paymentTime transactionId amount paymentBy paymentMode paymentStatus currency createdOn utr"
    )
    .populate("paymentBy", "first_name last_name email mobile")
    .sort({ _id: -1 })
    .skip(skip)
  ...
  res
    .status(201)
    .json({ message: "payment retreived", data: payment, count: count });
};

exports.getUserPayment = async (req, res, next) => {
  const id = req.params.id;
  try {
    const userPayment = await Payment.findOne({ userId: id }).select(
      "_id paymentTime transactionId amount paymentBy paymentMode paymentStatus currency utr"
    );
    res
      .status(201)
      .json({ message: "userPayment Retrived", data: userPayment });
  } catch {
    (err) => {
      res.status(401).json({ message: "err userPayment", error: err });
    };
  }
};
...
exports.initiatePayment = async (req, res) => {
  ...
  let merchantId =
    process.env.PROD == "true"
      ...
      : process.env.PHONEPE_MERCHANTID_STAGING;
  let merchantTransactionId = generateUniqueTransactionId();
  let merchantUserId = "MUID" + req.user._id;
  let redirectUrl =
    process.env.PROD == "true"
      ? `https://stoxhero.com/paymentstatus?merchantTransactionId=${merchantTransactionId}&redirectTo=${redirectTo}`
      : `http://43.204.7.180/paymenttest/status?merchantTransactionId=${merchantTransactionId}&redirectTo=${redirectTo}`;
  let callbackUrl =
    process.env.PROD == "true"
      ? "https://app.stoxhero.com/api/v1/payment/callback"
  ...
  const payment = await Payment.create({
    ...
    gstAmount:
      ...
      amount /
        100 /
        (1 +
          (setting[0]?.gstPercentage == 0
            ? 0
            : setting[0]?.gstPercentage / 100)),
  ...
  const saltKey =
    process.env.PROD == "true"
      ? process.env.PHONEPE_KEY
      : process.env.PHONEPE_KEY_STAGING; // This should be stored securely, not hardcoded
  const saltIndex = "1";
  const toHash = `${encodedPayload}/pg/v1/pay${saltKey}`;

  const checksum =
    crypto.createHash("sha256").update(toHash).digest("hex") +
    "###" +
    saltIndex;
...
exports.handleCallback = async (req, res, next) => {
  ...
  try {
    ...
    const decodedString = Buffer.from(encodedResponse, "base64").toString(
      "utf8"
    );
    ...
    if (!decodedResponse) {
      ...
      if (decodedResponse.data.amount !== payment.amount * 100) {
        return res
          .status(400)
          .json({ status: "error", message: "Amount mismatch" });
      }
      ...
      if (decodedResponse.code === "PAYMENT_SUCCESS") {
        ...
        await addMoneyToWallet(
          payment.amount - payment?.gstAmount,
          payment?.paymentBy
        );
        ...
        res
          .status(200)
          .json({ status: "success", message: "Payment was successful" });
        ...
        res
          .status(400)
          .json({ status: "error", message: "Unknown response code" });
      }
    }
  } catch (error) {
...
const SALT_KEY =
  process.env.PROD == "true"
    ? process.env.PHONEPE_KEY
    : process.env.PHONEPE_KEY_STAGING; // You may want to keep this in a secure environment variable or secret management tool.
...
const addCashback = async (amount, userId, coupon) => {
  const wallet = await UserWallet.findOne({ userId: userId });
  const couponDoc = await Coupon.findOne({ code: coupon }).select(
    "rewardType discountType discount maxDiscount"
  );
...
const sendWhatsAppNotification = async (payment) => {
  ...
  const user = await User.findOne({ _id: new ObjectId(paymentBy) }).select(
    "first_name creationProcess last_name mobile fcmTokens"
  );
  if (user?.fcmTokens?.length > 0) {
    await sendMultiNotifications(
...
const participateUser = async (
  ...
  switch (paymentFor) {
    case "Contest":
      if (productId) {
        const contest = await Contest.findById(productId).select(
          "entryFee contestName"
        );
        await handleSubscriptionDeduction(
          paymentBy,
          amount,
          contest?.contestName,
          contest?._id,
    ...
    case "TenX":
      if (productId) {
        const tenx = await TenX.findById(productId).select(
          "discounted_price plan_name"
        );
        await handleDeductSubscriptionAmount(
          paymentBy,
          amount,
          tenx?.plan_name,
          tenx?._id,
    ...
    case "MarginX":
      if (productId) {
        const marginX = await MarginX.findById(productId).populate(
          "marginXTemplate",
          "entryFee"
        );
        await handleDeductMarginXAmount(
          paymentBy,
          amount,
          marginX?.marginXName,
          marginX?._id,
    ...
    case "Course":
      if (productId) {
        const course = await Course.findById(productId).select(
          "courseName _id"
        );
        await handleDeductCourseFee(
          paymentBy,
          amount,
          course?.courseName,
          course?._id,
