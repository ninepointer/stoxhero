...
exports.createTradingHoliday = async (req, res, next) => {
  ...
  if (
    await TradingHoliday.findOne({
      holidayDate: { $gte: startTimeOfDay, $lte: endTimeOfDay },
    })
  ) {
  ...
  res.status(201).json({
    status: "success",
    message: "Trading Holiday successfully created.",
    data: holiday,
  });
};
...
exports.getTradingHolidays = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ status: "success", data: holiday, results: holiday.length });
  } catch (e) {
...
exports.getPastTradingHolidays = async (req, res, next) => {
  ...
  try {
    const holiday = await TradingHoliday.find({
      holidayDate: { $lt: beforeYesterday },
    }).sort({ holidayDate: -1 });
    res
      .status(200)
      .json({ status: "success", data: holiday, results: holiday.length });
...
exports.editTradingHoliday = async (req, res, next) => {
  ...
  const updated = await TradingHoliday.findByIdAndUpdate(id, filteredBody, {
    new: true,
  });

  res
    .status(200)
    .json({ message: "Successfully edited Trading Holiday.", data: updated });
};

exports.deleteTradingHoliday = async (req, res, next) => {
  const id = req.params.id;

  const history = await TradingHoliday.findOneAndUpdate(
    { _id: id },
    {
      $set: {
        isDeleted: true,
  ...
  res
    .status(200)
    .json({ message: "Successfully deleted trading holiday.", data: holiday });
};

exports.getTradingHolidayBetweenDates = async (req, res, next) => {
  let { startDate, endDate } = req.params;
  const newstartDate = moment(new Date(startDate));
  const newendDate = moment(new Date(endDate));

  let startDateDateComponent = newstartDate
    .clone()
    .startOf("day")
    .subtract(5, "hours")
    .subtract(30, "minutes");
  let endDateDateComponent = newendDate
    .clone()
    .endOf("day")
    .subtract(5, "hours")
    .subtract(30, "minutes");
    ...
    for (let i = 1; i < 30; i++) {
      ...
      const holiday = await TradingHoliday.find({
        holidayDate: {
          ...
          $lte: new Date(
            `${endOfTomorrow.toISOString().split("T")[0]}T23:59:00.000Z`
          ),
        },
      });
