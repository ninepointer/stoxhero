...
exports.overallLivePnlMismatchToday = async(req, res, next)=>{
  let {id} = req.params;
  let date = new Date();
  let todayDate = `${(date.getFullYear())}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`
  todayDate = "2023-08-28" + "T00:00:00.000Z";
  const today = new Date(todayDate);    
  let pnlLiveDetails = await DailyContestLiveCompany.aggregate([
    // {
    //   $match: {
...
exports.DailyContestPnlTWise = async (req, res, next) => {
  ...
  let user = await DailyContestLiveUser.aggregate(pipeline1)
  let x = await DailyContestLiveCompany.aggregate(pipeline)
...
exports.overallDailyContestCompanySidePnlThisMonth = async (req, res, next) => {
  ...
  // console.log("Month Start Date:",monthStartDate, yesterdayDate)
