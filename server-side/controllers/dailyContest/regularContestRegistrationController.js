...
exports.generateOTP = async (req, res, next) => {
    ...
    const { firstName, lastName, email, mobile, contest, campaignCode, referrerCode,
    } = req.body

    if (!contest) {
        return res.status(400).json({
            message: "This Contest doesn't exist!",
            status: 'error'
        });
    }
    ...
    if (!existingUser) {
        ...
        try {
            let obj = {
                ...
                status: 'Active',
                employeeid: userId,
                creationProcess: 'Contest SignUp',
            ...
            if (referrerCode) {
                if (match) {
                        ...
                        { _id: new ObjectId(affiliateObj?._id) },
                        {
                            $push: {
                                referrals: {
                                    userId: newuser._id,
                                    joinedOn: new Date(),
                                    affiliateUserId: referralUser?._id
                                }
                            }
                        },
                    ...
                    const referralProgramme = await ReferralProgram.findOneAndUpdate({ status: "Active" }, {
                        $push: {
                            users: {
                                userId: newuser._id, 
                                joinedOn: new Date()
