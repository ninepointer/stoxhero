...
exports.getInternshipTradersDateWiseStats = async(req, res)=>{
  ...
  let pnlDetails = await InternshipTrader.aggregate([
      { $match: { trade_time: {$gte : fromDate, $lte : toDate}, trader: new ObjectId(id), batch: new ObjectId(batchId), status: "COMPLETE"} },
...
exports.getInternshipTradersMonthlyPnlData= async(req,res,next) => {
  ...
  // console.log(pastYear,today);
  let pnlDetails = await InternshipTrader.aggregate([
      { $match: { trade_time: {$gte : pastYear, $lte : today}, trader: new ObjectId(id), batch: new ObjectId(batchId), status: "COMPLETE"} },
