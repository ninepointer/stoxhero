...
const LearningModule = require('../../models/learningModules/learningModule');
...
exports.createLearningModule = async (req, res) => {
    ...
    try {
        const {moduleNumber, moduleName, moduleColor, moduleDescription, moduleStatus} = req.body;
        if(!moduleName)return res.status(400).json({status: 'error', message: 'Enter all mandatory fields.'})
        const learningModule = await LearningModule.create({
            moduleNumber, moduleName, moduleColor, moduleDescription, moduleStatus,
