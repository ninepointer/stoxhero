...
router.get("/readmocktradeusercountTodaybyemail/:email", (req, res) => {
  ...
  MockTradeDetails.count(
    { order_timestamp: { $regex: todayDate }, userId: email },
    (err, data) => {
      if (err) {
        return res.status(500).send(err);
      } else {
...
router.get(
  "/readmocktradeusertodaydatapagination/:email/:skip/:limit",
  (req, res) => {
...
router.get(
  "/readmocktradeuserpariculardatewithemail/:date/:email",
  (req, res) => {
...
router.get("/pnlcalucationmocktradealluserthismonth", (req, res) => {
  ...
  const { email } = req.params;
  MockTradeDetails.find({
    trade_time: { $gte: monthStartDate, $lt: todayDate, status: "COMPLETE" },
  })
    .then((data) => {
      let overallnewpnl = pnlcalucationnorunninglots(data);
      //console.log(overallnewpnl);
...
router.get("/gettraderwisepnldetailsthismonth", async (req, res) => {
  ...
  var day = new Date();
  let dayDate = `${day.getFullYear()}-${String(day.getMonth() + 1).padStart(
    2,
    "0"
  ...
  let todayDate = `${yesterday.getFullYear()}-${String(
...
router.get(
  "/getoverallpnlmocktradeparticularusertoday/:email",
  async (req, res) => {
    const { email } = req.params;
    let date = new Date();
...
router.get(
  "/getuserreportdatewise/:email/:firstDate/:secondDate",
  async (req, res) => {
    ...
    let pnlDetails = await MockTradeDetails.aggregate([
      {
        $match: {
          trade_time: {
            $gte: `${firstDate} 00:00:00`,
            $lte: `${secondDate} 23:59:59`,
          },
          userId: email,
          status: "COMPLETE",
        },
...
router.get("/gethistorymocktradesparticularuser/:email", async (req, res) => {
  ...
  let pipeline = [
    { $match: { trade_time: { $lt: todayDate }, userId: email } },
    {
      $project: {
        _id: 0,
        trade_time: 1,
...
router.get("/gettopfivelossmakingtradersthismonthmock", async (req, res) => {
  ...
  let now = new Date();
  let nowDate = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(
    2,
    "0"
  ...
  let endDate = `${endDay.getFullYear()}-${String(
    endDay.getMonth() + 1
  ).padStart(2, "0")}-${String(endDay.getDate()).padStart(2, "0")}`;
  ...
  let pipeline = [
    ...
    { $addFields: { npnl: { $subtract: ["$gpnl", "$brokerage"] } } },
    { $sort: { npnl: 1 } },
    { $limit: 5 },
  ];
  ...
  let pipeline = [
    ...
    },
    {
      $group: {
        _id: {
          createdBy: "$createdBy",
          trade_time: { $substr: ["$trade_time", 0, 10] },
        },
...
router.get(
  "/traderwisetraderpnlreport/:startDate/:endDate",
  async (req, res) => {
...
router.get("/datewisetraderpnl/:queryDate/:traderName", async (req, res) => {
  console.log(
    "Inside Aggregate API - Date wise company pnl based on date entered"
  );
...
router.get(
  "/getuserreportdatewisename/:name/:firstDate/:secondDate",
  async (req, res) => {
...
router.get(
  "/getoverallpnlmocktradeparticularusertodaytraderside/:email",
  async (req, res) => {
    const { email } = req.params;
    let date = new Date();
    let todayDate = `${date.getFullYear()}-${String(
...
router.get(
  "/getDayWiseTradersTradeDetails/:startDay/:endDay/",
  async (req, res) => {
