...
router.post("/livetradecompanytemp", async (req, res) => {
  ...
  function buyBrokerage(totalAmount) {
    ...
    let gst =
      (brokerage + exchangeCharge) * (Number(brokerageDetailBuy[0].gst) / 100);
    let sebiCharges =
  ...
  function sellBrokerage(totalAmount) {
    ...
    let gst =
      (brokerage + exchangeCharge) * (Number(brokerageDetailSell[0].gst) / 100);
    let sebiCharges =
  ...
  if (realBuyOrSell === "BUY") {
    brokerageCompany = buyBrokerage(
      Math.abs(Number(realQuantity)) * average_price
    );
  } else {
  ...
  CompanyTradeData.findOne({ order_id: order_id })
    .then((dateExist) => {
      ...
      const companyTradeData = new CompanyTradeData({
        ...
        tradeBy,
        isRealTrade: true,
        amount: Number(realQuantity) * average_price,
        trade_time,
        order_req_time: order_timestamp,
        order_save_time: order_timestamp,
        exchange_order_id,
      });
...
router.post("/mocktradecompany", authoizeTrade.fundCheck, async (req, res) => {
  ...
  MockTradeDetails.findOne({ order_id: order_id })
    .then((dateExist) => {
      if (
        dateExist &&
        dateExist.order_timestamp !== newTimeStamp &&
      ...
      const mockTradeDetails = new MockTradeDetails({
        ...
        uId,
        createdBy,
        average_price: originalLastPriceCompany,
        Quantity: realQuantity,
        Product,
        buyOrSell: realBuyOrSell,
        order_timestamp: newTimeStamp,
        variety,
        validity,
        exchange,
        ...
        order_id,
        instrumentToken: real_instrument_token,
        brokerage: brokerageCompany,
        tradeBy: createdBy,
        isRealTrade: false,
        amount: Number(realQuantity) * originalLastPriceCompany,
    ...
    MockTradeDetailsUser.findOne({ order_id: order_id })
      .then((dateExist) => {
        ...
        const mockTradeDetailsUser = new MockTradeDetailsUser({
          ...
          uId,
          createdBy,
          average_price: originalLastPriceUser,
          Quantity,
          Product,
          buyOrSell,
          order_timestamp: newTimeStamp,
          ...
          brokerage: brokerageUser,
          tradeBy: createdBy,
          amount: Number(Quantity) * originalLastPriceUser,
          trade_time: trade_time,
        });
        ...
        mockTradeDetailsUser
          ...
          .then(() => {
            ...
            res
              .status(201)
              .json({ status: "Complete", message: "Trade Succesful" });
          })
          .catch((err) => {
...
router.get("/readmocktradecompanycountToday", (req, res) => {
  let date = new Date();
  let todayDate = `${String(date.getDate()).padStart(2, "0")}-${String(
    date.getMonth() + 1
  ).padStart(2, "0")}-${date.getFullYear()}`;

  MockTradeDetails.count(
    { order_timestamp: { $regex: todayDate } },
    (err, data) => {
      if (err) {
        return res.status(500).send(err);
      } else {
...
router.get("/readmocktradecompanyYesterday", (req, res) => {
  ...
  let yesterdayDate = `${yesterday.getFullYear()}-${String(
    yesterday.getMonth() + 1
  ).padStart(2, "0")}-${String(yesterday.getDate()).padStart(2, "0")}`;
  //console.log(yesterdayDate);

  MockTradeDetails.count(
    { trade_time: { $regex: yesterdayDate } },
    (err, data) => {
      if (err) {
        return res.status(500).send(err);
      } else {
...
router.get("/readmocktradecompanyDate", (req, res) => {
  ...
  const { email } = req.params;
  //console.log(todayDate)
  MockTradeDetails.find({ order_timestamp: { $regex: "06-01-2023" } })
    .sort({ trade_time: -1 })
    .then((data) => {
      return res.status(200).send(data);
    })
...
router.get(
  "/readmocktradecompanytodaydatapagination/:skip/:limit",
  (req, res) => {
    let date = new Date();
    let todayDate = `${String(date.getDate()).padStart(2, "0")}-${String(
      date.getMonth() + 1
...
router.get(
  "/readmocktradecompanypariculardatewithemail/:date/:email",
  (req, res) => {
...
router.get("/readmocktradecompanyThisMonth", (req, res) => {
  ...
  const { email } = req.params;
  let monthStart = `${String("01").padStart(2, "0")}-${String(
    date.getMonth() + 1
  ).padStart(2, "0")}-${date.getFullYear()}`;
  //console.log(todayDate)
  // MockTradeDetails.find({order_timestamp: {$regex: todayDate}})
...
router.get("/readmocktradecompanyLastMonth", async (req, res) => {
  ...
  let todayDate = `${
    ...
    date.getFullYear() - 1 == date.getFullYear() ? date.getMonth() : "12"
  ).padStart(2, "0")}-${String("01").padStart(2, "0")}`;
  const { email } = req.params;
  let lastmonthEnd = `${
    date.getFullYear() - 1 == date.getFullYear()
      ? date.getFullYear()
      : date.getFullYear() - 1
  }-${String(date.getMonth() != 0 ? date.getMonth() : "12").padStart(
    2,
    "0"
  )}-${String(nodaysinmonth).padStart(2, "0")}`;
...
router.get("/readmocktradecompanyThisWeek/:email", (req, res) => {
  ...
  //console.log(nextDay);
  let nextDate = `${nextDay.getFullYear()}-${String(
    nextDay.getMonth() + 1
  ...
  let weekStartDate = `${weekStartDay.getFullYear()}-${String(
    weekStartDay.getMonth() + 1
  ).padStart(2, "0")}-${String(weekStartDay.getDate()).padStart(2, "0")}`;

  MockTradeDetails.find({
    trade_time: { $gte: weekStartDate, $lt: nextDate },
    userId: email,
  })
    .then((data) => {
      return res.status(200).send(data);
...
router.get("/readmocktradecompanyThisMonth/:email", (req, res) => {
  ...
  let monthStart = `${date.getFullYear()}-${String(
    date.getMonth() + 1
  ).padStart(2, "0")}-01`;
  MockTradeDetails.find({
    trade_time: { $gte: monthStart, $lt: nextDay },
    userId: { $regex: email },
  })
    .then((data) => {
      return res.status(200).send(data);
...
router.get("/readmocktradecompanyThisYear/:email", (req, res) => {
  ...
  //console.log(email);
  MockTradeDetails.find({
    trade_time: { $gte: yearStart, $lt: nextDay },
    userId: email,
  })
    .then((data) => {
      return res.status(200).send(data);
...
router.get("/updatemocktradedatatradetime", async (req, res) => {
  ...
  for (let i = 0; i < datatoupdate.length; i++) {
    ...
    if (!datatoupdate[i].trade_time) {
      ...
      await MockTradeDetails.findByIdAndUpdate(
        datatoupdate[i]._id,
        { trade_time: trade_time },
  ...
  for (let i = 0; i < datatoupdate.length; i++) {
    if (!datatoupdate[i].amount) {
      ...
      await MockTradeDetails.findByIdAndUpdate(
        ...
        {
          amount:
            Number(datatoupdate[i].Quantity) * datatoupdate[i].average_price,
        },
        function (err, amount) {
          if (err) {
            //console.log(err)
...
router.get("/readmocktradecompanytodaycount", (req, res) => {
  ...
  let data = MockTradeDetails.find({
    order_timestamp: { $regex: todayDate },
  }).sort({ trade_time: -1 });
  data.count((err, data) => {
    if (err) {
...
router.get("/readmocktradecompanyagg", async (req, res) => {
  ...
  let yesterdayDate = `${date.getFullYear()}-${String(
    date.getMonth() + 1
  ).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0") - 1}`;
  //$gte : `${todayDate} 00:00:00`,
...
router.get("/getmocktradecompanydetailsthisweek", async (req, res) => {
  ...
  let startdayDate = `${startday.getFullYear()}-${String(
    startday.getMonth() + 1
  ...
  let pipeline = [
    {
      $match: {
        trade_time: {
          $gte: `${startdayDate} 00:00:00`,
          $lte: `${yesterdayDate} 23:59:59`,
        },
        status: "COMPLETE",
      },
    },
...
router.get("/getmocktradecompanydetailsthismonth", async (req, res) => {
  ...
  yesterday.setDate(day.getDate() - 1);
  //console.log("Yesterday "+yesterday);
  let monthStartDate = `${day.getFullYear()}-${String(
    day.getMonth() + 1
  ).padStart(2, "0")}-01`;
...
router.get("/getmocktradecompanydetailslastyear", async (req, res) => {
  ...
  let date = new Date();
  const days = date.getDay();
  let todayDate = `${date.getFullYear() - 1}-${String("01").padStart(
    2,
    "0"
  )}-${String("01").padStart(2, "0")}`;
  //console.log("Today "+todayDate)
  var day = new Date(todayDate);
  //console.log("Day "+day); // Apr 30 2000
  ...
  let yesterdayDate = `${date.getFullYear() - 1}-${String("12").padStart(
    2,
    "0"
...
router.get(
  "/getmocktradecompanydetailsdaybeforeyesterday",
  async (req, res) => {
    //console.log("Inside Aggregate API - Mock Trade Details Yesterday")
...
router.get(
  "/traderwisecompanypnlreport/:startDate/:endDate",
  async (req, res) => {
...
router.get(
  "/getoverallpnlmocktradeparticularusertodaycompanyside/:email",
  async (req, res) => {
    const { email } = req.params;
    let date = new Date();
    let todayDate = `${date.getFullYear()}-${String(
...
router.get(
  "/getoverallpnlmocktradecompanytoday/algowisedata/:algoId",
  async (req, res) => {
    let date = new Date();
...
router.get(
  "/gettraderwisepnlmocktradecompanytoday/algowiseData/:id",
  async (req, res) => {
    let date = new Date();
    const { id } = req.params;
    let todayDate = `${date.getFullYear()}-${String(
...
router.get("/updatealgoid", async (req, res) => {
  ...
  for (let i = 0; i < algoiddoc.length; i++) {
    if (
      !algoiddoc[i].algoBox.isDefault &&
      !algoiddoc[i].algoBox.marginDeduction
    ) {
      // console.log(algoiddoc[i]._id);
      await MockTradeDetails.findByIdAndUpdate(
        algoiddoc[i]._id,
        { "algoBox.isDefault": true, "algoBox.marginDeduction": false },
...
router.get(
  "/getoverallpnlmocktradecompanytoday/batchwisedata/:batchName",
  async (req, res) => {
    let date = new Date();
...
router.get(
  "/gettraderwisepnlmocktradecompanytoday/batchwiseData/:batchName",
  async (req, res) => {
    let date = new Date();
    const { batchName } = req.params;
    let todayDate = `${date.getFullYear()}-${String(
...
router.get(
  "/getDayWiseTradersTradeDetailsCompanySide/:startDay/:endDay/:traderName",
  async (req, res) => {
    ...
    let pipeline = [
      ...
      },
      {
        $group: {
          _id: {
            ...
            email: "$userId",
            joining_date: {
              $arrayElemAt: ["$userDetails.joining_date", 0],
            },
            cohort: {
              $arrayElemAt: ["$userDetails.cohort", 0],
            },
      ...
      },
      {
        $project: {
          ...
          totalDays: 1,
          serial_number: {
            $arrayElemAt: ["$pnl_by_day.serial_number", 0],
          },
          day: {
            $arrayElemAt: ["$pnl_by_day.values.day", 0],
          },
          gpnl: {
            $arrayElemAt: ["$pnl_by_day.values.pnl", 0],
          },
          brokerage: {
            $arrayElemAt: ["$pnl_by_day.values.brokerage", 0],
          },
          npnl: {
            $arrayElemAt: ["$pnl_by_day.values.npnl", 0],
          },
          runningLots: {
            $arrayElemAt: ["$pnl_by_day.values.runningLots", 0],
          },
          lotsUsed: {
            $arrayElemAt: ["$pnl_by_day.values.lotsUsed", 0],
          },
          trades: {
            $arrayElemAt: ["$pnl_by_day.values.trades", 0],
          },
        },
      },
      {
