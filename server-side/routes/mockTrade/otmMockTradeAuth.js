...
router.post("/mockOtmtradecompany", async (req, res) => {
  ...
  if (realBuyOrSell === "BUY") {
    brokerageCompany = buyBrokerage(
      Math.abs(Number(realQuantity)) * originalLastPrice
    );
  } else {
  ...
  if (buyOrSell === "BUY") {
    brokerageUser = buyBrokerage(
      Math.abs(Number(Quantity)) * originalLastPrice
    );
  } else {
  ...
  OtmMock.findOne({ uId: uId })
    .then((dateExist) => {
      ...
      const otmMock = new OtmMock({
        ...
        uId,
        createdBy,
        average_price: otmLastPrice,
        Quantity: otm_quantity,
        Product,
        buyOrSell,
        ...
        order_id,
        instrumentToken: otm_token,
        brokerage: brokerageCompany,
        tradeBy: createdBy,
        isRealTrade: false,
        amount: Number(otm_quantity) * otmLastPrice,
