...
router.post("/addInstrument", authentication, async (req, res) => {
  ...
  try {
    ...
    const setInstrument = JSON.parse(
      (await client.get("instrument-user")) || JSON.stringify({})
    );
    ...
    if (!instrumentToken) {
      return res
        .status(422)
        .json({ error: "Please enter a valid Instrument." });
    }
    // return res.status(422).json({ error: "Any of one feild is incorrect..." })
    // }

    const dataExist = await Instrument.findOne({
      instrumentToken: instrumentToken,
      status: "Active",
    });
    const checkIsAdded = await User.findOne({ _id: _id }).select(
      "watchlistInstruments"
    );

    const match = checkIsAdded.watchlistInstruments.some(
      (item) => dataExist?._id?.toString() === item?.toString()
    );
    if (match) {
      return res.status(201).json({
        status: "error",
        message: "This instrument is already exist in your watchlist.",
      });
    }
    if (dataExist) {
      const updateInstrument = await User.findOneAndUpdate(
        { _id: _id },
        {
          $push: {
      ...
      try {
        if (isRedisConnected) {
          ...
          const newredisClient = await client.SADD(
            _id.toString(),
            JSON.stringify(obj)
          );
          const allinstrument = await client.SADD(
            `${_id.toString()}allInstrument`,
            JSON.stringify(obj)
          );
        }
        ...
        await client.LPUSH(
          `${req.user._id.toString()}: instrument`,
          JSON.stringify({
            _id: dataExist._id,
        ...
        const uniqueUsers = [
          ...new Set(dataExist.users.map((obj) => obj.toString())),
        ];
      ...
      try {
        await client5.PUBLISH(
          "subscribe-single-token",
          JSON.stringify({ instrumentToken })
        );
        ...
        const tradable = await TradableInstrument.findOne({
          instrument_token: instrumentToken,
          status: "Active",
        }).sort({ _id: -1 });
        const addingInstruments = await Instrument.create({
          exchangeInstrumentToken: tradable.exchange_token,
          instrument: tradable.name,
          exchange: tradable.exchange,
          symbol: tradable.tradingsymbol,
          status: "Active",
          chartInstrument: tradable.chartInstrument,
          uId: "abcdef",
          createdBy: _id,
          lastModifiedBy: _id,
          lotSize: tradable.lot_size,
          instrumentToken,
          contractDate: tradable.expiry,
          maxLot: 1800 || tradable.max_lot,
          accountType,
          exchangeSegment: Number(exchangeSegment || 2),
          users: [_id?.toString()],
        });
  ...
  try {
    ...
    if (!exchange || !symbol || !status || !instrumentToken) {
      if (!instrumentToken) {
        return res
          .status(422)
          .json({ status: "error", message: "Instrument is not valid" });
      }
      return res
        .status(422)
    ...
    const dataExist = await EquityInstrument.findOne({
      instrumentToken: instrumentToken,
      status: "Active",
    });
    const checkIsAdded = await User.findOne({ _id: _id }).select(
    ...
    if (dataExist) {
      ...
      try {
        ...
        await client.LPUSH(
          `${req.user._id.toString()}: equity-instrument`,
          JSON.stringify({
            _id: dataExist._id,
            instrument: dataExist.instrument,
            exchange: dataExist.exchange,
      ...
      try {
        ...
        res
          .status(201)
          .json({ status: "success", message: "Instrument Added" });
      } catch (err) {
        console.log(err);
      }
    }
...
router.patch(
  "/inactiveInstrument/:instrumentToken/",
  authentication,
  async (req, res) => {
    const { role } = req.user;
    let isRedisConnected = getValue();
    ...
    try {
      ...
      if (roleObj.roleName === infinityTrader) {
        removeFromWatchlist = await InfinityInstrument.findOne({
          instrumentToken: instrumentToken,
          status: "Active",
        });
      } else {
        removeFromWatchlist = await Instrument.findOne({
          instrumentToken: instrumentToken,
          status: "Active",
        });
      }
      ...
      if (index !== -1 && isRedisConnected) {
        try {
          ...
          let removeInstrumentObject = {
            ...
            exchangeInstrumentToken:
              removeFromWatchlist.exchangeInstrumentToken,
          ...
          if (roleObj.roleName === infinityTrader) {
            removeInstrument = await client.LREM(
              `${_id.toString()}: infinityInstrument`,
              1,
              JSON.stringify(removeInstrumentObject)
            );
            // console.log("in if removeInstrument", removeInstrument)
          } else {
            let instrument = await client.LRANGE(
              `${_id.toString()}: instrument`,
              0,
              -1
            );
          ...
          const redisClient = await client.SREM(
            _id.toString(),
            removeFromSet[0]
          );
      ...
      const removing = await User.findOneAndUpdate(
        { _id: _id },
        {
          $set: {
  ...
  try {
    ...
    if (dailyContest) {
      ...
      let instrument = await Instrument.find({
        ...
        .select(
          "exchangeInstrumentToken instrument exchange symbol status lotSize maxLot instrumentToken contractDate _id chartInstrument"
        )
        .sort({ $natural: -1 });

      const instrumentJSONs = instrument.map((instrument) =>
        JSON.stringify(instrument)
      );
      if (instrumentJSONs.length > 0 && isRedisConnected) {
        await client.LPUSH(`${req.user._id.toString()}: contestInstrument`, [
          ...instrumentJSONs,
        ]);
      }
      res
        .status(200)
        .json({ message: "instruments received", data: instrument });

      // }
    } else {
      if (
        isRedisConnected &&
        (await client.exists(`${req.user._id.toString()}: instrument`))
      ) {
        let instrument = await client.LRANGE(
          `${req.user._id.toString()}: instrument`,
          0,
        ...
        res.status(200).json({
          message: "redis instrument received",
          data: instrumentJSONs,
        });
      } else {
        const user = await User.findOne({ _id: _id });
        ...
        if (instrumentJSONs.length > 0 && isRedisConnected) {
          await client.LPUSH(`${req.user._id.toString()}: instrument`, [
            ...instrumentJSONs,
          ]);
        }
...
router.get("/equityinstrumentDetails", authentication, async (req, res) => {
  ...
  try {
    if (
      isRedisConnected &&
      (await client.exists(`${req.user._id.toString()}: equity-instrument`))
    ) {
      let instrument = await client.LRANGE(
        `${req.user._id.toString()}: equity-instrument`,
        0,
      ...
      res
        .status(201)
        .json({ message: "redis instrument received", data: instrumentJSONs });
    } else {
      const user = await User.findOne({ _id: _id });

      let instrument = await EquityInstrument.find({
        _id: { $in: user.watchlistInstruments },
        status: "Active",
      })
        .select(
          "exchangeInstrumentToken instrument exchange symbol status instrumentToken _id chartInstrument name"
        )
        .sort({ $natural: -1 });
      ...
      // console.log("instrumentJSONs", instrumentJSONs)
      if (instrumentJSONs.length > 0 && isRedisConnected) {
        await client.LPUSH(`${req.user._id.toString()}: equity-instrument`, [
          ...instrumentJSONs,
        ]);
      }
      // console.log("instruments", instruments)
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const user = await User.findOne({ _id: _id });
      const removeFromWatchlist = await EquityInstrument.findOne({
        instrumentToken: instrumentToken,
        status: "Active",
      });
      ...
      if (index !== -1 && isRedisConnected) {
        try {
          ...
          const instrument = await client.LRANGE(
            `${_id.toString()}: equity-instrument`,
            0,
            -1
          );
          const removeInstrument = await client.LREM(
            `${_id.toString()}: equity-instrument`,
            1,
            JSON.stringify(removeInstrumentObject)
          );
          ...
          const redisClient = await client.SREM(
            _id.toString(),
            JSON.stringify(removeFromSet[0])
          );
        } catch (err) {
          console.log(err);
        }
